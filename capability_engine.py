...
READINESS_EVENTS: dict[str, dict] = {
    "escalera_al_infierno": {
        ...
        "tipo": "climbing_race",
        "descripcion": "Carrera de ascenso en montaña",
        "weights": {
            "motor_aerobico": 0.35,
            "escalada": 0.30,
    ...
    "gran_fondo_150": {
        ...
        "descripcion": "Fondo de resistencia en bicicleta",
        "weights": {
            "motor_aerobico": 0.45,
    ...
    "medio_maraton": {
        ...
        "descripcion": "Carrera de media distancia a pie",
        "weights": {
            "motor_aerobico": 0.40,
...
def validate_capability(
    ...
    return {
        ...
        "note": (
            "Primera línea base oficial; los cambios se evaluarán desde la siguiente ejecución."
            if previous is None
            else "Comparación contra la ejecución oficial anterior."
        ),
    }
...
def _capability(
    ...
    return {
        ...
        "limitante_principal": limiting["label"] if limiting else "Datos insuficientes",
        "impulsor_principal": driving["label"] if driving else "Datos insuficientes",
...
def _historical_aerobic_block(
    ...
    indicators = [
        _indicator(
            "efficiency",
            f"Eficiencia {'ciclismo' if sport == 'cycling' else 'running'}",
            current_efficiency,
            reference_efficiency,
            0.40,
...
def _aerobic_capability(conn, snapshots: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    indicators = [
        ...
        _indicator(
            "consistency", "Consistencia de 12 semanas",
            float(sum(1 for row in recent12 if int(row.get("sessions") or 0) > 0)),
            12.0,
            0.25, 1.0, unit="semanas",
        ),
        _indicator(
            "z2", "Tiempo en Z2 Mars", current_z2, reference_z2,
            0.20, z2_confidence, unit="%",
        ),
        _indicator(
            "long_endurance", "Fondo largo", current_long, reference_long,
    ...
    capability = _capability(
        ...
        recommendation=(
            "Consolidar cuatro semanas activas y elevar primero el indicador limitante."
        ),
...
def _weight_capability(snapshots: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    indicators = [
        _indicator(
            "weight_trend", "Tendencia reciente de peso", latest,
            trend_score_reference, 0.40, min(len(recent) / 8, 1), inverse=True, unit="kg",
        ),
        _indicator(
            "personal_range", "Posición en rango histórico personal", latest,
            personal_low, 0.35, min(len(weighted) / 52, 1), inverse=True, unit="kg",
        ),
        _indicator(
            "measurement_adherence", "Continuidad de medición", float(adherence),
            8.0, 0.25, 1.0, unit="semanas",
        ),
    ]
    return _capability(
        ...
        recommendation="Mantener mediciones semanales y registrar una meta estructurada en el perfil.",
...
def _support_capabilities(conn, snapshots: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    recovery = _simple_capability(
        ...
        [
            _indicator(
                "resting_hr", "FC de reposo", _as_float(avg_hr_rest),
                _as_float(personal_hr_rest_p25),
                0.35, min(int(hr_rest_28 or 0) / 14, 1), inverse=True, unit="bpm",
            ),
            _indicator(
                "wellness", "Continuidad wellness", float(wellness_28 or 0), 20.0,
                0.35, min(int(wellness_28 or 0) / 20, 1), unit="registros",
            ),
            _indicator(
                "rest_days", "Días de descanso", float(recent_rest_days),
                _as_float(rest_reference), 0.30, 1.0, unit="días/4 sem",
            ),
        ],
        maturity=min(70, int(wellness_28 or 0) * 2.5),
        recommendation="Registrar wellness y FC de reposo de forma consistente durante cuatro semanas.",
    )
    ...
    strength = _simple_capability(
        ...
        [
            _indicator(
                "sessions", "Sesiones por semana", strength_session_count / 8,
                2.0, 0.40, 1.0 if strength_session_count else 0.0, unit="ses/sem",
            ),
            _indicator(
                "consistency", "Semanas activas", float(strength_active_weeks),
                8.0, 0.30, 1.0, unit="semanas",
            ),
            _indicator(
                "progression", "Registros con carga", float(progression_rows or 0),
                8.0, 0.30, min(int(progression_rows or 0) / 8, 1), unit="registros",
            ),
        ],
        maturity=min(65, (int(strength_total or 0) + int(fuerza_total or 0)) * 2),
        recommendation="Registrar dos sesiones por semana con ejercicio, series, repeticiones y carga.",
    )
    ...
    climbing = _simple_capability(
        ...
        [
            _indicator(
                "weekly_ascent", "Desnivel en cuatro semanas", float(ascent_recent or 0),
                (_as_float(ascent_reference) or 0) * 4 or None,
                0.40, data_confidence, unit="m",
            ),
            _indicator(
                "climbing_sessions", "Sesiones con más de 500 m", float(climb_recent or 0),
                4.0, 0.35, data_confidence, unit="sesiones/8 sem",
            ),
            _indicator(
                "best_ascent", "Mayor desnivel por sesión",
        ...
        recommendation="Acumular ocho semanas con desnivel registrado y sesiones comparables.",
    )
    with conn.cursor() as cur:
        cur.execute("""
            SELECT
...
def _historical_body_composition_block(
    ...
    indicators = [
        _indicator(
            "weight_level", "Peso mínimo del periodo",
            block_min, prior_p10,
            0.50, adherence_confidence, inverse=True, unit="kg",
        ),
        _indicator(
            "measurement_adherence", "Semanas con medición",
            float(measured_weeks), 12.0,
            0.30, 1.0, unit="semanas",
        ),
        _indicator(
            "weight_trend", "Tendencia del periodo",
            end_avg, start_avg,
...
def _historical_climbing_block(
    ...
    indicators = [
        _indicator(
            "weekly_ascent", "Desnivel semanal promedio",
            avg_weekly, prior_p90_weekly, 0.40, data_confidence, unit="m",
        ),
        _indicator(
            "climbing_sessions", "Sesiones con más de 500 m",
...
def readiness(
    ...
    gap_text = (
        "Faltan datos en: " + ", ".join(data_gaps) + ". Registrar para lectura completa."
        if data_gaps else None
    )
    limit_text = (
        f"Factor limitante: {limiting['nombre']} ({limiting['score']:.1f}/100)."
    ...
    return {
        ...
        "recommendation": recommendation,
        "note": "Score 0 para capacidades sin datos — ausencia de evidencia, no capacidad nula.",
    }
...
_PATRON_LABELS: dict[str, str] = {
    "pico": "Pico de rendimiento",
    "descenso": "Descenso de forma",
    "abandono": "Brecha de entrenamiento",
    "estable": "Época estable",
...
def historical_similarity(snapshots: list[dict[str, Any]], top_n: int = 5) -> dict[str, Any]:
    ...
    """
    if len(snapshots) < 20:
        return {"ok": False, "error": "Historial insuficiente (mínimo 20 semanas)."}
...
def _similarity_mensaje(
    ...
    if total == 0:
        return "No se encontraron épocas suficientemente similares en el historial."
    parts = [f"{_PATRON_LABELS.get(k, k).lower()} ({v})" for k, v in counts.items()]
    dist = ", ".join(parts)
    if dominant == "pico" and dominant_count >= 2:
        return f"De tus {total} épocas más similares, {dominant_count} derivaron en un pico de rendimiento. El historial sugiere potencial ascendente. Distribución: {dist}."
    if dominant == "abandono" and dominant_count >= 2:
        return f"De tus {total} épocas similares, {dominant_count} terminaron en brecha de entrenamiento. Vale la pena revisar qué interrumpió esas épocas. Distribución: {dist}."
    if dominant == "descenso" and dominant_count >= 2:
        return f"De tus {total} épocas similares, {dominant_count} derivaron en descenso de forma. Distribución: {dist}."
    return f"Historial mixto: {dist}. Sin patrón dominante claro."
