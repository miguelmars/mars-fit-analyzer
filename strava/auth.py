...
def get_supabase():
    ...
    if _supabase is None:
        # Bloque A: si staging no está disponible (módulo o config ausentes),
        # degradar a 503 honesto en lugar de crashear con 500.
        try:
            from supabase import create_client  # lazy — no falla en startup
            _supabase = create_client(SUPABASE_URL, SUPABASE_KEY)
        except Exception as e:
            from fastapi import HTTPException
            raise HTTPException(503, f"Staging (Supabase) unavailable: {type(e).__name__}")
    return _supabase
