...
def _usable_intent_name(name: str) -> bool:
    ...
    if "type to confirm" in value:
        return False
    if "tipo por confirmar" in value:
        return False
    if value in {"garmin plan session", "sesion del plan garmin"}:
        return False
...
def _activity_supports_plan(row: dict[str, Any], session_type: str, name: str) -> int:
    text = _plain(f"{session_type or ''} {name or ''}")
    avg_hr = float(row.get("avg_hr") or row.get("avg_hr_bpm") or 0)
    duration_s = float(row.get("moving_time_s") or row.get("elapsed_time_s") or 0)
    distance_km = float(row.get("distance_m") or 0) / 1000.0
    score = 0
    if duration_s >= 1800 or distance_km >= 10:
        score += 4
    if any(x in text for x in ("tempo", "interval", "sweet", "threshold")):
        if avg_hr >= 145:
            score += 10
        elif avg_hr >= 135:
            score += 5
            ...
            score -= 18
    elif any(x in text for x in ("z2", "aerobic", "endurance", "base")) and 120 <= avg_hr <= 150:
        score += 8
    return score
...
def _find_plan_match(conn, row: dict[str, Any]) -> Optional[dict[str, Any]]:
    planned_date_text = (row.get("started_at_local") or row.get("started_at") or "")[:10]
    if not planned_date_text:
        return None
    try:
        planned_day = datetime.fromisoformat(planned_date_text).date()
    except Exception:
        return None
    ...
    with conn.cursor() as cur:
        cur.execute(
            """
            SELECT ps.description, ps.session_type, tp.name, ps.planned_date
            ...
            LIMIT 8
            """,
            (planned_day - timedelta(days=tolerance_days),
             planned_day + timedelta(days=tolerance_days),
             planned_day),
        )
    ...
    best = None
    best_score = -1
    for description, session_type, plan_name, pdate in matches:
        name = description or session_type
        if not _usable_intent_name(name):
            continue
        clean_name = str(name).replace(" (plan Garmin)", "").strip()
        date_diff = abs((pdate - planned_day).days)
        score = {0: 80, 1: 68, 2: 56}.get(date_diff, 0)
        if description and description != session_type:
            score += 8
        score += _activity_supports_plan(row, session_type or "", clean_name)
        if score > best_score:
            best_score = score
            best = {
                "source": "epoch_plan",
    ...
    if best and best["confidence"] >= 82:
        return best
    return None
...
async def reconcile_strava_names(
    ...
    response = (
        ...
        .select(
            ...
            "distance_m,moving_time_s,elapsed_time_s,avg_hr"
        )
