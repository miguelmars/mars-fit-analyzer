...
ADR-012: Strava es ingestión; Mars canonical es la fuente maestra.
          Las sesiones de Garmin NO se sobreescriben — coexisten por clean_session_id distinto.
ADR-011: Sin tocar goals ni prescripciones durante la transformación.

Notas de deduplicación:
  - Garmin:  clean_session_id = 'garmin_XXXXX'
  - Strava:  clean_session_id = 'strava_XXXXX'
  - No hay colisión de PK — el análisis los fusiona vía view sessions_clean_compat.
"""
import logging
...
def _strava_to_clean_session(row: dict) -> dict:
    ...
    return {
        ...
        "raw_json":            None,   # No duplicar — ya está en strava_activities_raw
    }
            ...
            with conn.cursor() as cur:
                cur.execute("""
                    INSERT INTO clean_sessions (
                        ...
                        route_id, power_available, efficiency_speed_hr,
                        ...
                        %(route_id)s, %(power_available)s, %(efficiency_speed_hr)s,
...
async def transform_all_pending(batch_size: int = BATCH_SIZE) -> dict:
    ...
    while True:
        ...
        if batch_transformed == 0 and batch_errors == 0:
            break
