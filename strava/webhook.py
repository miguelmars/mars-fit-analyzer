...
async def backfill_streams(
    ...
    # Actividades sin streams = las que NO tienen fila en strava_streams_raw.
    streamed = sb.table("strava_streams_raw").select("strava_activity_id").execute()
    streamed_ids = {r["strava_activity_id"] for r in (streamed.data or [])}
    ...
    if not rows:
        ...
        return JSONResponse({
            ...
            "total_activities": total_act.count or 0,
    ...
    for row in rows:
        ...
        try:
            ...
            # Marcar como procesada (has_gps=false) para no reintentar indefinidamente
            try:
                sb.table("strava_activities_raw").update({
                    "has_gps": False, "stream_quality": 0.0,
                }).eq("strava_id", activity_id).execute()
            except Exception:
                pass
            results["error"] += 1
...
async def stream_completeness():
    ...
    # Actividades con streams
    streamed_r = (
        sb.table("strava_streams_raw")
        .select("strava_activity_id", count="exact")
        .execute()
    )
    with_streams = len({r["strava_activity_id"] for r in (streamed_r.data or [])})

    # Métricas de cobertura desde strava_activities_raw
    cov_r = (
        sb.table("strava_activities_raw")
        .select(
            "sport_type,gps_coverage,hr_coverage,cadence_coverage,power_coverage,stream_quality"
        )
...
async def strava_activities(
    ...
    q = sb.table("strava_activities_raw").select(
        "strava_id,name,sport_type,distance,moving_time,elapsed_time,"
        "start_date,average_heartrate,max_heartrate,total_elevation_gain,"
        "average_speed,has_gps,has_cadence,has_power,"
        "hr_coverage,gps_coverage,cadence_coverage,power_coverage,stream_quality"
    )
    if sport:
        q = q.ilike("sport_type", f"%{sport}%")
    q = q.order("start_date", desc=True).range(offset, offset + limit - 1)

    res = q.execute()
    rows = res.data or []

    # contar unique stream IDs
    streamed_r = sb.table("strava_streams_raw").select("strava_activity_id").execute()
    streamed_ids = {r["strava_activity_id"] for r in (streamed_r.data or [])}

    activities = []
    for r in rows:
        dist_km = round(r.get("distance", 0) / 1000, 2) if r.get("distance") else None
        moving_s = r.get("moving_time") or 0
        ...
        activities.append({
            ...
            "max_hr":            r.get("max_heartrate"),
            "elevation_m":       r.get("total_elevation_gain"),
            "avg_speed_kmh":     round(r.get("average_speed", 0) * 3.6, 1) if r.get("average_speed") else None,
