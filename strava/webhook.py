...
from .rename import reconcile_strava_names, rename_strava_activity
...
async def trigger_backfill(
    ...
    after: int = Query(
        default=None,
        description="Unix timestamp. Only activities after this date."
    ),
    ...
    force: bool = Query(
        default=False,
        description="Ignore the running state — useful if Railway killed the previous process."
    ),
):
    """
...
async def backfill_streams(
    ...
    if not rows:
        ...
        return JSONResponse({
            "status":                    "completed",
            "message":                   "All activities have streams.",
    ...
    return JSONResponse({
        ...
        "message": (
            f"Batch of {results['processed']} activities processed. "
            f"Call again until remaining_without_streams = 0."
        ),
    })
...
async def register_webhook():
    ...
    if not STRAVA_CLIENT_SECRET:
        return JSONResponse(
            {"status": "error", "message": "STRAVA_CLIENT_SECRET not configured"},
            status_code=400,
        )
    ...
    if resp.status_code == 201:
        ...
        return JSONResponse({
            "status":          "ok",
            "message":         "Webhook registered successfully on Strava 🎉",
    ...
    if resp.status_code == 422:
        return JSONResponse({
            ...
            "message": "An active webhook already exists for this app.",
            "details": data,
        })
...
async def sync_now(
    pages: int = Query(
        default=5,
        description="How many pages of 200 activities to pull (5 = 1,000 activities)."
    ),
    force: bool = Query(
        default=True,
    ...
    try:
        ...
        if "RATE_LIMIT" in err_str or "429" in err_str or "Too Many Requests" in err_str:
            ...
            return JSONResponse({
                "status": "rate_limited",
                "message": "Strava rate limit (200 req/15min). Wait 15 minutes and retry.",
    ...
    return JSONResponse({
        "status": "ok",
        **results,
        "message": f"Done. {results['ingested']} new, {results['duplicates']} already existed.",
    })
...
async def trigger_transform(
    ...
    total = {"transformed": 0, "skipped": 0, "errors": 0, "batches_run": 0}
    ...
    return JSONResponse({
        "status":    "ok",
        **total,
        "message":   f"{total['transformed']} sessions → clean_sessions. "
                     f"{total['skipped']} covered by Garmin. "
...
async def backfill_status():
    ...
    # Estado real: si quedan streams, seguimos en backfill aunque sync de actividades haya terminado
    if remaining_streams > 0:
        effective_status = "backfilling"
        semaforo = "🟡"
    ...
    return JSONResponse({
        ...
        "pct_streams":           pct_streams,
        "transform_pending":     activities_pending,
        "transform_done":        activities_done,
        "transform_error":       activities_error,
...
async def dedup_diagnosis(
    ...
    try:
        ...
        # Retorna 200 con error para que los clientes lean el detalle
        return JSONResponse({
            "status": "error",
            "message": str(e),
...
async def dedup_samples(
    diagnosis_id: str = Query(..., description="UUID from /dedup-diagnosis"),
):
    """
...
async def approve_samples(request: Request):
    ...
    if not diagnosis_id:
        return JSONResponse({"error": "diagnosis_id required"}, status_code=400)
...
# ── Rename / intent reconciliation ───────────────────────

@router.post("/activity/{activity_id}/rename")
async def rename_activity_endpoint(
    activity_id: int,
    name: str = Query(..., min_length=1, max_length=80),
    execute: bool = Query(False, description="false = preview, true = update Strava"),
):
    """
    Rename one Strava activity.
    Requires activity:write and a fresh Strava authorization after that scope ships.
    """
    try:
        result = await rename_strava_activity(activity_id, name, execute=execute)
        return JSONResponse(result)
    except PermissionError as exc:
        return JSONResponse({"status": "needs_reauthorization", "message": str(exc)}, status_code=403)
    except Exception as exc:
        logger.error("Strava rename failed activity_id=%s: %s", activity_id, exc, exc_info=True)
        return JSONResponse({"status": "error", "message": str(exc)}, status_code=400)


@router.post("/rename-from-garmin")
async def rename_from_garmin_endpoint(
    execute: bool = Query(False, description="false = preview, true = update Strava"),
    limit: int = Query(50, ge=1, le=200),
    only_generic: bool = Query(True, description="Only rename generic Strava names"),
):
    """
    Preview/apply Garmin/EPOCH workout names to Strava titles.

    Garmin/plan intent wins. Strava names like Morning Ride are treated as
    display noise, not the source of training truth.
    """
    try:
        result = await reconcile_strava_names(
            execute=execute,
            limit=limit,
        ...
        logger.error("Strava bulk rename failed: %s", exc, exc_info=True)
        return JSONResponse({"status": "error", "message": str(exc)}, status_code=400)


@router.get("/yearly-summary")
async def strava_yearly_summary():
    """
    Resumen anual de actividades desde strava_activities_raw.
    Agrupa por año y tipo de deporte — base para Mi Evolución.
    """
    from .auth import get_supabase
    sb = get_supabase()

    # Traer todos los campos necesarios con paginación
    rows: list = []
    page = 0
    while True:
        chunk = (
            sb.table("strava_activities_raw")
            .select("started_at,sport_type,distance_m,moving_time_s,elevation_gain_m,avg_hr")
    ...
    # Agrupar por año y deporte en Python
    from collections import defaultdict
    buckets: dict = defaultdict(lambda: {"sessions": 0, "km": 0.0, "hours": 0.0, "elevation_m": 0.0, "hr_sum": 0.0, "hr_count": 0})
    for r in rows:
        started = r.get("started_at") or ""
        if not started:
            continue
        year = started[:4]
        sport = r.get("sport_type") or "Other"
        key = (year, sport)
        d = buckets[key]
        d["sessions"] += 1
        d["km"]          += (r.get("distance_m") or 0) / 1000
        d["hours"]       += (r.get("moving_time_s") or 0) / 3600
        d["elevation_m"] += r.get("elevation_gain_m") or 0
        hr = r.get("avg_hr")
        if hr:
            d["hr_sum"]   += hr
            d["hr_count"] += 1
    ...
    yearly = []
    for (year, sport), d in sorted(buckets.items(), key=lambda x: (x[0][0], -x[1]["km"]), reverse=False):
        yearly.append({
            "year":        int(year),
            "sport_type":  sport,
            "sessions":    d["sessions"],
            ...
            "elevation_m": round(d["elevation_m"], 0),
            "avg_hr":      round(d["hr_sum"] / d["hr_count"], 1) if d["hr_count"] else None,
        })

    # Totales globales
    total_km   = round(sum(r.get("distance_m", 0) for r in rows) / 1000, 1)
    total_h    = round(sum(r.get("moving_time_s", 0) for r in rows) / 3600, 1)
    total_elev = round(sum(r.get("elevation_gain_m", 0) or 0 for r in rows), 0)
    years_active = sorted(set(r.get("started_at", "")[:4] for r in rows if r.get("started_at")))
...
@router.get("/recent-weeks")
async def strava_recent_weeks(weeks: int = 12):
    """
    Resumen por semana de las últimas N semanas desde strava_activities_raw.
    Usado para gráficas de tendencia en Mi Evolución.
    """
    ...
    buckets: dict = defaultdict(lambda: {"sessions": 0, "km": 0.0, "hours": 0.0, "hr_sum": 0.0, "hr_count": 0, "sports": set()})
    for r in rows:
        started = r.get("started_at") or ""
        if not started:
            continue
    ...
    return JSONResponse({"weeks": result, "total_weeks": len(result)})


@router.get("/daily-heatmap")
async def strava_daily_heatmap(months: int = 3):
    """
    Mapa diario de actividad para los últimos N meses.
    Devuelve {date, sessions, km, sport_types[]} por día activo.
    """
    from datetime import datetime, timedelta, timezone
    from .auth import get_supabase
    sb = get_supabase()
    ...
    res = (
        sb.table("strava_activities_raw")
        .select("started_at,sport_type,distance_m")
        .gte("started_at", since)
        .order("started_at", desc=False)
        .execute()
    )
    ...
    from collections import defaultdict
    buckets: dict = defaultdict(lambda: {"sessions": 0, "km": 0.0, "sport_types": set()})
    for r in rows:
    ...
    days = [
        {
            ...
            "sessions":    d["sessions"],
            "km":          round(d["km"], 1),
            "sport_types": list(d["sport_types"]),
        }
        for day, d in sorted(buckets.items())
    ]
    return JSONResponse({"days": days, "total_days": len(days)})
...
# ── v6.5: Backfill de laps (síncrono, por lotes, dry_run) ────────────────────

@router.post("/backfill-laps")
async def backfill_laps(
    batch: int = Query(default=50, ge=1, le=150,
                       description="Actividades por llamada. 1 request Strava c/u."),
    dry_run: bool = Query(default=False,
                          description="true = solo estimar pendientes, sin requests a Strava."),
):
    """
    Descarga laps para actividades en strava_activities_raw que aún no tienen
    filas en strava_laps_raw. Idempotente. Se detiene ante rate limit (429)
    y devuelve resumen parcial.

    Con batch=50 → 50 requests por llamada (límite Strava: 200/15min, 2000/día).
    Después de poblar staging, correr POST /api/strava/transform-laps.
    """
    from .auth import get_supabase
    from .client import get_activity_laps
    import httpx as _httpx

    sb = get_supabase()
    summary = {"ok": True, "dry_run": dry_run, "pending": 0, "processed": 0,
               "inserted": 0, "no_laps": 0, "errors": 0, "rate_limited": False}

    # IDs que ya tienen laps (paginado: Supabase corta en 1,000)
    lapped_ids: set = set()
    page = 0
    while True:
        chunk = (sb.table("strava_laps_raw").select("strava_activity_id")
                 .range(page * 1000, (page + 1) * 1000 - 1).execute().data or [])
        lapped_ids.update(r["strava_activity_id"] for r in chunk)
        if len(chunk) < 1000:
            break
        page += 1

    # Actividades sin laps (recientes primero)
    todo = []
    page = 0
    total_scanned = 0
    while True:
        rows = (sb.table("strava_activities_raw")
                .select("strava_id")
                .order("started_at", desc=True)
                .range(page * 500, (page + 1) * 500 - 1).execute().data or [])
        if not rows:
            break
        total_scanned += len(rows)
        for r in rows:
            if r["strava_id"] not in lapped_ids:
                todo.append(r["strava_id"])
        if len(rows) < 500:
            break
        page += 1
    ...
    if dry_run or not todo:
        summary["message"] = (f"DRY RUN: {len(todo)} activities without laps in staging. "
                              f"Nothing downloaded." if dry_run else "Staging complete.")
        return JSONResponse(summary)

    for activity_id in todo[:batch]:
        try:
            laps_raw = await get_activity_laps(activity_id)
            summary["processed"] += 1
            if not laps_raw:
                summary["no_laps"] += 1
                # Marcar con fila sentinel para no re-pedirla (lap_index=-1)
                sb.table("strava_laps_raw").insert(
                    {"strava_activity_id": activity_id, "lap_index": -1}).execute()
                continue
            laps_rows = [
                {
                    "strava_activity_id": activity_id,
            ...
            sb.table("strava_laps_raw").insert(laps_rows).execute()
            summary["inserted"] += len(laps_rows)
        except _httpx.HTTPStatusError as e:
            if e.response.status_code == 429:
                summary["rate_limited"] = True
                summary["ok"] = False
                summary["message"] = "Strava rate limit — retry in 15 min."
                break
            summary["errors"] += 1
        except Exception:
            summary["errors"] += 1
            if summary["errors"] >= 5:
                summary["ok"] = False
                summary["message"] = "Too many consecutive errors — stopped."
                break

    summary["remaining"] = max(0, summary["pending"] - summary["processed"])
    if "message" not in summary:
        summary["message"] = "Re-run until remaining=0, then POST /api/strava/transform-laps."
    return JSONResponse(summary)
