...
async def backfill_laps(
    ...
    if dry_run or not todo:
        summary["message"] = (f"DRY RUN: {len(todo)} actividades sin laps en staging. "
                              f"Nada descargado." if dry_run else "Staging completo.")
    ...
    for activity_id in todo[:batch]:
        try:
            ...
            if e.response.status_code == 429:
                ...
                summary["ok"] = False
                summary["message"] = "Rate limit Strava — reintentar en 15 min."
                break
            summary["errors"] += 1
        except Exception:
            summary["errors"] += 1
            if summary["errors"] >= 5:
                summary["ok"] = False
                summary["message"] = "Demasiados errores seguidos — detenido."
                break

    summary["remaining"] = max(0, summary["pending"] - summary["processed"])
    if "message" not in summary:
        summary["message"] = "Re-ejecutar hasta remaining=0, luego POST /api/strava/transform-laps."
    return JSONResponse(summary)
