...
async def backfill_streams(
    ...
    # Buscar actividades que NO tienen streams en strava_streams_raw todavía.
    # Usamos stream_quality IS NULL como señal de "nunca procesada para streams".
    # has_gps puede ser false por actividades indoor — no es indicador confiable.
    query = (
        sb.table("strava_activities_raw")
        .select("strava_id, name, sport_type")
        .is_("stream_quality", "null")   # NULL = nunca intentada
        .limit(batch)
    )
    if sport:
    ...
    # Contar cuántas quedan sin streams (stream_quality NULL = nunca procesada)
    remaining = (
        sb.table("strava_activities_raw")
        .select("id", count="exact")
        .is_("stream_quality", "null")
        .execute()
    )
