...
def run_dedup_diagnosis(sport_type: Optional[str] = None) -> dict:
    ...
    # ── 1. Leer actividades de Strava staging ────────────────────────────────
    ...
    query = (
        ...
        .neq("transform_status", "done")  # Solo las que no están ya transformadas
    )
    if sport_type:
        query = query.ilike("sport_type", f"%{sport_type}%")

    strava_rows = query.execute().data or []
