        ...
        with db_init_lock:
            if db_conn is None:
                try:
                    ...
                    _init_db(connection)
                    _apply_v5_migrations(connection)
                    db_conn = connection
...
def _ensure_garmin_staging_tables(conn):
    ...
    with conn.cursor() as cur:
        ...
        for idx_sql in [
            ...
            "CREATE INDEX IF NOT EXISTS idx_garmin_export_activities_active_start ON garmin_export_activities(is_active_snapshot, start_time_utc)",
...
def _ensure_clean_sessions_table(conn):
    ...
    with conn.cursor() as cur:
        ...
        for column, sql_type in [
            ...
            cur.execute(f"ALTER TABLE clean_sessions ADD COLUMN IF NOT EXISTS {column} {sql_type}")
        for idx_sql in [
...
def _ensure_zone_model_system(conn):
    """Create append-only zone models and non-destructive calculated fields."""
    ...
    with conn.cursor() as cur:
        cur.execute("""
            CREATE TABLE IF NOT EXISTS zone_models (
                ...
                notes             TEXT,
                created_at        TIMESTAMPTZ DEFAULT NOW(),
                CHECK (status IN ('active', 'historical', 'future'))
            )
        """)
        cur.execute(
            "CREATE INDEX IF NOT EXISTS idx_zone_models_sport_status "
            "ON zone_models(sport, status)"
        )
        clean_additions = [
            ("z2_pct_original", "DECIMAL(5,2)"),
            ("z2_pct_mars", "DECIMAL(5,2)"),
            ("zone_model_used", "TEXT"),
            ("zone_confidence", "TEXT"),
            ("z2_confidence_score", "NUMERIC(3,2)"),
            ("zone_recalculated_at", "TIMESTAMPTZ"),
        ]
        for column, sql_type in clean_additions:
            cur.execute(
                f"ALTER TABLE clean_sessions ADD COLUMN IF NOT EXISTS {column} {sql_type}"
            )
        snapshot_additions = [
            ("z2_pct_original", "DECIMAL(5,2)"),
            ("z2_pct_mars", "DECIMAL(5,2)"),
        ...
        for column, sql_type in snapshot_additions:
            cur.execute(
                f"ALTER TABLE athlete_snapshots ADD COLUMN IF NOT EXISTS {column} {sql_type}"
            )
    conn.commit()


def _ensure_session_environment_table(conn):
    """Create non-destructive environmental context for analytical sessions."""
    _ensure_clean_sessions_table(conn)
    with conn.cursor() as cur:
        cur.execute("""
            CREATE TABLE IF NOT EXISTS session_environment (
                clean_session_id        TEXT PRIMARY KEY
                    REFERENCES clean_sessions(clean_session_id) ON DELETE CASCADE,
                start_altitude_m        NUMERIC(8,2),
                avg_altitude_m          NUMERIC(8,2),
                min_altitude_m          NUMERIC(8,2),
                max_altitude_m          NUMERIC(8,2),
                altitude_range_m        NUMERIC(8,2),
                ascent_m                INT,
                start_lat               NUMERIC(10,7),
                start_lon               NUMERIC(10,7),
                end_lat                 NUMERIC(10,7),
                ...
                habitual_altitude_m     NUMERIC(8,2),
                altitude_delta_m        NUMERIC(8,2),
                altitude_band           TEXT,
                relative_altitude_band  TEXT,
                prior_21d_exposure_days INT,
                acclimatization_status  TEXT,
                altitude_source         TEXT,
                location_source         TEXT,
                altitude_confidence     NUMERIC(3,2),
        ...
        """)
        cur.execute(
            "CREATE INDEX IF NOT EXISTS idx_session_environment_altitude "
            "ON session_environment(avg_altitude_m)"
        )
        cur.execute(
            "CREATE INDEX IF NOT EXISTS idx_session_environment_country "
            "ON session_environment(country_code)"
        )
        cur.execute(
            "CREATE INDEX IF NOT EXISTS idx_session_environment_cluster "
            "ON session_environment(location_cluster)"
        )
    conn.commit()


def _ensure_capability_runs_table(conn):
    """Store audited capability executions without changing source data."""
    with conn.cursor() as cur:
        cur.execute("""
...
def _ensure_clean_sessions_compat_view(conn):
    """Expose canonical sessions with the legacy column names for read endpoints."""
    _ensure_zone_model_system(conn)
    _ensure_canonical_sessions_view(conn)
    with conn.cursor() as cur:
        cur.execute("""
            CREATE OR REPLACE VIEW sessions_clean_compat AS
            SELECT
                ...
                start_date,
                power_available,
                avg_power_w::double precision AS avg_power_w,
                max_power_w::double precision AS max_power_w,
                normalized_power_w::double precision AS normalized_power_w
            FROM canonical_sessions
        """)
    conn.commit()


def _ensure_canonical_sessions_view(conn):
    """Create the non-destructive Garmin-history + new-Strava read layer.

    The active Garmin export owns history through its latest UTC activity.
    Strava contributes only sessions after that exact cutoff. Recent internal
    sessions remain visible only when Strava does not already represent them.
    No source row is deleted from clean_sessions.
    """
...
def _ensure_athlete_snapshots_table(conn):
    """Create the weekly historical spine used by Phase 2 analysis."""
    with conn.cursor() as cur:
        cur.execute("""
        ...
        additions = [
            ...
            ("aerobic_hours", "DECIMAL(7,2)"),
            ("cycling_efficiency", "DECIMAL(8,5)"),
            ("running_efficiency", "DECIMAL(8,5)"),
            ("efficiency_sport", "TEXT"),
            ("avg_temperature_c", "DECIMAL(5,2)"),
            ("z2_estimated", "BOOLEAN DEFAULT TRUE"),
            ("weight_source", "TEXT"),
        ...
        for column, sql_type in additions:
            cur.execute(f"ALTER TABLE athlete_snapshots ADD COLUMN IF NOT EXISTS {column} {sql_type}")
        cur.execute(
            "CREATE INDEX IF NOT EXISTS idx_athlete_snapshots_week_start "
            "ON athlete_snapshots(week_start)"
        )
    conn.commit()
...
def _ensure_wellness_table(conn):
    """No-op: wellness is created by _init_db with full schema."""
    pass


def _apply_v5_migrations(conn):
    """
    Migraciones v5.0 — se ejecutan al startup, todas idempotentes (IF NOT EXISTS).
    Equivalen a migrations/002, 003, 005 para Railway PostgreSQL.
    """
    with conn.cursor() as cur:
        # 002 — Source lineage en clean_sessions (B1)
        cur.execute("ALTER TABLE clean_sessions ADD COLUMN IF NOT EXISTS source TEXT DEFAULT 'garmin_fit'")
        cur.execute("ALTER TABLE clean_sessions ADD COLUMN IF NOT EXISTS source_activity_id TEXT")
        cur.execute("ALTER TABLE clean_sessions ADD COLUMN IF NOT EXISTS source_confidence FLOAT DEFAULT 1.00")
        cur.execute("ALTER TABLE clean_sessions ADD COLUMN IF NOT EXISTS source_batch TEXT")
        cur.execute("ALTER TABLE clean_sessions ADD COLUMN IF NOT EXISTS original_file_available BOOLEAN DEFAULT true")
        cur.execute("UPDATE clean_sessions SET source = 'garmin_fit', source_confidence = 1.00 WHERE source IS NULL")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_clean_sessions_source ON clean_sessions (source)")

        # 003 — Source lineage en wellness (B2)
        cur.execute("ALTER TABLE wellness ADD COLUMN IF NOT EXISTS source TEXT DEFAULT 'manual_morning_check'")
        cur.execute("ALTER TABLE wellness ADD COLUMN IF NOT EXISTS source_record_id TEXT")
        cur.execute("ALTER TABLE wellness ADD COLUMN IF NOT EXISTS source_confidence FLOAT DEFAULT 0.75")
        cur.execute("ALTER TABLE wellness ADD COLUMN IF NOT EXISTS source_batch TEXT")
        cur.execute("ALTER TABLE wellness ADD COLUMN IF NOT EXISTS is_subjective BOOLEAN DEFAULT true")
        cur.execute("UPDATE wellness SET source = 'manual_morning_check', source_confidence = 0.75, is_subjective = true WHERE source IS NULL")

        # 005 — dedup_approval_log (C3)
        cur.execute("""
        ...
        """)
        cur.execute("CREATE INDEX IF NOT EXISTS idx_dedup_approval_diagnosis_id ON dedup_approval_log (diagnosis_id)")
    conn.commit()


def _ensure_fuerza_table(conn):
    """No-op: fuerza is created by _init_db with full schema."""
    pass


def _ensure_goals_table(conn):
    """
    E26B — Goal Registry.
    ADR-011: el usuario define sus metas activas; el sistema solo sugiere.
    """
    with conn.cursor() as cur:
        cur.execute("""
            CREATE TABLE IF NOT EXISTS mars_goals (
                id            SERIAL PRIMARY KEY,
                event_name    TEXT NOT NULL,
                event_type    TEXT NOT NULL DEFAULT 'cycling',
                event_date    DATE,
                distance_km   FLOAT,
                elevation_m   FLOAT,
