...
def get_post_session(session_id: str):
    ...
    try:
        with conn.cursor() as cur:
            ...
            row = cur.fetchone()
            if not row:
                raise HTTPException(404, f"No post-session record for {session_id}")
    ...
    try:
        ...
        for row in rows:
            ...
            if pct >= 60:
                ...
                label = "Replace now" if pct >= 85 else "Check soon"
...
def gpt_gear_alerts():
    ...
    try:
        ...
        for row in rows:
            ...
            item["label"] = "Replace now" if pct >= 85 else "Check soon" if pct >= 60 else "OK"
            alerts.append(item)
...
def gpt_athlete_status(sport: str = "cycling"):
    ...
    try:
        ...
        if km_4w_avg > 0:
            ...
            if p > 10: fitness = "rising"
            elif p < -15: fitness = "declining"
        fatiga = "low"
        ...
        rec = "continue Z2"
        if fatiga == "high": rec = "reduce intensity, recovery session"
        elif fatiga == "moderate": rec = "hold Z2, avoid Z4-Z5 this week"
        elif fitness == "rising" and fatiga == "low": rec = "good form, you can add a Z3 tempo session"
...
def get_athlete_profile():
    ...
    try:
        with conn.cursor() as cur:
            ...
            if not row:
                return {
                    ...
                    "zones": {
                        "Z1": "0-108 bpm",
                        "Z2": "134-150 bpm (Aerobic)",
                        "Z3": "151-160 bpm (Tempo)",
                        "Z4": "161-168 bpm (Threshold)",
                        "Z5": "169+ bpm (Maximum)"
                    },
                    "active_bike": "Orbea Aluminum Road 2020 matte black",
                    "nota": "Profile not saved yet — use POST /gpt/athlete-profile to initialize it"
                }
            cols = [d[0] for d in cur.description]
        ...
        profile["zones"] = {
            "Z1": f"0-{lt-60} bpm (Recovery)",
            "Z2": f"{lt-34}-{lt-18} bpm (Aerobic)",
            "Z3": f"{lt-17}-{lt-8} bpm (Tempo)",
            "Z4": f"{lt-7}-{lt} bpm (Threshold)",
            "Z5": f"{lt+1}+ bpm (Maximum)"
        }
        return profile
    except Exception as e:
        raise HTTPException(500, str(e))
...
# ── U6 (UI v2) — Body Map: qué parte del cuerpo recibió mantenimiento ───────

@router.get("/gpt/body-map")
def gpt_body_map():
    """Estado corporal por zona: molestias activas (rojo) y terapias de los
    últimos 7 días (verde). El cuerpo como mapa, no como lista."""
    conn = get_db()
    if not conn:
    ...
    with conn.cursor() as cur:
        cur.execute("""
            SELECT pain_zone, pain_level, pain_start, notes
            FROM wellness
            WHERE category='pain' AND pain_zone IS NOT NULL
              AND (pain_end IS NULL OR pain_end >= CURRENT_DATE)
            ORDER BY pain_start DESC NULLS LAST
        """)
        for z, lvl, start, note in cur.fetchall():
            if z not in zones:
                zones[z] = {"pain": None, "therapies": []}
            if zones[z]["pain"] is None:
                zones[z]["pain"] = {"level": lvl,
                                    "since": start.isoformat() if start else None,
                                    "note": note}
        cur.execute("""
            SELECT category, muscle_zone, date, compex_program
            FROM wellness
            WHERE date >= CURRENT_DATE - INTERVAL '7 days'
              AND category IN ('compex_recovery','massage_gun','ceragem','massage','mobility')
            ORDER BY date DESC
        """)
        for cat, mzs, d, prog in cur.fetchall():
            for z in (mzs or ["general"]):
                if z not in zones:
                    zones[z] = {"pain": None, "therapies": []}
                zones[z]["therapies"].append({"kind": cat, "date": d.isoformat(),
                                              "program": prog})
    pains_n = sum(1 for z in zones.values() if z["pain"])
    ther_n = sum(len(z["therapies"]) for z in zones.values())
    return {"ok": True, "zones": zones,
            "active_pains": pains_n, "therapies_7d": ther_n,
            "nota": ("Red = active niggle · green = maintenance in the last 7 days. "
                     "The body is the map; recovery is where adaptation happens.")}
...
def gpt_gear_status():
    ...
    try:
        ...
        for r in rows:
            ...
            d["km_used_note"] = "Approximated from total bike km minus installation km; it becomes exact once activity-part links exist."
        ...
        with conn.cursor() as cur:
            cur.execute("""
                SELECT id, date::text, category, subcategory,
                       muscle_groups, intensity, duration_min, notes
