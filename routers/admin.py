...
def admin_clean_device_artifacts(execute: bool = False, token: str = None):
    ...
    generic name 'Cycling', no heart rate, very short. Preview by default; pass
    execute=true to delete. New artifacts are already blocked at ingestion
    (strava/transform.py), so this only cleans what is already stored."""
    _check_token(token)
    conn = get_db()
    if not conn:
        raise HTTPException(503, "DB no disponible")
    where = ("(name = 'Cycling' OR name IS NULL OR name = '') "
             "AND avg_hr_bpm IS NULL AND COALESCE(duration_s, 0) < 1800")
    try:
        with conn.cursor() as cur:
            cur.execute(f"""
        ...
        return {
            ...
            "matched": total,
            "deleted": deleted,
            "criteria": "name='Cycling' · no HR · under 30 min",
            "sample": sample,
