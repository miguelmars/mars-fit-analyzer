...
@router.get("/api/canonical-status")
def canonical_status_endpoint():
    """Public, read-only status of the active Garmin/Strava canonical layer."""
    try:
        from tools.auto_activate_garmin import canonical_status

        return {"ok": True, **canonical_status()}
    except RuntimeError as exc:
        raise HTTPException(503, str(exc))
    except Exception as exc:
        logger.error(f"Canonical status error: {exc}")
...
def admin_backfill_snapshots(token: str = None):
    ...
    try:
        ...
        if not snapshots:
            return {"ok": True, "message": "No sessions in clean_sessions — Strava backfill pending", "count": 0}
        ...
        return {
            ...
            "message": f"{len(snapshots)} weeks of snapshots generated. Capacities now have data."
        }
    except Exception as e:
...
def admin_clean_sessions(token: str = None):
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            cur.execute("""
                SELECT MAX(start_time_utc)::text
                FROM garmin_export_activities
                WHERE is_active_snapshot IS TRUE
            """)
            garmin_cutoff_utc = cur.fetchone()[0]
            cur.execute("""
                SELECT source, COUNT(*)
        ...
        return {
            "ok": True,
            "clean_sessions_raw": total,
            "canonical_sessions": canonical_total,
            "garmin_cutoff_utc": garmin_cutoff_utc,
            ...
            "verdict": "canonical_sessions manda: Garmin activo como historia y Strava solo después del corte Garmin.",
        }
    except Exception as e:
...
def admin_backup(token: str = ""):
    ...
    if token != admin_token:
        raise HTTPException(403, "Invalid token. Use ?token=YOUR_TOKEN")
    conn = get_db()
    if not conn:
        raise HTTPException(503, "DB no disponible")
    ...
    try:
        ...
        if execute:
            if validation["blockers"]:
                raise HTTPException(
                    409,
                    "Validation has blockers and cannot be recorded as official.",
                )
            with conn.cursor() as cur:
                cur.execute("""
...
# ── /admin/garmin-sleep-coverage — D2 ────────────────────────────────────────

@router.get("/admin/garmin-sleep-coverage")
def garmin_sleep_coverage(token: str = None):
    """
    D2: Reporte de cobertura de sueño Garmin por año.
    Determina qué años son confiables para E25C Recuperación History.

    Criterio: >= 80% confiable · 40-79% parcial · < 40% insuficiente
    """
    _check_token(token)
    import sys
    from pathlib import Path
    sys.path.insert(0, str(Path(__file__).resolve().parents[1] / "tools"))
    from garmin_sleep_to_wellness import coverage_report
    try:
...
# ── /admin/import-garmin-sleep — D1 ──────────────────────────────────────────

@router.post("/admin/import-garmin-sleep")
def import_garmin_sleep(
    token: str = None,
    dry_run: bool = True,
):
    """
    D1: Importa datos de sueño de garmin_export_sleep → wellness.

    dry_run=true  → solo reporte, no escribe nada (default seguro)
    dry_run=false → importa datos aplicando todas las reglas de protección

    Reglas:
      - 1 registro por día
      - NO sobrescribe wellness manual existente
      - source = garmin_sleep_export · confidence = 0.90
    """
    _check_token(token)
    import sys
    from pathlib import Path
    sys.path.insert(0, str(Path(__file__).resolve().parents[1] / "tools"))
    from garmin_sleep_to_wellness import import_sleep_to_wellness
    try:
        result = import_sleep_to_wellness(dry_run=dry_run)
    except Exception as e:
        logger.error(f"Error en import-garmin-sleep: {e}", exc_info=True)
        raise HTTPException(500, str(e))
    return result


# ── /admin/clean-device-artifacts ────────────────────────────────────────────

@router.get("/admin/clean-device-artifacts")
def admin_clean_device_artifacts(execute: bool = False, token: str = None):
    """Remove fake 'rides' the speed sensor logs while washing or moving the bike:
    Strava source, speed-sensor device marker, generic name 'Cycling', no heart
    rate, very short. Preview by default; pass execute=true to delete. New
    artifacts are already blocked at ingestion (strava/transform.py), so this
    only cleans what is already stored."""
    _check_token(token)
    conn = get_db()
    if not conn:
        raise HTTPException(503, "DB no disponible")
    where = (
        "source = 'strava' "
        "AND (name = 'Cycling' OR name IS NULL OR name = '') "
        "AND avg_hr_bpm IS NULL "
        "AND COALESCE(duration_s, 0) < 1800 "
        "AND COALESCE(raw_json::text, '') ILIKE '%speed sensor%'"
    )
    try:
        with conn.cursor() as cur:
            cur.execute(f"""
                SELECT clean_session_id, start_date, name, duration_s, distance_km
                FROM clean_sessions WHERE {where}
            ...
            cur.execute(f"SELECT COUNT(*) FROM clean_sessions WHERE {where}")
            total = cur.fetchone()[0]
            deleted = 0
            if execute and total:
                cur.execute(f"DELETE FROM clean_sessions WHERE {where}")
        ...
        return {
            ...
            "matched": total,
            "deleted": deleted,
            "criteria": "source=strava · device contains Speed Sensor · name='Cycling' · no HR · under 30 min",
            "sample": sample,
            "note": "New artifacts already blocked at ingestion. Add execute=true to delete.",
        }
    except Exception as e:
        conn.rollback()
        raise HTTPException(500, str(e))
...
@router.get("/admin/garmin-sync")
def admin_garmin_sync(days: int = 14, token: str = None):
    """Pull activities (with their REAL names) and wellness straight from Garmin
    into clean_sessions + wellness. Uses garmin_sync (the free garminconnect
    library). Requires a cached Garmin token or GARMIN_EMAIL/GARMIN_PASSWORD env
    vars, plus a one-time local login to clear MFA (see garmin_sync.py __main__)."""
    _check_token(token)
    conn = get_db()
    if not conn:
        raise HTTPException(503, "DB no disponible")
    try:
        from garmin_sync import sync_to_db
        return sync_to_db(conn, days=days)
    except Exception as e:
        conn.rollback()
