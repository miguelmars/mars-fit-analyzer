...
def _terrain_class(ascent_m, distance_km):
    """Clase de terreno por m de ascenso por km: flat <6 · rolling 6-14 · climbing >14."""
    if not distance_km or distance_km <= 0:
        return None
    mpk = (ascent_m or 0) / float(distance_km)
    return "flat" if mpk < 6 else ("rolling" if mpk <= 14 else "climbing")


def _load_effort_class_reps(cur, ref, broad=False, limit=12):
    """V9.1 — Comparación para atletas que NO repiten rutas.
    broad=False → clase de esfuerzo: misma intención + modalidad + distancia ±25% + mismo terreno.
    broad=True  → línea base por intención: misma intención + modalidad, últimos 180 días.
    La intención se evalúa por sesión (igual que _load_repetitions): Z2 jamás vs intervalos."""
    dist = float(ref.get("distance_km") or 0)
    params = [ref["sport_type"], ref["clean_session_id"]]
    dist_sql = ""
    if not broad and dist > 0:
        dist_sql = "AND cs.distance_km BETWEEN %s AND %s"
        params += [dist * 0.75, dist * 1.25]
    cur.execute(f"""
        SELECT cs.clean_session_id, cs.start_date, cs.distance_km, cs.duration_s,
               cs.avg_hr_bpm, cs.avg_speed_kmh, cs.efficiency_speed_hr, cs.ascent_m,
               cs.avg_cadence,
        ...
        WHERE cs.sport_type = %s
          AND cs.clean_session_id != %s
          AND cs.start_date >= CURRENT_DATE - INTERVAL '180 days'
          {dist_sql}
        GROUP BY cs.clean_session_id
        ORDER BY cs.start_date DESC
        LIMIT 80
    """, tuple(params))
    ref_terrain = _terrain_class(ref.get("ascent_m"), ref.get("distance_km"))
    reps = []
    for r in cur.fetchall():
        avg_hr = float(r[4]) if r[4] else None
        intent = _session_type_from_row(avg_hr, r[7], float(r[2]) if r[2] else 0,
        ...
        if intent != ref["intent"]:
            continue
        if not broad and ref_terrain and _terrain_class(r[7], r[2]) != ref_terrain:
            continue  # mismo esfuerzo pero otro terreno — no comparable en nivel 2
        reps.append({
            "clean_session_id": r[0],
            "date": r[1].isoformat() if r[1] else None,
            "distance_km": float(r[2]) if r[2] else None,
...
# Niveles de comparación (V9.1): el más específico que los datos permitan,
# con confianza DECLARADA — nunca se finge que una clase amplia es un laboratorio.
_COMPARISON_LEVELS = {
    "route_lab":       {"confidence": "high",
                        "label": "same route, same intent",
                        "note": ""},
    "effort_class":    {"confidence": "medium",
                        "label": "similar efforts (same intent, similar distance and terrain)",
                        "note": (" BASIS: you have not repeated this route, so Epoch compares "
                                 "against your similar efforts — same intent, distance within 25% "
                                 "and same terrain class. Less exact than a repeated route; "
                                 "declared as medium confidence.")},
    "intent_baseline": {"confidence": "low",
                        "label": "all your sessions with this intent (180 days)",
                        "note": (" BASIS: no repeated route and no close effort match — this read "
                                 "compares against all your sessions with the same intent. "
                                 "Broad group, low confidence: direction only, not verdicts.")},
}


@router.get("/gpt/session/{clean_session_id}/progression")
def session_progression(clean_session_id: str):
    """¿Mejoré en ESTE workout? V9.1: jerarquía de comparación con confianza
    declarada — ruta exacta (high) → clase de esfuerzo (medium) → línea base
    por intención (low). El día Z2 'lento' se compara contra otros Z2, repita
    rutas el atleta o no."""
    conn = get_db()
    if not conn:
    ...
    with conn.cursor() as cur:
        ...
        # V9.1 — jerarquía de comparación: laboratorio exacto → clase de
        # esfuerzo → línea base por intención. Siempre se declara el nivel.
        level = "route_lab"
        reps = _load_repetitions(cur, ref) if ref["route_id"] else []
        if len(reps) < 2:
            ec = _load_effort_class_reps(cur, ref, broad=False)
            if len(ec) >= 2:
                reps, level = ec, "effort_class"
            else:
                ib = _load_effort_class_reps(cur, ref, broad=True)
                if len(ib) >= 2:
                    reps, level = ib, "intent_baseline"
                elif reps:
                    pass  # 1 repetición de ruta vale más que nada
                elif ec:
                    reps, level = ec, "effort_class"
        level_meta = _COMPARISON_LEVELS[level]
    ...
    if not reps:
        return {"ok": True, "session": ref, "repetitions": [], "comparison": None,
                "explanation_text": (f"OBSERVATION: First {intent_label} session of this "
                                     f"kind on record. INTERPRETATION: No comparable sessions yet "
                                     f"— at any level (route, similar effort, or intent). "
                                     f"This one becomes the baseline.")}
    ...
    # U7 (UI v2) — variante de entorno: urbano no se compara igual que abierto
    ref_urban = (ref.get("pauses") or 0) >= 4
    rep_pauses = [r.get("pauses") for r in prev if r.get("pauses") is not None]
    if rep_pauses:
        reps_urban = sum(1 for p in rep_pauses if p >= 4) >= len(rep_pauses) / 2
        if ref_urban and not reps_urban:
            terrain_note += (" VARIANT: today was urban riding (frequent stops) while "
                             "your repetitions were mostly open road — speed reads with "
                             "caution; HR carries the comparison.")
        elif not ref_urban and reps_urban:
            terrain_note += (" VARIANT: today was open road while your repetitions were "
                             "mostly urban — part of the speed gain may be fewer stops, "
                             "not just fitness.")
    if level == "route_lab":
        obs_where = f"on a known route — repetition #{len(reps)+1}"
    else:
        obs_where = f"compared against {len(reps)} {level_meta['label']}"
    obs = (f"OBSERVATION: {intent_label.capitalize()} {obs_where}. "
           f"Today: {ref['avg_speed_kmh'] or '—'} km/h, "
           f"HR {round(ref['avg_hr_bpm']) if ref['avg_hr_bpm'] else '—'}"
           + (f", {climate['session_c']}°C" if climate else "")
           + (f" · {_mod[0]}" if _mod[0] else "") + ".")
    # En línea base amplia no se emiten veredictos negativos — solo dirección
    if level == "intent_baseline" and verdict == "por_debajo":
        verdict = "estable"
    ...
    return {
        ...
        "group_key": {"route_id": ref["route_id"], "intent": ref["intent"],
                      "signature": ref["signature"], "comparison_level": level},
        "repetitions": reps,
        "comparison": {"baseline": f"last {len(prev)} comparable sessions",
                       "level": level,
                       "level_label": level_meta["label"],
                       "confidence": level_meta["confidence"],
                       "deltas": deltas, "verdict": verdict, "medal": medal},
        "climate": climate,
        "modality": _mod[0],
        "explanation_text": (obs + " " + interp + nota + level_meta["note"]
                             + climate_note + wind_note + terrain_note),
    }


# ── V9.2 — Session Demand: qué tan demandante fue la sesión PARA TI ─────────

_DEMAND_LABELS = [(25, "light"), (45, "moderate"), (65, "demanding"),
                  (85, "very demanding"), (101, "peak demand")]


@router.get("/gpt/session/{clean_session_id}/demand")
def session_demand(clean_session_id: str):
    """Session Demand (V9.2): demanda de la sesión relativa a TUS últimos 60
    días, no a una tabla externa. Componentes transparentes (duración,
    intensidad, desnivel, estructura) + modificadores (decoupling, calor).
    Propone lectura, nunca califica al atleta."""
    conn = get_db()
    if not conn:
        raise HTTPException(503, "DB unavailable")
    _ensure_training_tables(conn)
    ...
    with conn.cursor() as cur:
        ...
        if not ref:
            raise HTTPException(404, "Session not found")
        # Percentil de duración dentro de TUS últimos 60 días (misma modalidad)
        cur.execute("""
            SELECT COUNT(*) FILTER (WHERE duration_s <= %s), COUNT(*)
            FROM canonical_sessions
            WHERE sport_type = %s AND duration_s IS NOT NULL
              AND start_date >= CURRENT_DATE - INTERVAL '60 days'
              AND clean_session_id != %s
        """, (ref["duration_s"] or 0, ref["sport_type"], clean_session_id))
        below, total60 = cur.fetchone()
        cur.execute("""
            SELECT decoupling_pct, temp_avg_c FROM session_streams_summary
            WHERE clean_session_id = %s
        """, (clean_session_id,))
        srow = cur.fetchone()
    decoupling = float(srow[0]) if srow and srow[0] is not None else None
    temp_c = float(srow[1]) if srow and srow[1] is not None else None

    # ── Componentes 0–1, cada uno explicable ────────────────────────────────
    components = {}
    weights = {}
    if ref["duration_s"] and total60 and total60 >= 5:
        components["duration"] = round(below / total60, 2)
        weights["duration"] = 0.35
    hr = ref.get("avg_hr_bpm")
    if hr:
        # 110 bpm = suelo fácil · 168 = LT Mars. Lineal entre ambos.
        components["intensity"] = round(min(max((hr - 110) / (168 - 110), 0), 1), 2)
        weights["intensity"] = 0.35
    if ref.get("distance_km"):
        mpk = (ref.get("ascent_m") or 0) / float(ref["distance_km"])
        components["climbing"] = round(min(mpk / 20.0, 1), 2)
        weights["climbing"] = 0.15
    if ref.get("work_laps") is not None:
        components["structure"] = round(min((ref["work_laps"] or 0) / 6.0, 1), 2)
        weights["structure"] = 0.15

    if not components:
        return {"ok": True, "session": ref, "demand_score": None,
                "explanation_text": ("OBSERVATION: Not enough data to read this "
                                     "session's demand. MISSING: duration history, HR or laps.")}

    wsum = sum(weights.values())
    score = round(sum(components[k] * weights[k] for k in components) / wsum * 100)

    modifiers = []
    if decoupling is not None and decoupling > 7:
        score = min(score + 5, 100)
        modifiers.append(f"cardiac decoupling {decoupling}% — your body felt it harder than the plan suggests")
    if temp_c is not None and temp_c >= 28:
        modifiers.append(f"{temp_c}°C — heat raised the real cost without showing in pace")

    label = next(lbl for cap, lbl in _DEMAND_LABELS if score < cap)

    # Capacidad que más gravó esta sesión
    taxed = "aerobic engine"
    if components.get("climbing", 0) >= 0.6:
        taxed = "strength-endurance"
    elif components.get("structure", 0) >= 0.5 and components.get("intensity", 0) >= 0.7:
        taxed = "power"

    conf = "high" if (hr and total60 and total60 >= 10) else ("medium" if hr else "low")
    conf_reason = {"high": f"HR present and {total60} sessions in your 60-day window",
                   "medium": "HR present but few recent sessions to rank against",
                   "low": "no HR — duration and terrain only"}[conf]
    missing = []
    if not hr:
        missing.append("heart rate")
    if not total60 or total60 < 5:
        missing.append("more sessions in the last 60 days")
    if decoupling is None:
        missing.append("streams summary (run summarize-streams)")

    parts = " · ".join(f"{k} {v}" for k, v in components.items())
    obs = (f"OBSERVATION: demand {score}/100 ({label}) — {parts}.")
    interp = (f"INTERPRETATION: relative to YOUR last 60 days, this was a {label} "
              f"session. It mainly taxed your {taxed}.")
    mod_txt = (" CONTEXT: " + "; ".join(modifiers) + ".") if modifiers else ""
    sug = (" PRUDENT SUGGESTION: a very demanding day earns an easier one after it — "
           "adaptation happens in the recovery." if score >= 65 else
           " PRUDENT SUGGESTION: room to keep building — demand like this is sustainable.")

    return {
        "ok": True,
        "session": {k: ref[k] for k in ("clean_session_id", "name", "date",
                                        "sport_type", "intent", "distance_km",
                                        "duration_s", "avg_hr_bpm", "ascent_m")},
        "demand_score": score,
        "demand_label": label,
        "components": components,
        "weights": weights,
        "modifiers": modifiers,
        "capacity_taxed": taxed,
        "confidence": conf,
        "confidence_reason": conf_reason,
        "missing": missing,
        "nota": "Demand is measured against YOUR recent history, never an external table.",
        "explanation_text": obs + " " + interp + mod_txt
                            + (" EVIDENCE: " + conf_reason + ".")
...
def workout_groups(min_reps: int = Query(3, ge=2)):
    ...
    groups = [{"route_id": r[0], "route_name": r[8], "sport_type": r[1], "repetitions": r[2],
              ...
              for r in rows]
    # V9.1 — clases de esfuerzo: para el atleta que NO repite rutas, sus
    # "laboratorios" son tipos de esfuerzo (modalidad + banda de distancia +
    # terreno). La intención se filtra por sesión en /progression.
    with conn.cursor() as cur:
        cur.execute("""
            SELECT cs.sport_type,
                   CASE WHEN cs.distance_km < 20 THEN '<20km'
                        WHEN cs.distance_km < 40 THEN '20-40km'
                        WHEN cs.distance_km < 70 THEN '40-70km'
                        WHEN cs.distance_km < 120 THEN '70-120km'
                        ELSE '120km+' END AS dist_band,
                   CASE WHEN COALESCE(cs.ascent_m,0) / NULLIF(cs.distance_km,0) < 6 THEN 'flat'
                        WHEN COALESCE(cs.ascent_m,0) / NULLIF(cs.distance_km,0) <= 14 THEN 'rolling'
                        ELSE 'climbing' END AS terrain,
                   COUNT(*), MAX(cs.start_date),
                   AVG(cs.avg_hr_bpm), AVG(cs.avg_speed_kmh), AVG(cs.efficiency_speed_hr)
            FROM canonical_sessions cs
            WHERE cs.start_date >= CURRENT_DATE - INTERVAL '180 days'
    ...
    effort_classes = [{"sport_type": r[0], "distance_band": r[1], "terrain": r[2],
                       "sessions": r[3],
                       "last": r[4].isoformat() if r[4] else None,
                       "avg_hr": round(float(r[5]), 1) if r[5] else None,
                       "avg_speed_kmh": round(float(r[6]), 2) if r[6] else None,
    ...
    return {"ok": True, "groups": groups,
            ...
            "nota": ("v1 grouping by route. Intent is evaluated per session in "
                     ...
                     "effort_classes (V9.1) are the labs of athletes who do not repeat "
                     "routes: modality + distance band + terrain, last 180 days.")}
...
def test_recommendation():
    ...
    with conn.cursor() as cur:
        ...
        cur.execute("""
            ...
            FROM canonical_sessions cs LEFT JOIN routes rt ON rt.route_id=cs.route_id
            WHERE cs.route_id IS NOT NULL AND cs.sport_type IN ('Ride','VirtualRide')
            GROUP BY cs.route_id ORDER BY COUNT(*) DESC LIMIT 1
        """)
        lab = cur.fetchone()
        cur.execute("""
            SELECT planned_date, session_type, description, target
            FROM plan_sessions
            WHERE planned_date BETWEEN CURRENT_DATE AND CURRENT_DATE + 3
              AND status IN ('planned','matched','scheduled')
    ...
    hard_keywords = ("z5", "vo2", "threshold", "tempo", "interval", "hard", "intense")
    hard_upcoming = []
    for planned_date, session_type, description, target in upcoming:
        blob = " ".join([
            str(session_type or ""),
            str(description or ""),
            json.dumps(target, default=str) if target else "",
        ]).lower()
        if any(k in blob for k in hard_keywords):
    ...
    caution = None
    suggested_window = "Pick a fresh day after easy/rest, not the day before hard intensity."
    if hard_upcoming:
        caution = ("A hard planned session is close. Do not stack the threshold test "
                   "right before it; move one of them or choose the next fresh window.")
        suggested_window = "Rest/easy day -> test day -> easy day -> next hard workout."
    ...
    return {
        ...
        "explanation_text": ("OBSERVATION: " + "; ".join(reasons).capitalize() + ". "
                             "PRUDENT SUGGESTION: if you feel fresh in a clean window, "
                             f"a 20-minute sustained test on {lab_name} would tell you where your threshold "
                             "really is — and every zone recalibrates from it. "
                             f"{caution + ' ' if caution else ''}A proposal, not an order."),
    }
...
# ── U1 (UI v2) — Calidad de lectura: Lab vs Field ───────────────────────────
# "Epoch no juzga una sesión como laboratorio cuando ocurrió en el mundo real."

def _reading_quality(sport_type, pauses, paused_s, temp_c, wind_kmh):
    """high = sesión controlada, comparación válida · medium = hay variables,
    la tendencia sirve · low = mucho ruido externo, interpretar con cautela."""
    if sport_type == "VirtualRide":
        return {"reading_quality": "high",
                "reading_quality_note": ("Controlled session (trainer) — no wind, "
                                         "traffic or stops. Clean comparison for "
                                         "intensity and cadence; it does not measure "
                                         "real-road skills.")}
    noise = 0
    signals = []
    if (pauses or 0) >= 4 or (paused_s or 0) >= 300:
        noise += 1; signals.append("frequent stops")
    if temp_c is not None and temp_c >= 28:
        noise += 1; signals.append("heat")
    if wind_kmh is not None and wind_kmh >= 25:
        noise += 1; signals.append("wind")
    if noise >= 2:
        return {"reading_quality": "low",
                "reading_quality_note": ("Noisy context (" + ", ".join(signals) + ") — "
                                         "do not conclude progress or regression from "
                                         "this session alone. It still counts as real-"
                                         "world experience.")}
    if noise == 1:
        return {"reading_quality": "medium",
                "reading_quality_note": ("Some external variables (" + signals[0] + ") — "
                                         "the trend is useful, mind the context.")}
    if pauses is None and temp_c is None and wind_kmh is None:
        return {"reading_quality": "medium",
                "reading_quality_note": ("Outdoor session without context data yet — "
                                         "treated as medium until streams and weather "
                                         "are summarized.")}
    return {"reading_quality": "high",
            "reading_quality_note": ("Clean outdoor session — few stops, mild "
                                     "weather. Valid comparison against similar days.")}
...
def get_streams_summary(clean_session_id: str):
    ...
    with conn.cursor() as cur:
        cur.execute("""
            SELECT ss.decoupling_pct, ss.temp_avg_c, ss.temp_max_c, ss.pauses_count,
                   ss.paused_time_s, ss.time_in_zone_s, ss.hr_points,
                   ss.wind_kmh, ss.wind_gust_kmh, ss.weather_label, ss.country,
                   ss.humidity_pct, cs.sport_type
            FROM canonical_sessions cs
            LEFT JOIN session_streams_summary ss USING (clean_session_id)
            WHERE cs.clean_session_id=%s
        """, (clean_session_id,))
    ...
    sport_type = r[12]
    if r[0] is None and r[3] is None and r[1] is None:
        # Sin resumen de streams: el rodillo igual puede declararse controlado
        rq = _reading_quality(sport_type, None, None, None, None)
        return {"ok": True, "available": False, "sport_type": sport_type,
                **rq,
                "message": "No streams summary yet — run POST /api/strava/summarize-streams."}
    ...
    rq = _reading_quality(sport_type,
                          r[3], r[4],
                          float(r[1]) if r[1] is not None else None,
                          wind)
    return {"ok": True, "available": True, "sport_type": sport_type,
            **rq,
