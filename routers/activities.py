...
def list_routes():
    conn = get_db()
    if not conn:
        return {"error": "Database not connected. Set DATABASE_URL."}
    try:
    ...
    try:
        ...
        if len(sessions_list) >= 2:
            ...
            if first.get("avg_hr_bpm") and last.get("avg_hr_bpm"):
                ...
                progress["hr_note"] = (
                    f"HR dropped {abs(hr_delta)} bpm → real aerobic improvement" if hr_delta < -3
                    else f"HR rose {hr_delta} bpm" if hr_delta > 3
                    else "HR steady between first and last run"
                )
            if first.get("avg_speed_kmh") and last.get("avg_speed_kmh"):
                ...
                progress["speed_note"] = (
                    f"Speed rose {spd_delta} km/h across {len(sessions_list)} runs"
                    if spd_delta > 0.5 else "Speed steady"
                )
...
<main>
  <div class="top">
    <button onclick="if(history.length>1){{history.back()}}else{{location.href='/activities'}}">Back</button>
    <a href="/activities">Activities</a>
  </div>
  <section class="card">
    <div class="label">Matched rides</div>
    <h1 id="title">Loading comparison...</h1>
  ...
  <section class="card">
    <div class="scroll">
      <table>
        <thead><tr><th>Date</th><th class="right">Speed</th><th class="right">Time</th><th class="right">HR</th><th class="right">Cad.</th><th class="right">Eff.</th></tr></thead>
...
fetch('/route/' + encodeURIComponent(rid) + '/matched?format=json').then(async r => {{
  ...
  const route = d.route || {{}};
  document.getElementById('title').textContent = route.name || 'Garmin route';
        ...
        with conn.cursor() as cur:
            ...
            if route_sport in ("cycling", "ciclismo", "biking") and route_distance < min_distance_km:
                return {
                    ...
                    "message": f"Route filtered: for cycling, routes of {min_distance_km:g} km or more are prioritized. Lower min_distance_km to inspect this route.",
                }
...
def get_charts(session_id: str):
    ...
    if not records:
        ...
        msg = (
            "This session was imported from Garmin Connect. "
            "Second-by-second charts are only available for activities uploaded as a FIT file."
        ...
        stats_html = (
            _stat("Date", str(session.get("start_time", ""))[:10]) +
            _stat("Sport", session.get("sport", "").replace("_", " ").title()) +
            _stat("Distance", f"{session.get('distance_km', '')}", "km") +
            _stat("Duration", _fmt_dur(session.get("duration_s"))) +
            _stat("Avg HR", session.get("avg_hr_bpm", ""), "bpm") +
            _stat("Avg speed", session.get("avg_speed_kmh", ""), "km/h") +
            _stat("Cadence", session.get("avg_cadence_rpm") or session.get("avg_cadence", ""), "rpm") +
            _stat("Climbing", session.get("ascent_m", ""), "m")
        )
...
<body>
  <main>
    ...
    <section class="card">
      <h2>{session.get('workout_name') or 'Session summary'}</h2>
      <div class="notice">{msg}</div>
      <div class="stats">{stats_html}</div>
      <div class="actions">
        <a class="btn" href="/activities">Activities</a>
        <a class="btn" href="/activities?telemetry=1">View sessions with charts</a>
      </div>
    </section>
  </main>
...
<div class="top">
  <div class="top-left">
    <button class="back" onclick="if(history.length>1){{history.back()}}else{{location.href='/activities'}}">Back</button>
  </div>
  <a href="/activities">Activities</a>
</div>
<main>
<section class="hero">
  ...
  <div class="sub" id="subline">Charts over time</div>
  <div class="mode"><button class="on">Time</button><button>Distance</button></div>
  <div class="summary">
    <div class="mini"><div class="mini-k">Distance</div><div class="mini-v" id="sd">—</div></div>
    <div class="mini"><div class="mini-k">Duration</div><div class="mini-v" id="sdur">—</div></div>
    <div class="mini"><div class="mini-k">Avg HR</div><div class="mini-v" id="shr">—</div></div>
    <div class="mini"><div class="mini-k">Avg speed</div><div class="mini-v" id="sspd">—</div></div>
  </div>
</section>
<section class="section"><div class="section-title">Map</div><div class="mapbox" id="mapBox"><div class="mapnote">Loading map...</div></div></section>
...
<section class="section"><div class="section-title">Mars Profile Analysis</div><div id="marsAnalysis" style="color:#c8c8cf;font-size:15px">Loading analysis...</div></section>
</main>
<script>
{data_js}
function fmtDate(v){{
...
document.getElementById('ttl').textContent=(sessionInfo.workout_name||fmtDate(sessionInfo.start_time)||'Session').slice(0,40)||'Session';
  ...
  try{{
    ...
    if(a&&a.analysis){{
      ...
      document.getElementById('marsAnalysis').innerHTML=
        ...
        '<div style="padding:8px 12px;background:rgba(61,214,140,.08);border-left:3px solid #3dd68c;border-radius:6px"><b style="color:#3dd68c">Zone:</b> '+an.zone+'</div>'+
        '<div style="padding:8px 12px;background:rgba(74,158,255,.08);border-left:3px solid #4a9eff;border-radius:6px"><b style="color:#4a9eff">Cadence:</b> '+an.cadence+'</div>'+
        '<div style="padding:8px 12px;background:rgba(167,139,250,.08);border-left:3px solid #a78bfa;border-radius:6px"><b style="color:#a78bfa">Efficiency:</b> '+an.efficiency+'</div>'+
        '<div style="padding:8px 12px;background:rgba(232,89,60,.08);border-left:3px solid #e8593c;border-radius:6px"><b style="color:#e8593c">vs History:</b> '+an.comparison+'</div>'+
        '</div>';
    }}else{{document.getElementById('marsAnalysis').textContent='No analysis data';}}
  }}catch(e){{document.getElementById('marsAnalysis').textContent='Analysis unavailable';}}
}})();
...
def analyze_session_vs_profile(session_id: str):
    ...
    try:
        ...
        if hr < trans[0]: zone_eval="Z1 recovery — very easy"
        elif hr <= trans[1]: zone_eval="Transition — warm-up/cool-down"
        elif hr < z2lo: zone_eval="Z1 very easy — raise intensity"
        elif hr <= z2hi: zone_eval="Z2 perfect — aerobic engine"
        elif hr <= z3hi: zone_eval="Z3 tempo — good threshold"
        else: zone_eval="Z4/Z5 — high intensity, recover"
        cad_obj = p.get("cadencia_obj",100)
        cad_eval = f"Cadence {cadval:.0f} rpm — {'+' if cadval>cad_obj else ''}{cadval-cad_obj:.0f} vs target {cad_obj}"
        eff = round(spd/hr,4) if hr>0 and spd>0 else None
        eff_base = p.get("eff_base",0.1483)
        eff_eval = f"Efficiency {eff:.4f} ({'+' if eff and eff>eff_base else ''}{(eff-eff_base):.4f} vs base)" if eff else "No efficiency data"
        comp = []
        if hist["avg_hr"] and hr: comp.append(f"HR {'+' if hr-hist['avg_hr']>0 else ''}{hr-hist['avg_hr']:.1f} bpm vs hist")
        if hist["avg_spd"] and spd: comp.append(f"speed {'+' if spd-hist['avg_spd']>0 else ''}{spd-hist['avg_spd']:.1f} km/h vs hist")
        ...
        return {"session":s,"analysis":{"zone":zone_eval,"cadence":cad_eval,"efficiency":eff_eval,
                "comparison":" · ".join(comp) or "Not enough history"},"historical":hist}
