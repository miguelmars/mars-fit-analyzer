...
_CAP_LABEL = {"aerobic_fitness": "motor aeróbico", "endurance": "resistencia",
              "power": "potencia", "strength_endurance": "fuerza-resistencia",
              "recovery": "recuperación", "tempo": "umbral"}
...
def weekly_intelligence(week_offset: int = Query(0, ge=0, le=52,
    ...
    with conn.cursor() as cur:
        ...
        cur.execute("""
            ...
            FROM clean_sessions CROSS JOIN wk
    ...
    obs = (f"OBSERVACIÓN: {n_ses} sesiones, {km} km, {hours} h. "
           + " · ".join(f"{v}× {k}" for k, v in types_count.items())
           + f". Semana previa: {prev_ctx['sessions']} sesiones, {prev_ctx['km']} km.")
    if not sessions:
        interp = "INTERPRETACIÓN: Semana sin sesiones registradas."
        cap_txt = "CAPACIDAD CONSTRUIDA: ninguna esta semana."
    else:
        interp = (f"INTERPRETACIÓN: El tiempo de la semana se concentró en "
                  f"{_CAP_LABEL.get(dominant, dominant)} ({cap_pct.get(dominant, 0)}%). "
                  f"{structured_n} de {n_ses} sesiones tuvieron estructura de bloques.")
        cap_txt = ("CAPACIDAD CONSTRUIDA: " +
                   ", ".join(f"{_CAP_LABEL.get(c, c)} {p}%" for c, p in cap_pct.items()) + ".")
    evid = (f"EVIDENCIA: {with_hr}/{n_ses} sesiones con FC, {with_laps}/{n_ses} con laps, "
            f"{structured_n} estructuradas." if sessions else "EVIDENCIA: sin sesiones.")
    conf_pct = round((with_hr / n_ses) * 100) if n_ses else 0
    conf = (f"CONFIANZA: {'alta' if conf_pct >= 80 else 'media' if conf_pct >= 50 else 'baja'} "
            f"({conf_pct}% de sesiones con FC).")
    missing = []
    if not has_goal:
        missing.append("meta activa")
    if not has_plan:
        missing.append("plan estructurado")
    if n_ses and with_laps < n_ses:
        missing.append(f"laps en {n_ses - with_laps} sesiones")
    if wellness_checks is not None and wellness_checks < 4:
        missing.append(f"checks matutinos ({wellness_checks or 0}/7)")
    falta = "QUÉ FALTA: " + (", ".join(missing) + "." if missing else "nada crítico.")
    if not sessions:
        sug = "SUGERENCIA PRUDENTE: sin datos esta semana — registrar o sincronizar actividades."
    elif not (has_goal or has_plan):
        sug = ("SUGERENCIA PRUDENTE: la semana se explica sola, pero sin meta ni plan no se "
               "puede decir si construye en la dirección correcta. Registrar una meta daría dirección.")
    else:
        sug = (f"SUGERENCIA PRUDENTE: el volumen se concentró en {_CAP_LABEL.get(dominant, dominant)}; "
               "contrastar con lo que pide la fase actual del plan.")
...
def seed_garmin_plan():
    ...
    with conn.cursor() as cur:
        ...
        if cur.fetchone():
            return {"ok": True, "seeded": False, "message": "Plan ya existe — sin cambios."}
        cur.execute("""
            ...
            VALUES (%s, %s, 'garmin_coach', '2026-05-04', '2026-10-03', 22, 'active',
                    %s::jsonb)
        """, (plan_id, "Garmin Coach — Time Trial Plan",
              '{"phases":[{"name":"base","start":"2026-05-04","end":"2026-06-27",'
              '"focus":"aerobic fitness y volumen"},'
              '{"name":"build","start":"2026-06-28","end":"2026-08-22",'
              '"focus":"intensidad y capacidad aerobica"},'
              '{"name":"peak","start":"2026-08-23","end":"2026-10-03",'
              '"focus":"afinar fitness con rodadas largas e intensas"}],'
              '"event":"Time Trial","fuente":"screenshots Garmin 2026-06-10"}'))
        ...
        cur.execute("""
            ...
            VALUES
              ...
              (%s, 6, '2026-06-11', 'ride', 'Sesión del plan Garmin (tipo por confirmar)', NULL),
              (%s, 6, '2026-06-13', 'ride', 'Sesión del plan Garmin (tipo por confirmar)', NULL)
        """, (plan_id, plan_id, plan_id))
    conn.commit()
    return {"ok": True, "seeded": True, "plan_id": plan_id,
            "message": "Plan Time Trial 22 semanas sembrado con 3 sesiones de la semana 6. "
                       "GET /gpt/training-context ahora tiene plan estructurado."}
...
def _match_plan_sessions(conn):
    ...
    with conn.cursor() as cur:
        ...
        for ps_id, pdate in pending:
            cur.execute("""
                SELECT clean_session_id FROM clean_sessions
                WHERE start_date = %s
...
def deactivate_plan(plan_id: str, reason: str = Query("otro", max_length=200),
    ...
    with conn.cursor() as cur:
        ...
        if row[0] != "active":
            return {"ok": True, "changed": False,
                    "message": f"El plan ya estaba '{row[0]}' — sin cambios."}
        cur.execute("""
    ...
    return {"ok": True, "changed": True, "plan_id": plan_id, "status": outcome,
            "reason": reason,
            "message": "Plan dado de baja. Todo el registro histórico se conserva — "
                       "sesiones, matches y cumplimiento siguen consultables."}
