...
def _create_epoch_notification(conn, category, title, body, severity="info", route=None,
                               source=None, payload=None, dedupe_key=None):
    ...
    with conn.cursor() as cur:
        if dedupe_key:
            cur.execute("""
                ...
                WHERE category=%s
                  AND COALESCE(payload->>'dedupe_key','')=%s
...
def _threshold_evidence_summary(conn, sport="cycling"):
    ...
    next_action = "Add a threshold test or trusted external threshold evidence."
    message = "No active threshold anchor yet."
    if active_lt:
        message = f"Current {sport} threshold is {active_lt} bpm."
        if external_matching:
            signal = "confirms"
            next_action = "Keep current zones and keep collecting confirmation through hard sessions."
            names = sorted({e["source_name"] for e in external_matching})
            message += " External confirmation: " + ", ".join(names) + "."
        elif conflicting:
            signal = "questions"
            review_required = True
            next_action = "Review zones before relying on threshold-heavy Build work."
            message += " External evidence disagrees; review zones."
        else:
            signal = "insufficient_data"
            next_action = "Add external or test evidence to raise threshold confidence."
            message += " Add external or test evidence to raise confidence."
    elif evidence:
        signal = "questions"
        review_required = True
        next_action = "Create or update the active zone anchor from the best evidence."
...
def save_threshold_evidence(body: ThresholdEvidenceIn):
    ...
    try:
        ...
        notification_id = _create_epoch_notification(
            conn,
            category="threshold_evidence_found",
            severity=severity,
            title=title,
            body=note_body,
            route="/progress",
            ...
            dedupe_key=(
                f"threshold:{body.sport}:{body.threshold_bpm}:"
                f"{body.source_name}:{body.evidence_type}:{observed_at}"
            ),
        )
        return {
            "ok": True,
...
def workout_analysis(clean_session_id: str):
    ...
    if not laps:
        ...
        duration_h = (session["duration_s"] or 0) / 3600.0
        ascent_m = session["ascent_m"] or 0
        avg_hr = session["avg_hr_bpm"]
        ascent_per_km = (ascent_m / distance_km) if distance_km else 0
        threshold = _threshold_evidence_summary(conn, "cycling")
        ...
        elif ascent_per_km > 15:
            workout_type = "climb"
        elif duration_h >= 2.5 or distance_km >= 65:
            workout_type = "long_ride"
        elif active_lt and avg_hr and avg_hr >= int(active_lt) - 15:
            workout_type = "tempo"
        elif active_lt and avg_hr and avg_hr >= int(active_lt) - 25:
            workout_type = "z2"
        else:
            workout_type = "z2"

        summary_cap_map = {
            "tempo": "tempo durability",
            "climb": "climbing strength-endurance",
            "long_ride": "durable aerobic endurance",
            "z2": "aerobic base",
            "recovery": "recovery",
        }
        capacity_built = summary_cap_map.get(workout_type, "aerobic base")
        training_effect_summary = (
            f"Provisional read: this workout likely built {capacity_built}, "
            "but block/lap data is missing."
        )
        threshold_message = "No laps/HR distribution available for threshold reading."
        if active_lt and avg_hr:
            threshold_message = (
                f"Average HR was {round(avg_hr)} bpm against active threshold "
                f"{int(active_lt)} bpm, but laps/HR distribution are missing."
            )
        threshold_signal = {"status": "insufficient_data",
                            "message": threshold_message,
                            "active_threshold_bpm": int(active_lt) if active_lt else None}
        next_action = {
            "type": "transform_laps",
            "label": "Transform laps or inspect source data",
            "message": "Run lap/stream transform before making the final workout read.",
        }
        return {"ok": True, "session": session, "blocks": [],
                "workout_type": workout_type, "structured": False,
                "confidence_score": 0.25,
                "warnings": ["No laps recorded — this is a provisional summary-based read."],
                ...
                "explanation_text": (
                    f"No blocks recorded for this session. Provisional classification is "
                    f"{workout_type} from summary metrics only: {round(duration_h, 2)} h, "
                    f"{round(distance_km, 1)} km, {round(ascent_m)} m ascent"
                    + (f", {round(avg_hr)} bpm avg HR." if avg_hr else ".")
                )}
...
_GARMIN_TT_PLAN_ID = "garmin_tt_2026"
_GARMIN_TT_START = date(2026, 5, 4)
_GARMIN_TT_END = date(2026, 10, 3)
_GARMIN_TT_TOTAL_WEEKS = 22


def _garmin_tt_phase_for_date(planned_date: date) -> dict[str, str]:
    if date(2026, 5, 4) <= planned_date <= date(2026, 6, 27):
        return {"name": "base", "focus": "aerobic fitness and volume"}
    if date(2026, 6, 28) <= planned_date <= date(2026, 8, 22):
        return {"name": "build", "focus": "intensity and aerobic capacity"}
    return {"name": "peak", "focus": "sharpening fitness with long, intense rides"}


def _garmin_tt_target(
    *,
    phase: dict[str, str],
    session_type: str,
    duration_min: int,
    intensity: str,
    source: str = "garmin_coach_reconstructed",
    ...
    target = {
        ...
        "intensity": intensity,
        "primary_zone": intensity,
        "source": source,
        "source_confidence": source_confidence,
        "source_note": (
            "Reconstructed from confirmed Garmin Time Trial phases and known workout "
            "patterns; exact Garmin title is only treated as confirmed when marked so."
        ),
    }
    if details:
...
def _garmin_tt_session(planned_date: date, session_type: str, description: str,
                       duration_min: int, intensity: str,
                       source_confidence: str = "medium",
                       details: Optional[dict[str, Any]] = None) -> dict[str, Any]:
    phase = _garmin_tt_phase_for_date(planned_date)
    week_number = ((planned_date - _GARMIN_TT_START).days // 7) + 1
    return {
        "week_number": week_number,
        "planned_date": planned_date,
        "session_type": session_type,
        "description": description,
        "target": _garmin_tt_target(
            phase=phase,
            session_type=session_type,
            duration_min=duration_min,
            intensity=intensity,
...
def _build_garmin_tt_plan_sessions() -> list[dict[str, Any]]:
    sessions: list[dict[str, Any]] = []
    for week in range(1, _GARMIN_TT_TOTAL_WEEKS + 1):
        monday = _GARMIN_TT_START + timedelta(days=(week - 1) * 7)

        if week <= 8:
            long_duration = 120 if week < 4 else 150 if week < 7 else 165
            sessions.extend([
                _garmin_tt_session(monday + timedelta(days=1), "z2_endurance",
                                   "Aerobic endurance ride (Garmin TT Base scaffold)",
                                   75, "z2"),
                _garmin_tt_session(monday + timedelta(days=3), "tempo_development",
                                   "Tempo development ride (Garmin TT Base scaffold)",
                                   75, "tempo",
                                   details={"work": "steady tempo blocks"}),
                _garmin_tt_session(monday + timedelta(days=5), "long_ride",
                                   "Long endurance ride (Garmin TT Base scaffold)",
                                   long_duration, "z2"),
                _garmin_tt_session(monday + timedelta(days=6), "recovery",
                                   "Recovery spin (Garmin TT Base scaffold)",
                                   45, "recovery"),
            ])
        elif week <= 16:
            sessions.extend([
                _garmin_tt_session(monday + timedelta(days=1), "threshold_intervals",
                                   "Threshold intervals (Garmin TT Build scaffold)",
                                   80, "threshold",
                                   details={"work": "threshold repeats"}),
                _garmin_tt_session(monday + timedelta(days=3), "sweet_spot",
                                   "Sweet spot durability ride (Garmin TT Build scaffold)",
                                   90, "sweet_spot",
                                   details={"work": "sustained sweet spot"}),
                _garmin_tt_session(monday + timedelta(days=5), "long_intense_ride",
                                   "Long ride with intensity (Garmin TT Build scaffold)",
                                   180, "z2_tempo_mix"),
                _garmin_tt_session(monday + timedelta(days=6), "recovery",
                                   "Recovery spin (Garmin TT Build scaffold)",
                                   45, "recovery"),
            ])
        elif week <= 20:
            sessions.extend([
                _garmin_tt_session(monday + timedelta(days=1), "race_pace_intervals",
                                   "Race pace intervals (Garmin TT Peak scaffold)",
                                   75, "threshold"),
                _garmin_tt_session(monday + timedelta(days=3), "vo2_high_intensity",
                                   "High-intensity sharpening ride (Garmin TT Peak scaffold)",
                                   65, "high_intensity",
                                   details={"work": "short hard repeats"}),
                _garmin_tt_session(monday + timedelta(days=5), "long_intense_ride",
                                   "Long intense ride (Garmin TT Peak scaffold)",
                                   150, "z2_threshold_mix"),
                _garmin_tt_session(monday + timedelta(days=6), "recovery",
                                   "Recovery spin (Garmin TT Peak scaffold)",
                                   40, "recovery"),
            ])
        elif week == 21:
            sessions.extend([
                _garmin_tt_session(monday + timedelta(days=1), "race_pace_openers",
                                   "Race pace openers (Garmin TT taper scaffold)",
                                   60, "threshold"),
                _garmin_tt_session(monday + timedelta(days=3), "z2_endurance",
                                   "Reduced endurance ride (Garmin TT taper scaffold)",
                                   60, "z2"),
                _garmin_tt_session(monday + timedelta(days=5), "openers",
                                   "Short openers (Garmin TT taper scaffold)",
                                   45, "tempo"),
                _garmin_tt_session(monday + timedelta(days=6), "recovery",
                                   "Recovery spin (Garmin TT taper scaffold)",
                                   35, "recovery"),
            ])
        else:
            sessions.extend([
                _garmin_tt_session(monday + timedelta(days=1), "openers",
                                   "Race-week openers (Garmin TT Peak scaffold)",
                                   45, "tempo"),
                _garmin_tt_session(monday + timedelta(days=3), "recovery",
                                   "Easy spin before event (Garmin TT Peak scaffold)",
                                   35, "recovery"),
                _garmin_tt_session(date(2026, 10, 3), "event",
                                   "Time Trial event",
                                   90, "race",
                                   source_confidence="high",
                                   details={"event": "Time Trial"}),
            ])

    confirmed = {
        date(2026, 6, 10): {
            "session_type": "tempo_intervals",
            "description": "5 Min. Tempo Intervals (plan Garmin)",
            "target": _garmin_tt_target(
                phase=_garmin_tt_phase_for_date(date(2026, 6, 10)),
                session_type="tempo_intervals",
                duration_min=75,
                intensity="tempo",
                source="garmin_coach_confirmed",
                source_confidence="high",
                details={"intervals": {"work_min": 5, "zone": "tempo"}},
            ),
        },
        date(2026, 6, 11): {
            "session_type": "ride",
            "description": "Garmin plan session (type to confirm)",
            "target": _garmin_tt_target(
                phase=_garmin_tt_phase_for_date(date(2026, 6, 11)),
                session_type="ride",
                duration_min=75,
                intensity="unknown",
        ...
        date(2026, 6, 13): {
            ...
            "description": "Garmin plan session (type to confirm)",
            "target": _garmin_tt_target(
                phase=_garmin_tt_phase_for_date(date(2026, 6, 13)),
                session_type="ride",
                duration_min=120,
                intensity="unknown",
                source="garmin_coach_confirmed_date",
    ...
    by_date = {session["planned_date"]: session for session in sessions}
    for planned_date, override in confirmed.items():
        phase = _garmin_tt_phase_for_date(planned_date)
        by_date[planned_date] = {
            "week_number": ((planned_date - _GARMIN_TT_START).days // 7) + 1,
            "planned_date": planned_date,
        ...
        by_date[planned_date]["target"]["phase"] = phase["name"]
        by_date[planned_date]["target"]["phase_focus"] = phase["focus"]
...
def seed_garmin_plan():
    """
    Siembra el plan Garmin Coach como dato estructurado. Idempotente: refresca
    fases y hace upsert por fecha sin duplicar ni perder matches existentes.
    ...
    plan_meta = {
        ...
        "event": "Time Trial",
        "source": "Garmin Coach screenshots + EPOCH reconstructed scaffold",
        "source_policy": "Confirmed dates/titles are marked high confidence; reconstructed sessions are usable plan truth until Garmin official sync is available.",
    }
    sessions = _build_garmin_tt_plan_sessions()
    inserted = updated = confirmed_sessions = reconstructed_sessions = 0
    with conn.cursor() as cur:
        ...
        for session in sessions:
            ...
            if str(target.get("source", "")).startswith("garmin_coach_confirmed"):
                confirmed_sessions += 1
            else:
                reconstructed_sessions += 1

            cur.execute("""
                SELECT id, matched_clean_session_id, status
                FROM plan_sessions
                WHERE plan_id=%s AND planned_date=%s
                ORDER BY matched_clean_session_id IS NOT NULL DESC, id
                LIMIT 1
            """, (plan_id, session["planned_date"]))
            ...
            if existing:
                cur.execute("""
                    ...
                    SET week_number=%s,
                        ...
                        target=%s::jsonb,
                        status=CASE
                            WHEN matched_clean_session_id IS NOT NULL THEN status
                            WHEN status IN ('completed', 'moved') THEN status
                            ELSE 'planned'
                        END
                    WHERE id=%s
                """, (session["week_number"], session["session_type"],
                      session["description"], json.dumps(target), existing[0]))
                updated += 1
            else:
                cur.execute("""
                    INSERT INTO plan_sessions
                        (plan_id, week_number, planned_date, session_type, description, target)
                    VALUES (%s, %s, %s, %s, %s, %s::jsonb)
                """, (plan_id, session["week_number"], session["planned_date"],
                      session["session_type"], session["description"],
                      json.dumps(target)))
                inserted += 1
    conn.commit()
    return {
        "ok": True,
        "seeded": not plan_existed,
        "updated": plan_existed,
        "plan_id": plan_id,
        "weeks": _GARMIN_TT_TOTAL_WEEKS,
        "sessions": len(sessions),
        "inserted_sessions": inserted,
        "updated_sessions": updated,
        "confirmed_sessions": confirmed_sessions,
        "reconstructed_sessions": reconstructed_sessions,
        "phases": plan_meta["phases"],
        "message": (
            "22-week Time Trial plan is structured for Base, Build and Peak. "
            "Confirmed Garmin sessions stay marked; reconstructed sessions are "
            "usable plan truth until official Garmin sync is available."
        ),
    }
...
def phase_report(phase: str = Query(None, description="base|build|peak. Empty = current phase")):
    ...
    if phase_name == "base":
        capacity_built.append("aerobic base")
        if eff_delta_pct is not None and eff_delta_pct > 1:
            capacity_built.append("aerobic efficiency")
    elif phase_name == "build":
        capacity_built.append("threshold durability")
        capacity_built.append("repeatable intensity")
    elif phase_name == "peak":
        capacity_built.append("race specificity")
        capacity_built.append("sharpness")
    if t[6] and t[6] >= max(2, weeks_elapsed * 2):
        capacity_built.append("training consistency")

    completion_rate = None
    if pc[0]:
        completion_rate = round(float(pc[1] or 0) / float(pc[0]) * 100, 1)

    threshold = _threshold_evidence_summary(conn, "cycling")
    testing_calibration = _testing_calibration_recommendation(conn, target, "cycling")

    verdict = "building"
    verdict_reason = "The phase has usable work recorded, but the signal is still developing."
    if int(t[0] or 0) == 0:
        verdict = "insufficient_data"
        verdict_reason = "No completed sessions are available inside this phase window."
    elif threshold.get("review_required"):
        verdict = "needs_threshold_review"
        verdict_reason = "Training exists, but threshold evidence questions the current zones."
    elif completion_rate is not None and completion_rate >= 70 and built:
        verdict = "on_track"
        verdict_reason = "Plan completion and adaptation signals are both strong enough."
    elif completion_rate is not None and completion_rate < 35 and not in_progress:
        verdict = "underbuilt"
        verdict_reason = "Too little of the registered plan was completed in this phase."

    if verdict == "needs_threshold_review":
        next_action = {
            "type": "review_threshold",
            "label": "Review zones before the next hard block",
            "reason": threshold.get("next_action"),
        }
    elif testing_calibration.get("recommended") and testing_calibration.get("test"):
        next_action = {
            "type": "schedule_calibration_test",
            "label": testing_calibration["test"]["name"],
            "reason": "Calibration is the next useful evidence for this phase.",
        }
    elif verdict == "underbuilt":
        next_action = {
            "type": "repeat_foundation",
            "label": "Repeat the key foundation work before progressing",
            "reason": "The phase did not yet create enough completed work to progress cleanly.",
        }
    else:
        next_action = {
            "type": "continue_plan",
            "label": "Continue the plan",
            "reason": "Current evidence supports moving forward.",
        }
    ...
    return {
        ...
        "evidence": {
            ...
            "plan_completion_rate_pct": completion_rate,
        ...
        "threshold_evidence": {
            ...
            "review_required": threshold.get("review_required"),
            "active_anchor": threshold.get("active_anchor"),
            "latest_evidence": threshold.get("latest_evidence"),
            ...
            "next_action": threshold.get("next_action"),
            "message": threshold.get("message"),
        },
        ...
        "explanation_text": (
            f"{obs} VERDICT: {verdict.replace('_', ' ')} - {verdict_reason} "
            f"{interp} {cap_txt} NEXT: {next_action['label']}. "
            f"{falta} {sin_culpa}{threshold_msg}{testing_msg}"
        ).strip(),
    }
