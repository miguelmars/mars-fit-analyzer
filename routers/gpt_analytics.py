...
from db import (
    ...
    _ensure_goals_table,
    _ensure_zone_model_system,
    _ensure_clean_sessions_compat_view,
)
from shared.models import AthleteTestIn
from mars_context import _get_profile, MARS_ZONES
from mars_load import build_training_load

logger = logging.getLogger("mars_fit")
router = APIRouter(tags=["gpt_analytics"])

CURRENT_TRAINING_ERA_START = "2026-04-28"


def _zone_key_for_sport(sport: str) -> str:
    return "running" if sport in ("running", "trail_running", "treadmill_running") else "cycling"


def _active_threshold_hr(conn, sport: str) -> dict:
    """Current LTHR source of truth: active zone model, with static fallback."""
    zone_key = _zone_key_for_sport(sport)
    fallback = MARS_ZONES[zone_key]["lt_bpm"]
    try:
        _ensure_zone_model_system(conn)
        with conn.cursor() as cur:
            cur.execute("""
                SELECT model_id, lt_bpm, source, effective_from, method
                FROM zone_models
                WHERE sport=%s AND status='active' AND lt_bpm IS NOT NULL
        ...
        try:
            ...
            pass
        logger.warning(f"Active zone lookup failed; using fallback: {e}")
    return {"value": fallback, "source": "mars_zones.default", "model_id": None,
            "raw_source": "fallback", "effective_from": None, "method": "static"}


def _active_ftp_watts(conn) -> Optional[int]:
    """FTP is optional; without it power data must not drive TSS."""
    try:
        with conn.cursor() as cur:
            cur.execute("""
                SELECT ftp_watts FROM athlete_profile
...
def _active_plan_phase(conn, today) -> Optional[dict]:
    """Structured plan phase beats generic weeks-to-event math."""
    try:
        with conn.cursor() as cur:
            cur.execute("""
                SELECT plan_id, name, meta
                FROM training_plans
                WHERE status='active'
        ...
        for phase in ((row[2] or {}).get("phases") or []):
            name = (phase.get("name") or "").lower()
            if phase.get("start", "") <= today_s <= phase.get("end", ""):
                return {
                    "plan_id": row[0],
                    "plan_name": row[1],
                    "name": name,
        ...
        try:
            ...
            pass
        logger.warning(f"Structured plan phase lookup failed: {e}")
    return None
...
def adaptive_coach():
    ...
    if gr:
        ...
        if gr[3]:
            ...
            phase_source = "weeks_to_event_fallback"

    structured_phase = _active_plan_phase(conn, _date.today())
    if structured_phase and structured_phase.get("name") in {"base", "build", "peak", "taper"}:
        phase = structured_phase["name"]
        phase_source = "structured_training_plan"
...
def gpt_fitness_timeline(sport: str = "cycling"):
    ...
    try:
        ...
        if recientes:
            ...
            indicadores["tendencia"] = "improving" if len(recientes) == 2 and float(recientes[-1].get("eff_ratio") or 0) > float(recientes[-2].get("eff_ratio") or 0) else "steady"
        return {"sport": sport, "timeline": data, "indicadores_mars": indicadores}
    except Exception as e:
        raise HTTPException(500, str(e))


# ── GET /gpt/training-load ────────────────────────────────────────────────────

@router.get("/gpt/training-load")
def gpt_training_load(sport: str = "cycling", days: int = 180):
    """Fitness / Fatigue / Form (CTL / ATL / TSB) from the athlete's own workouts.

    Tiered load model (see mars_load.py): power when available, otherwise heart
    rate, pace, or duration — never assumes a power meter. Threshold HR comes from
    the Mars zones (lt_bpm) and is cross-checked against training history.

    Args:
        sport: activity type (default "cycling").
        days: history window to build the curve (default 180; CTL needs ~42 days
              of continuous data to be meaningful).
    """
    conn = get_db()
    if not conn:
        raise HTTPException(503, "DB no disponible")
    ...
    threshold_info = _active_threshold_hr(conn, sport)
    threshold_hr = threshold_info["value"]
    ftp_watts = _active_ftp_watts(conn)

    try:
        with conn.cursor() as cur:
            cur.execute("""
                SELECT start_time, duration_s, avg_hr_bpm, avg_speed_kmh,
                       normalized_power_w, avg_power_w, power_available
                FROM sessions_clean_compat
                WHERE sport = %s
                  AND start_time IS NOT NULL
                  AND duration_s > 0
                  AND start_time::date >= GREATEST((CURRENT_DATE - %s::int), %s::date)
                ORDER BY start_time ASC
            """, (sport, days, CURRENT_TRAINING_ERA_START))
            rows = cur.fetchall()

        sessions = [{
            "date": (r[0] or "")[:10],
            "duration_s": r[1],
            "avg_hr_bpm": float(r[2]) if r[2] is not None else None,
            "avg_speed_kmh": float(r[3]) if r[3] is not None else None,
            # Power tier only activates when both power and FTP exist.
            "normalized_power_w": float(r[4] or r[5]) if (r[4] or r[5]) else None,
            "power_available": bool(r[6]),
        } for r in rows]

        load = build_training_load(sessions, {
            "threshold_hr": threshold_hr,
            "ftp_watts": ftp_watts,
        })

        # Validate the configured threshold against history. A session's average HR
        # cannot exceed LTHR by much, so the highest sustained average HR is a useful
        # sanity ceiling — honest by design, never alarmist.
        sustained = [s["avg_hr_bpm"] for s in sessions
                     if s.get("avg_hr_bpm") and (s.get("duration_s") or 0) >= 1200]
        observed_ceiling = round(max(sustained)) if sustained else None
        if observed_ceiling is None:
            agrees, note = True, "Not enough sustained sessions to validate."
        elif observed_ceiling > threshold_hr + 3:
            agrees, note = False, (f"Sustained HR reached {observed_ceiling} bpm, above the "
                                   f"configured {threshold_hr} — your threshold may be too low.")
        elif observed_ceiling < threshold_hr - 15:
            agrees, note = True, (f"Mostly easy training (peak sustained avg {observed_ceiling} bpm); "
                                  f"no recent threshold efforts to confirm {threshold_hr}.")
        else:
            agrees, note = True, f"Consistent with your history (peak sustained avg {observed_ceiling} bpm)."

        load["sport"] = sport
        load["training_era"] = {
            "start_date": CURRENT_TRAINING_ERA_START,
            "source": "founder_review_2026_06_25",
            "note": "Current cycling thresholds are judged from the post-zone-change era, not the full nine-year history.",
        }
        load["threshold_hr"] = {
            "value": threshold_hr,
            "source": threshold_info["source"],
            "model_id": threshold_info.get("model_id"),
            "effective_from": threshold_info.get("effective_from"),
        ...
        load["ftp_watts"] = {
            "value": ftp_watts,
            "source": "athlete_profile" if ftp_watts else None,
            "note": "Power TSS enabled." if ftp_watts else "No FTP saved; power data falls back to heart rate.",
        }
        return load
    except Exception as e:
        raise HTTPException(500, str(e))
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            cur.execute("""
                ...
                JOIN canonical_sessions cs USING (clean_session_id)
    ...
    try:
        ...
        return {
            ...
            "peso_rendimiento": {
                ...
                "lectura": (
                    "Even controlling by year, the association comes out counterintuitive; "
                    "it must not be used to recommend weight until route, volume and form are controlled for."
                    if weight_confounded else
                    f"Comparing weeks within the same year, losing 1 kg is associated with "
                    f"{-weight_regression['slope']:+.5f} of {sport} efficiency."
                    if weight_regression else "Not enough weeks with matching weight and efficiency."
                ),
            },
            "notas": [
                "Rest compares the prior interval with the next session efficiency.",
                "Volume uses the previous week km against the following week efficiency.",
                "Heat statistically controls for speed, but not route, wind or elevation.",
                "Weight is not interpolated and is compared within each year to avoid mixing eras.",
            ],
        }
    except Exception as e:
        raise HTTPException(500, str(e))
