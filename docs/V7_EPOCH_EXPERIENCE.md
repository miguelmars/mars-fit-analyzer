...
| V7.1 | **Workout Identity + Progression** (firma de estructura + route_id + nombre) | El corazón: "¿mejoré en ESTE workout?" — data ya existe en session_laps |
| V7.2 | My Plan + Home por escenas + Phase Report Card | El cambio visible: plan vivo, sin culpa, con evidencia |
| V7.3 | Capacidades spine + Event Readiness gap + Epoch Tests | Conecta workout↔fase↔evento↔capacidades — el círculo completo |
| V7.4 | Epochs (eras) + Speak Epoch (EN) + dominio useepoch.app | El momento marca: historia + inglés + identidad |
