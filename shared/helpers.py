...
def detect_and_save_achievements(conn, session_id, result):
    ...
    new_achievements = []
    checks = [
        ("max_distance", "distance_km", s.get("distance_km"), "Longest distance"),
        ("max_ascent", "ascent_m", s.get("ascent_m"), "Most climbing"),
        ("max_speed", "avg_speed_kmh", s.get("avg_speed_kmh"), "Highest average speed"),
        ("best_efficiency", "efficiency",
         round(float(s.get("avg_speed_kmh", 0)) / float(s.get("avg_hr_bpm", 1)), 4)
         if s.get("avg_hr_bpm") else None,
         "Best aerobic efficiency"),
    ]
