...
async function loadPlan(){
  ...
  try{
    ...
    if(!tp.ok||!tp.plan){
      el.innerHTML='<div class="card"><div style="font-size:13px;line-height:1.6">No hay plan activo. '+
    ...
    const ring='<div class="card" style="text-align:center">'+
      ...
      '<div style="font-size:12px;color:var(--muted);margin-top:8px">de '+total+' semanas · '+fmtDate(p.start_date)+' → '+fmtDate(p.end_date)+'</div>'+
    '</div>';
    ...
    const weekRows=(tp.this_week||[]).map(function(s2){
      ...
      '</div>';
    }).join('')||'<div style="font-size:12px;color:var(--muted);padding:8px 0">Sin sesiones registradas esta semana — se agregan con la captura semanal del plan.</div>';
    ...
    if(erg&&erg.ok&&erg.goal&&erg.readiness_score!=null){
      ...
      gapCard='<div class="card" style="border-left:3px solid #f59e0b">'+
        ...
        ((erg.data_gaps||[]).length?'<div style="font-size:10px;color:var(--muted);margin-top:6px">Sin datos: '+erg.data_gaps.join(', ')+'</div>':'')+
      '</div>';
    }
...
window.showPhaseReport=async function(btn,phase){
  ...
  try{
    ...
    box.innerHTML='<b style="color:#fb923c">Reporte · '+phase.toUpperCase()+(d.in_progress?' (in progress)':'')+'</b><br>'+
      ...
      '<span>'+t.sessions+' sesiones</span><span>'+t.km+' km</span><span>'+t.hours+' h</span><span>'+t.ascent_m+' m↑</span>'+
