...
async function loadProgress(mode){
  ...
  try{
    ...
    const [hist,mp,tr,perf,wh,corr,tend,ltd,athletic,stravaYearly,stravaWeeks,wgroups,epochsData,thresholdEvidence]=(await Promise.allSettled([
      ...
      mode==='general'?Promise.resolve({}):fetch(API+'/gpt/lt-detect?sport='+sport).then(r=>r.json()),
      ...
      mode==='general'?Promise.resolve({}):fetch(API+'/gpt/threshold-evidence?sport='+sport).then(r=>r.json())
    ...
    const arcLabel=mode==='cycling'?'Bike':mode==='running'?'Run':'Total';
    ...
    const sportTrendHtml=
      '<div class="card" style="border-left:3px solid '+trendColor+'"><div class="head"><h3>'+(mode==='running'?'Running':'Cycling')+' trend</h3><span style="color:'+trendColor+'">'+trendTitle+'</span></div>'+
        ...
        '</div></div>'+
      (function(){
        if(!ltd||!ltd.ok||ltd.consensus_lt_bpm==null)return '';
        const cCol=ltd.confidence==='high'?'#3dd68c':ltd.confidence==='medium'?'#f59e0b':'#8e95a3';
        const d=ltd.delta_from_current;
        return '<div class="card" style="border-left:3px solid '+cCol+'">'+
          '<div class="head"><h3>Fitness signal</h3><span style="color:'+cCol+'">confidence '+ltd.confidence+'</span></div>'+
          '<div class="grid2" style="margin-bottom:8px">'+
            metric('Threshold signal','~'+ltd.consensus_lt_bpm+' bpm','from '+ltd.n_sessions+' sessions')+
            metric('Current zones use',ltd.current_lt_bpm+' bpm',(d!=null?(d>0?'+':'')+d+' bpm vs signal':''))+
          '</div>'+
          '<div style="font-size:11px;color:var(--muted);line-height:1.5">'+(ltd.interpretation||'')+'</div>'+
          '<div style="font-size:10px;color:var(--muted);margin-top:6px"><b>A signal, never the center.</b> Averages mix intensities and altitude (~2300 m) raises HR — this cross-checks your zones, it does not auto-update them.</div>'+
        '</div>';
      })()+
      (function(){
        if(!thresholdEvidence||!thresholdEvidence.ok||!thresholdEvidence.active_anchor)return '';
        const conf=thresholdEvidence.confidence||'needs evidence';
        const col=conf==='very_high'||conf==='high'?'#3dd68c':conf==='medium'?'#f59e0b':'#8e95a3';
        const anchor=thresholdEvidence.active_anchor||{};
        const ev=(thresholdEvidence.evidence||[]).slice(0,3).map(function(e){
          return esc(e.source_name||'Evidence')+' · '+esc(String(e.threshold_bpm||''))+' bpm · '+fmtDate(e.observed_at);
        }).join('<br>');
        return '<div class="card" style="border-left:3px solid '+col+'">'+
          '<div class="head"><h3>Threshold confidence</h3><span style="color:'+col+'">'+esc(conf).replaceAll('_',' ')+'</span></div>'+
          '<div class="grid2" style="margin-bottom:8px">'+
            metric('Active threshold',(anchor.lt_bpm||'--')+' bpm',anchor.source||'zone model')+
            metric('External matches',thresholdEvidence.external_matching_evidence||0,'same threshold')+
          '</div>'+
          '<div style="font-size:11px;color:var(--muted);line-height:1.5">'+esc(thresholdEvidence.message||'')+'</div>'+
          (ev?'<div style="font-size:10px;color:var(--muted);line-height:1.5;margin-top:6px">'+ev+'</div>':'')+
        '</div>';
      })()+
    ...
    function labsCard(){
      ...
      const rows=gs.slice(0,6).map(function(g,i){
        return '<div class="row" style="cursor:pointer" onclick="loadGroupTrend(this,\''+g.route_id+'\',\''+g.sport_type+'\')">'+
          ...
          '<div class="r-main"><div class="r-title">'+esc(g.route_name||('Route '+g.route_id.slice(0,8)))+' · '+esc(g.sport_type)+' <span onclick="event.stopPropagation();renameRoute(\''+g.route_id+'\')" style="font-size:11px;color:var(--muted);cursor:pointer" title="Name route">✏️</span></div>'+
    ...
    window.loadGroupTrend=async function(rowEl,routeId,sport){
      ...
      try{
        ...
        box.innerHTML='<span style="color:'+col+';font-weight:800">'+(v?v.toUpperCase():'FEW REPS')+'</span> · '+esc(d.explanation_text)+
    ...
    function epochsCard(){
      ...
      const rows=eps.map(function(ep){
        ...
        return '<div class="row">'+
          ...
          '<div class="r-main"><div class="r-title" style="color:'+col+'">'+esc(ep.name)+(ep.is_current?' · now':'')+'</div>'+
