async function loadCoach(){
  const el=$('coach-data');
  try{
    const [mp,d,w,wh,ac,lastAct,tc,wi,testRec,ta,sp,calib]=(await Promise.allSettled([
      ...
      fetch(API+'/gpt/starting-point').then(r=>r.json()),
      fetch(API+'/gpt/testing-calibration').then(r=>r.json())
    ...
    if(pains>0){
      obs='Active niggle(s): '+molestias.map(function(m){return esc(m.pain_zone||'area')}).join(', ')+'.';
      ...
      sug=hasPlan?('Continue '+esc(plan.nombre)+' phase '+esc(plan.fase||'Base')+': HR '+z2[0]+'–'+z2[1]+' bpm, cadence '+(mp.cadencia_obj||100)+' rpm.'):('Keep the current pattern: riding Z2 ('+z2[0]+'–'+z2[1]+' bpm) is still your best return.');
    ...
    if(ac && ac.prescription){
      ...
      const goalLine=ac.goal?`<div style="font-size:12px;color:var(--muted);margin-bottom:8px">🎯 ${esc(ac.goal.event_name)}${ac.weeks_to_event!=null?' · '+ac.weeks_to_event+' weeks':''}</div>`:`<div style="font-size:12px;margin-bottom:8px;display:flex;align-items:center;gap:8px"><span style="color:var(--muted)">No active goal</span><button onclick="go('metas')" style="background:rgba(167,139,250,.15);border:1px solid rgba(167,139,250,.35);color:#a78bfa;border-radius:8px;padding:3px 10px;font-size:11px;font-weight:800;cursor:pointer">+ Add goal →</button></div>`;
      ...
      acBlock=`<div class="card" style="border-left:3px solid ${phCol};margin-bottom:12px">
        ...
        <div style="font-size:13px;line-height:1.7">
          ${pr.km_objetivo?`<b>${pr.km_objetivo} km/wk</b> · `:''}${pr.sesiones} sessions · ${pr.z2_pct_objetivo}% Z2<br>
    ...
    const lastActCtx=la?
      `<div style="background:rgba(232,89,60,.07);border:1px solid rgba(232,89,60,.15);border-radius:12px;padding:9px 14px;margin-bottom:12px;display:flex;align-items:center;gap:10px">
        ...
        <div style="flex:1;min-width:0">
          ...
          <div style="font-size:11px;color:var(--muted)">${esc(la.name||la.sport_type)} · ${la.distance_km||'—'} km · ${la.duration_hms||'—'} · FC ${la.avg_hr||'—'} bpm</div>
          <div id="coach-ssum-line"></div>
        </div>
      </div>`:
      '<div style="font-size:11px;color:var(--muted);margin-bottom:12px">No recent Strava activities</div>';
    ...
    if(ac&&ac.goal&&ac.weeks_to_event!=null){
      ...
      countdownCard='<div onclick="go(\'plan\')" class="phase-hero" style="cursor:pointer;border:1px solid var(--line);border-radius:18px;padding:16px 18px;margin-bottom:12px">'+
        '<div style="display:flex;align-items:baseline;gap:8px;margin-bottom:4px">'+
          ...
          '<span style="font-size:13px;font-weight:800">week'+(_wks!==1?'s':'')+' to '+esc(_g.event_name)+'</span>'+
        '</div>'+
    ...
    if(testRec&&testRec.ok&&testRec.recommended&&testRec.test){
      testCard='<div class="card" style="border-left:3px solid #c8f135;margin-bottom:12px">'+
        ...
        '<div style="font-size:12px;line-height:1.6;margin-bottom:8px">'+esc(testRec.explanation_text)+'</div>'+
        '<div style="background:rgba(200,241,53,.07);border:1px solid rgba(200,241,53,.2);border-radius:10px;padding:10px 12px;font-size:11px;line-height:1.6">'+
          '<b>'+esc(testRec.test.name)+'</b><br>'+esc(testRec.test.protocol)+'<br>'+
          '<span style="color:var(--muted)">Where: '+esc(testRec.test.where)+' · '+esc(testRec.test.updates)+'</span>'+
        '</div></div>';
    }
    ...
    if(prog&&prog.ok&&prog.comparison&&(prog.repetitions||[]).length>0){
      ...
      progCard='<div class="card" style="border-left:3px solid '+vCol+';margin-bottom:12px">'+
        ...
        '<div style="font-size:12px;line-height:1.6">'+esc(prog.explanation_text)+'</div>'+
    ...
    const waCard=(wa&&wa.ok&&wa.structured!=null&&(wa.summary||{}).total_blocks>1)?`
      <div class="card" style="border-left:3px solid #22d3ee;margin-bottom:12px">
        ...
        <div style="font-size:12px;line-height:1.6">${esc(wa.explanation_text)}</div>
    ...
    const limitanteHero=limCapCoach?`
      <div style="background:linear-gradient(135deg,rgba(167,139,250,.18),rgba(167,139,250,.06));border:1px solid rgba(167,139,250,.35);border-radius:18px;padding:18px;margin-bottom:12px">
        ...
        ${ac.goal?`<div style="font-size:11px;color:var(--muted);margin-top:6px">🎯 ${esc(ac.goal.event_name)}${ac.weeks_to_event!=null?' · '+ac.weeks_to_event+' weeks':''}</div>`:''}
      </div>`:'';
    ...
    const spLabels={
      ...
      returning_from_pause:'Returning from a pause',
      new_or_low_data:'Building the baseline',
      training_without_structured_plan:'Training without plan context',
      active_but_needs_calibration:'Active, needs calibration',
      active_but_fatigued:'Active, but recovery leads',
      event_pressure:'Event pressure',
      active_training:'Active training'
    };
    const startingPointCard=(sp&&sp.ok)?`
      <div class="card" style="border-left:3px solid #22d3ee;margin-bottom:12px">
        <div class="head"><h3 style="color:#22d3ee">Starting point</h3><span>${esc(spLabels[sp.state]||sp.state||'assessment')}</span></div>
        <div style="font-size:12px;line-height:1.6;margin-bottom:8px">${esc(sp.first_focus||sp.explanation_text||'')}</div>
        <div style="display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px;margin-bottom:8px">
          ${metric('28d hours',((sp.training_context||{}).hours_28d!=null?(sp.training_context||{}).hours_28d:'—'),'training')}
          ${metric('28d days',((sp.training_context||{}).active_days_28d!=null?(sp.training_context||{}).active_days_28d:'—'),'active')}
          ${metric('Plan',((sp.plan_context||{}).has_active_plan?'yes':'no'),((sp.plan_context||{}).registered_plan_sessions||0)+' sessions')}
        </div>
        ${((sp.next_actions||[]).length?'<div style="font-size:11px;color:var(--muted);line-height:1.5">Next: '+esc((sp.next_actions||[]).slice(0,2).join(' · '))+'</div>':'')}
      </div>`:'';
    const cRec=(calib&&calib.ok)?calib:(sp&&sp.testing_calibration?sp.testing_calibration:null);
    const calibrationCard=(cRec&&cRec.recommended&&cRec.test)?`
      <div class="card" style="border-left:3px solid #c8f135;margin-bottom:12px">
        <div class="head"><h3 style="color:#c8f135">Testing & calibration</h3><span>${esc(cRec.test.type||'test')}</span></div>
        <div style="font-size:12px;line-height:1.6;margin-bottom:8px">${esc(cRec.test.name)} — ${esc(cRec.test.protocol)}</div>
        <div style="font-size:11px;color:var(--muted);line-height:1.5">Why: ${esc((cRec.reasons||[]).slice(0,2).join(' · ')||'current phase asks for a clean calibration read')}.</div>
      </div>`:'';
    ...
    const recCard=`<div class="card" style="border-left:3px solid ${recCol};margin-bottom:12px">
      ...
      <div class="head" style="margin-bottom:8px"><h3 style="color:${recCol}">${recTitle}</h3><span style="color:var(--muted)">${esc(plan.fase||'today')}</span></div>
      <div style="font-size:13px;line-height:1.65;margin-top:6px">
        ${_taOverride?esc(ta.explanation_text):
    ...
    el.innerHTML=
      ...
      startingPointCard+
      recCard+
      (pains?'<div class="card" style="border-color:rgba(232,89,60,.3);margin-bottom:12px"><div class="head"><h3 style="color:#e8593c">Active niggles</h3><span>'+pains+'</span></div>'+molestias.map(function(m){return'<div class="row"><div class="r-main"><div class="r-title">'+esc(m.pain_zone||'—')+'</div></div><div class="r-val" style="color:#e8593c">Level '+(m.pain_level||'?')+'/10</div></div>';}).join('')+'</div>':'')+
      wiCard+
      calibrationCard+
      testCard+
      progCard+
