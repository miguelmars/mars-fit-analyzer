...
async function loadSesion(){
  ...
  try{
    const [wa,laps,prog,ssum]=(await Promise.allSettled([
    ...
    const hero='<div style="background:linear-gradient(135deg,rgba(34,211,238,.12),rgba(34,211,238,.03));border:1px solid rgba(34,211,238,.3);border-radius:18px;padding:16px 18px;margin-bottom:12px">'+
      ...
      '<div style="font-size:15px;font-weight:650;line-height:1.5">'+(ses.name||wtLabel)+' · '+fmtDate(ses.date)+'</div>'+
    ...
    if(prog&&prog.ok&&prog.comparison&&(prog.repetitions||[]).length){
      ...
      progCard='<div class="card" style="border-left:3px solid '+pCol+'">'+
        '<div class="head"><h3>Did I improve at this workout?</h3><span>'+prog.repetitions.length+' repetitions</span></div>'+
    ...
    el.innerHTML=hero+bloquesCard+veredictoCard+progCard+streamsCard+
      evd('Full detail',detalle)+
      (wa.planned_target&&wa.planned_target.description?'<div class="card" style="border-left:3px solid #fb923c"><div class="head"><h3>Lo planeado</h3></div><div style="font-size:12px">'+wa.planned_target.description+'</div></div>':'');
