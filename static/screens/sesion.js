...
async function loadSesion(){
  ...
  try{
    ...
    const wtLabel={z2:'Z2 aerobic',tempo:'Sustained tempo',sweet_spot:'Sweet spot',intervals:'Intervals',long_ride:'Long ride',high_intensity:'High intensity',climb:'Long climbs',endurance:'Endurance / Z2',recovery:'Recovery'}[wa.workout_type]||wa.workout_type;
    ...
    const hero='<div style="background:linear-gradient(135deg,rgba(34,211,238,.12),rgba(34,211,238,.03));border:1px solid rgba(34,211,238,.3);border-radius:18px;padding:16px 18px;margin-bottom:12px">'+
      ...
      '<div style="font-size:13px;line-height:1.55;margin-top:6px">'+esc(wa.training_effect_summary||('This was a '+wtLabel.toLowerCase()+' session. It built '+(wa.capacity_built||'').toLowerCase()+'.'))+'</div>'+
    ...
    const pm=wa.plan_match||{};
    const ts=wa.threshold_signal||{};
    const nx=wa.next_action||{};
    const pmCol=pm.status==='matched'||pm.status==='moved'?'#3dd68c':pm.status==='unmatched'?'#f59e0b':'#8e95a3';
    const sigCol=ts.status==='confirms'?'#3dd68c':ts.status==='questions'?'#f59e0b':'#8e95a3';
    const builtCard='<div class="card" style="border-left:3px solid #22d3ee">'+
      '<div class="head"><h3>What it built</h3><span>'+esc(wtLabel)+'</span></div>'+
      '<div style="font-size:13px;line-height:1.6;margin-bottom:8px"><b>'+esc(wa.capacity_built||'Training adaptation')+'</b></div>'+
      '<div style="font-size:12px;line-height:1.55;color:var(--muted);margin-bottom:8px">'+esc(wa.training_effect_summary||'')+'</div>'+
      (nx.message?'<div style="background:rgba(34,211,238,.07);border:1px solid rgba(34,211,238,.18);border-radius:10px;padding:9px 11px;font-size:12px;line-height:1.5"><b style="color:#22d3ee">'+esc(nx.label||'Next action')+':</b> '+esc(nx.message)+'</div>':'')+
    '</div>';
    const planMatchCard=(pm.status&&pm.status!=='no_plan')?'<div class="card" style="border-left:3px solid '+pmCol+'">'+
      '<div class="head"><h3>Plan match</h3><span style="color:'+pmCol+'">'+esc(pm.status)+'</span></div>'+
      '<div style="font-size:12px;line-height:1.55">'+esc(pm.message||'')+'</div>'+
      ((pm.description||pm.session_type)?'<div style="font-size:11px;color:var(--muted);margin-top:7px">'+esc(pm.session_type||'planned')+(pm.planned_date?' · '+esc(pm.planned_date):'')+'<br>'+esc(pm.description||'')+'</div>':'')+
      ((pm.nearby_candidate&&pm.nearby_candidate.description)?'<div style="font-size:11px;color:var(--muted);margin-top:7px">Nearest: '+esc(pm.nearby_candidate.planned_date||'')+' · '+esc(pm.nearby_candidate.description)+'</div>':'')+
    '</div>':'';
    const thresholdCard=(ts.status&&ts.status!=='insufficient_data' || ts.active_threshold_bpm)?'<div class="card" style="border-left:3px solid '+sigCol+'">'+
      '<div class="head"><h3>Zones / threshold signal</h3><span style="color:'+sigCol+'">'+esc(ts.status||'insufficient')+'</span></div>'+
      '<div style="font-size:12px;line-height:1.55">'+esc(ts.message||'No strong threshold signal.')+'</div>'+
      (ts.active_threshold_bpm?'<div style="font-size:11px;color:var(--muted);margin-top:7px">Active threshold: '+ts.active_threshold_bpm+' bpm'+(ts.near_threshold_s!=null?' · near/above '+Math.round(((ts.near_threshold_s||0)+(ts.above_threshold_s||0))/60)+' min':'')+'</div>':'')+
    '</div>':'';
    ...
    el.innerHTML=hero+builtCard+planMatchCard+thresholdCard+bloquesCard+veredictoCard+demandCard+surveyCard+progCard+streamsCard+
      evd('Full detail',detalle)+
      (wa.planned_target&&wa.planned_target.description?'<div class="card" style="border-left:3px solid #fb923c"><div class="head"><h3>Planned</h3></div><div style="font-size:12px">'+esc(wa.planned_target.description)+'</div></div>':'');
