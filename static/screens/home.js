async function loadHome(){
  try{
    ...
    const d=_hv(0,{}),w=_hv(1,{}),mp=_hv(2,{}),wh=_hv(3,{}),wr=_hv(4,[]),bf=_hv(5,{}),lastAct=_hv(6,{}),ac=_hv(7,{}),rw=_hv(8,{}),tc=_hv(9,{}),ta=_hv(10,{});
    ...
    const _planBadge=_planW?'<div style="margin-top:8px"><span class="chip ok">Plan · semana '+_planW.week_number+'/'+_planW.total_weeks+'</span>'+(_planPhase?'<span class="chip">fase '+_planPhase+'</span>':'')+'</div>':'';
    const _todaySes=((tc&&tc.today)||[])[0];
    const _todayLine=_todaySes&&_todaySes.status==='planned'?
      '<div style="font-size:11px;color:var(--muted);margin-top:6px">Today per the plan: <b style="color:var(--text)">'+(_todaySes.description||_todaySes.session_type)+'</b></div>':'';
    const _goalLine=(ac&&ac.goal&&ac.weeks_to_event!=null)?
      '<div onclick="go(\'plan\')" style="font-size:11px;color:#fb923c;font-weight:800;margin-top:8px;cursor:pointer">⏱ '+ac.weeks_to_event+' week'+(ac.weeks_to_event!==1?'s':'')+' para '+ac.goal.event_name+' →</div>':'';
    ...
    if(ta&&ta.ok&&ta.status&&ta.status!=='sin_lectura'){
      ...
      adaptCard='<div class="card" style="border-left:3px solid '+aCol+'">'+
        ...
        '<div class="head" style="margin-bottom:6px"><h3 style="color:'+aCol+'">'+aTit+'</h3>'+(ta.planned?'<span>'+(ta.planned.description||'')+'</span>':'')+'</div>'+
        '<div style="font-size:12px;line-height:1.6">'+ta.explanation_text+'</div>'+
