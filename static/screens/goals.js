  ...
  try{
    ...
    try{
      ...
      if(rr.ok){
        ...
        const gaps=rr.data_gaps&&rr.data_gaps.length?'<div style="font-size:10px;color:var(--muted);margin-top:6px">Missing data: '+rr.data_gaps.join(', ')+'</div>':'';
        readinessCard=`<div class="card" style="border-left:3px solid ${rCol};margin-bottom:12px">
          ...
          ${(rr.components||[]).filter(c=>c.score!=null).map(c=>{
            ...
            return `<div style="display:grid;grid-template-columns:minmax(0,1fr) 36px;gap:6px;align-items:center;padding:4px 0;border-bottom:1px solid var(--line)">
              <div><div style="font-size:11px;margin-bottom:2px">${esc(c.nombre)}</div><div style="height:3px;background:rgba(255,255,255,.07);border-radius:2px"><div style="height:100%;width:${pct(c.score)}%;background:${barCol}"></div></div></div>
    ...
    let rows='';
    if(goals.length===0){
      rows='<div class="loading">No active goals. Add your first goal below.</div>';
    } else {
      goals.forEach(g=>{
        ...
        rows+=`<div class="row">
          ...
          <div class="r-main">
            <div class="r-title">${esc(g.event_name)}</div>
            <div class="r-sub">${fecha}${meta?' · '+meta:''} · ${priLabel[g.priority]||'Goal'}</div>
            ${g.notes?`<div class="r-sub" style="margin-top:3px;font-style:italic">${esc(g.notes)}</div>`:''}
          </div>
