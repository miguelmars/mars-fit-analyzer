async function loadPerfil(){
  ...
  try{
    ...
    const _resumen='<div style="background:linear-gradient(135deg,rgba(61,214,140,.12),rgba(61,214,140,.03));border:1px solid rgba(61,214,140,.3);border-radius:18px;padding:16px 18px;margin-bottom:12px">'+
      ...
      '<div style="font-size:13px;line-height:1.6">You weigh <b>'+peso+' kg</b> (target '+pesoObj+'). You follow <b>'+(plan.nombre||'Garmin Coach')+'</b> in the '+(plan.fase||'Base')+' phase. Your bike Z2 is '+(zc.z2?zc.z2[0]+'–'+zc.z2[1]:'134–150')+' bpm and you ride the '+(bici.nombre||'Rarotonga')+'.</div>'+
    '</div>';
    const _peso=`
      <!-- PESO -->
      <div class="card">
...
`;
    const _equipo=`
      <!-- BICI + GEAR -->
      <div class="card">
        <div class="head"><h3>Current bike</h3><span style="color:#f59e0b">${bici.nombre||'Rarotonga'}</span></div>
...
`;
    const _plan=`
      <!-- PLAN GARMIN -->
      <div class="card">
        <div class="head"><h3>Garmin Plan</h3><span style="color:#4a9eff">${plan.nombre||'Garmin Coach'}</span></div>
...
`;
    const _detalle=`
      ...
      <div class="card">
        ...
        ${rutas.map(r=>'<div class="row"><div class="r-main"><div class="r-title">'+r.nombre+'</div><div class="r-sub">'+r.desc+'</div></div><div class="r-val" style="color:var(--theme)">'+r.km+' km</div></div>').join('')||'<div style="color:var(--muted);font-size:12px">No routes defined</div>'}
      </div>
