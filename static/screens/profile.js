async function loadPerfil(){
  ...
  try{
    ...
    const _resumen='<div style="background:linear-gradient(135deg,rgba(61,214,140,.12),rgba(61,214,140,.03));border:1px solid rgba(61,214,140,.3);border-radius:18px;padding:16px 18px;margin-bottom:12px">'+
      ...
      '<div style="font-size:13px;line-height:1.6">You weigh <b>'+peso+' kg</b> (target '+pesoObj+'). You follow <b>'+esc(plan.nombre||'Garmin Coach')+'</b> in the '+(plan.fase||'Base')+' phase. Your bike Z2 is '+(zc.z2?zc.z2[0]+'–'+zc.z2[1]:'134–150')+' bpm and you ride the '+esc(bici.nombre||'Rarotonga')+'.</div>'+
    '</div>';
    const _peso=`
      <!-- PESO -->
      <div class="card">
