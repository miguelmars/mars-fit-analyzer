  ...
  try{
    ...
    el.innerHTML=stravaPerfilCard+sqCard+`
      ...
      <div class="card">
        ...
        <button class="btn btn2" style="font-size:11px;padding:8px;margin-top:10px" onclick="deactivatePlan()">Dar de baja este plan</button>
        <div style="font-size:10px;color:var(--muted);margin-top:4px">El historial se conserva: sesiones, matches y cumplimiento quedan registrados.</div>
      </div>
...
async function deactivatePlan(){
  ...
  const pick=prompt('¿Por qué das de baja el plan?\n1 = Lesión\n2 = Nuevo objetivo\n3 = Ya no quiero seguirlo\n4 = Otra razón\n\nEscribe el número (o cancela):');
  if(!pick)return;
  const reason=reasons[pick.trim()]||'otro';
  if(!confirm('El plan pasará a histórico (no se borra nada: sesiones, matches y cumplimiento se conservan). ¿Confirmar?'))return;
  try{
    ...
    if(!pid){toast('No hay plan estructurado activo');return;}
    ...
    toast(d.ok?'Plan dado de baja — historial conservado':'Error: '+(d.message||''));
    loadPerfil();
