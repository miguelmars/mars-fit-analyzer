const THEME={home:'#c8cbd2',plan:'#fb923c',sesion:'#22d3ee',perfil:'#3dd68c',coach:'#4a9eff',progress:'#a78bfa',capacidades:'#22d3ee',dashboard:'#e8593c',activities:'#e8593c',gear:'#f59e0b',calendar:'#22d3ee',performance:'#a78bfa',fuerza:'#c8f135',wellness:'#4a9eff',eficiencia:'#3dd68c',correlaciones:'#a78bfa',nutricion:'#f59e0b',metas:'#fb923c'};
const TITLE={home:['Epoch','Home'],plan:['My Week','what I am building'],sesion:['Session','full read'],perfil:['Profile','system'],coach:['Coach','guidance'],progress:['Evolution','athletic'],capacidades:['Capacities','athletic'],dashboard:['Dashboard','stats'],activities:['Data','sessions'],gear:['Gear','maintenance'],calendar:['Calendar','heatmap'],performance:['Records','personal'],fuerza:['Strength','Compex'],wellness:['Wellness','recovery'],eficiencia:['Efficiency','aerobic'],correlaciones:['Correlations','HR · Weight'],nutricion:['Nutrition','fueling'],metas:['Goals','active']};
let current='home';
let navStack=[];
...
window.openSesion=function(id){window._epochSesionId=id;go('sesion');};
...
const _SCREEN_EL={home:'home-data',plan:'plan-data',sesion:'sesion-data',dashboard:'dash-data',activities:'act-list',gear:'gear-data',calendar:'cal-data',performance:'perf-data',fuerza:'fuerza-data',wellness:'well-data',eficiencia:'eff-data',correlaciones:'corr-data',nutricion:'nutri-summary',perfil:'perfil-data',coach:'coach-data',capacidades:'cap-data',metas:'metas-data',progress:'progress-data'};
function _watchdog(s){
  const id=_SCREEN_EL[s];if(!id)return;
  setTimeout(()=>{
    const el=$(id);
    if(el&&el.querySelector&&el.querySelector('.spin')){
      el.innerHTML='<div class="card" style="text-align:center;color:var(--muted);font-size:13px">Could not load this screen.<br><button class="btn btn2" style="margin-top:10px;width:auto;padding:10px 20px" onclick="load(\''+s+'\')">Retry</button></div>';
    }
  },15000);
}
async function load(s){
  _watchdog(s);
  try{
    if(s==='home')return await loadHome();if(s==='plan')return await loadPlan();if(s==='sesion')return await loadSesion();if(s==='dashboard')return await loadDash();if(s==='activities')return loadActs();if(s==='gear')return await loadGear();if(s==='calendar')return await loadCal();if(s==='performance')return await loadPerf();if(s==='fuerza')return await loadFuerza();if(s==='wellness')return await loadWell();if(s==='eficiencia')return await loadEficiencia();if(s==='correlaciones')return await loadCorrelaciones();if(s==='nutricion')return await loadNutricion();if(s==='perfil'){loadPerfil();loadStravaStatus();return;}if(s==='coach')return await loadCoach();if(s==='capacidades')return await loadCapacidades();if(s==='metas')return await loadMetas();if(s==='progress')return await loadProgress(progressMode);
  }catch(e){
    const id=_SCREEN_EL[s];const el=id&&$(id);
    if(el)el.innerHTML='<div class="card" style="color:var(--muted)">Error: '+esc(e.message||e)+'</div>';
  }
}
...
async function loadDash(){
  try{
    ...
    $('dash-data').innerHTML=`<div class="grid2">${metric('Fitness',a.fitness||'—','Epoch Index '+(a.mars_index||'—'))}${metric('Fatigue',a.fatiga||'—','freshness '+(c.tsb||0))}${metric('Load',c.estado||'—','current')}${metric('Z2',Number(z.pct_z2_4_semanas||0).toFixed(1)+'%','4 weeks')}</div><div class="card"><div class="head"><h3>This week</h3><span>${s.sesiones||0} sessions</span></div>${row(icoKm,'Weekly distance','Garmin total',Number(s.km||0).toFixed(1)+' km')}${row(icoTime,'Weekly time','Load hours',Number(s.horas||0).toFixed(1)+' h')}${row(icoCal,'Calories','Garmin export',calories)}</div><div class="card"><div class="head"><h3>Historic calories</h3><span>${ca.validacion==='ok'?'OK':'review'}</span></div>${row(icoCal,'All-time total','clean_sessions layer',Number(ca.calorias_historicas||0).toLocaleString('es-MX')+' kcal')}${row('', 'Avg kcal/km', ca.nota||'', (ca.kcal_por_km_promedio||'—'))}</div>`;
  }catch(e){
    $('dash-data').innerHTML='<div class="card">Error: '+esc(e.message)+'</div>';
  }
}
...
async function loadStravaActs(reset=true){
  ...
  $('act-list').innerHTML='<div class="loading"><span class="spin"></span>Loading...</div>';
  try{
    ...
    if(!arr.length){$('act-list').innerHTML=filterBar+'<div style="color:var(--muted);padding:20px;text-align:center">No activities</div>';return;}
    const rows=arr.map(s=>{
      ...
      return `<div class="row" style="align-items:flex-start;cursor:pointer" onclick="showStravaDetail(${s.strava_id})">
        ...
        <div class="r-main" style="flex:1;min-width:0">
          <div class="r-title" style="white-space:normal">${esc(s.name||s.sport_type)}</div>
    ...
    const more=rem>0?`<button onclick="_stravaOffset+=${arr.length};loadStravaActs(false)" class="btn btn2" style="margin-top:8px">Load more (${rem.toLocaleString()} restantes)</button>`:'';
    $('act-list').innerHTML=filterBar+rows+more;
  }catch(e){$('act-list').innerHTML='<div style="color:var(--muted)">'+esc(e.message)+'</div>';}
}
...
async function showStravaDetail(stravaId){
  ...
  try{
    ...
    document.getElementById('strava-detail-body').innerHTML=
      ...
      (d.hr&&d.hr.length?'<div style="margin-bottom:14px"><div style="font-size:11px;font-weight:800;color:#4a9eff;margin-bottom:5px">❤ Heart Rate</div>'+hrSparkline(d.hr,Math.min(360,window.innerWidth-48),50)+'</div>':'')+
  ...
  }catch(err){document.getElementById('strava-detail-body').innerHTML='<div style="color:var(--muted)">'+esc(err.message)+'</div>';}
}
...
async function uploadFit(file){
  if(!file)return;
  $('upload-result').innerHTML='';$('upload-result').innerHTML='<div class="card">Processing '+esc(file.name)+'...</div>';
  ...
  try{
    ...
    let html='<div class="card"><div class="head"><h3>'+(d.duplicate?'Sesion existente':'Sesion guardada')+'</h3><span>'+d.session_id+'</span></div>'+
      ...
      '</div>'+
      '<button class="btn" style="margin-top:8px" onclick="window.location.href=\x27/charts/'+d.session_id+'\x27">View charts</button>'+
    ...
    try{
      ...
      if(mr.matched&&mr.matched.length>0){
        ...
        mrHtml+='<div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:11px">'+
          '<tr style="color:var(--muted)"><td style="padding:4px">Date</td><td style="padding:4px;text-align:right">km/h</td><td style="padding:4px;text-align:right">FC</td><td style="padding:4px;text-align:right">Efic.</td></tr>'+
    ...
    loadActs();
  }catch(e){$('upload-result').innerHTML='<div class="card">Error: '+esc(e.message)+'</div>'}
}
...
async function loadGear(){
  ...
  try{
    ...
    function gearRow(g, retiredFlag){
      ...
      return `<div class="row"><div class="r-ico">${retiredFlag?'R':'A'}</div><div class="r-main"><div class="r-title">${esc(g.name||g.model||g.type||'Component')}</div><div class="r-sub">${sub}${pct!=null?`<div class="gearbar"><div class="gearfill" style="width:${pct}%"></div></div>`:''}</div></div><div class="r-val">${pct!=null?pct+'%':(retiredFlag?'ret.':'cat.')}</div></div>`;
    }
    let html=`<div class="card"><div class="head"><h3>Alerts</h3><span>${(a.alerts||[]).length}</span></div>${(a.alerts||[]).length?(a.alerts||[]).map(x=>row('',esc(x.name||x.type||'Alert'),esc(x.message||x.detail||''),x.km_left?x.km_left+' km':'' )).join(''):'No gear alerts'}</div>`;
    html+=`<div class="card"><div class="head"><h3>Active components</h3><span>${items.length||0}</span></div>${items.length?items.map(g=>gearRow(g,false)).join(''):'No active components registered'}</div>`;
    html+=`<div class="card"><div class="head"><h3>Active Garmin catalog</h3><span>${active.length||0}</span></div><div class="r-sub" style="margin-bottom:8px">${note}</div>${active.length?active.map(g=>gearRow(g,false)).join(''):'No active parts in Garmin export'}</div>`;
    html+=`<div class="card"><div class="head"><h3>Garmin retired</h3><span>${retired.length||0}</span></div>${retired.length?retired.map(g=>gearRow(g,true)).join(''):'No retired parts in Garmin export'}</div>`;
    $('gear-data').innerHTML=html;
  }catch(e){$('gear-data').innerHTML='<div class="card">Error: '+esc(e.message)+'</div>'}
}
  ...
  try{
    ...
    $('cal-data').innerHTML=
      '<div class="grid2" style="margin-bottom:12px">'+
        '<div class="mini"><div class="label">Active days</div><div class="value" style="color:#22d3ee">'+active+'</div><div class="unit">12 weeks</div></div>'+
      ...
      '<div class="card">'+
        ...
        '</div>'+
      '</div>';
  }catch(e){$('cal-data').innerHTML='<div class="card" style="color:var(--muted)">'+esc(e.message)+'</div>'}
}
...
async function loadPerf(){
  try{
    ...
    $('perf-data').innerHTML='<div class="grid2">'+
      ...
      metric('Frescura',c.tsb||0,'balance carga/descanso')+
  ...
  }catch(e){$('perf-data').innerHTML='<div class="card" style="color:var(--muted)">'+esc(e.message)+'</div>'}
}
...
async function loadFuerza(){
  try{
    ...
    $('fuerza-data').innerHTML=
      '<div class="grid2">'+metric('Sessions 8w',d.total_sesiones||0,'')+metric('Total hours',d.total_horas||0,'')+'</div>'+
      (items.length?'<div class="card"><div class="head"><h3>Latest sessions</h3></div>'+
...
async function loadWell(){
  try{
    ...
    let streakDots='';
    const catLabel={sleep:'Sleep',nap:'Nap',compex_recovery:'Compex',massage_gun:'Massage gun',ceragem:'Ceragem',pain:'Pain',stress:'Stress'};
    for(let i=27;i>=0;i--){
    ...
    const streakCard=`<div class="card" style="margin-bottom:8px">
      <div class="head"><h3>Recovery · 28 days</h3><span style="color:${e25cActive?'#22c55e':'#4a9eff'}">${checkCount}/28 checks</span></div>
      ...
      <div style="font-size:11px;color:var(--muted);margin-bottom:8px">
        ...
        <span style="color:#2a2d36;border:1px solid #444;border-radius:50%;display:inline-block;width:8px;height:8px"></span> No data
      </div>
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px">
        <div style="font-size:12px;font-weight:800;color:#4a9eff">🔥 Streak: ${streak} day${streak!==1?'s':''}</div>
        <div style="font-size:11px;color:${e25cActive?'#22c55e':'var(--muted)'}">E25C: ${e25cActive?'✅ active':''+checkCount+'/21 days'}</div>
      </div>
      ...
      ${!e25cActive?'<div style="font-size:10px;color:var(--muted);margin-top:5px">'+(21-checkCount)+' checks to go to unlock the Recovery Score (E25C)</div>':''}
    </div>`;

    $('well-data').innerHTML=
      '<div class="grid2">'+
        metric('Avg sleep',sleep,'h/night')+
        metric('Resting HR',fc,'bpm')+
        metric('Avg stress',stress,'/10')+
        metric('Niggles',pains.length,pains.length?'active':'no alerts')+
      '</div>'+
      (function(){
        // U6 (UI v2) — Body Map: el cuerpo como mapa, no como lista
        if(!bm||!bm.ok)return '';
        const Z=bm.zones||{};
        const zName={hombro:'Shoulders',espalda_alta:'Upper back / neck',espalda_baja:'Lower back',cadera:'Hips / glutes',cuadriceps:'Quads',rodilla:'Knees',pie_tobillo:'Feet / ankles',gemelos:'Calves',otro:'Other',general:'General'};
        const tName={compex_recovery:'Compex',massage_gun:'Massage gun',ceragem:'Ceragem',massage:'Massage',mobility:'Mobility'};
        const zc=function(k){const z=Z[k];if(!z)return '#23262d';if(z.pain)return '#e8593c';if((z.therapies||[]).length)return '#3dd68c';return '#23262d'};
        const zs=function(k){return 'fill:'+zc(k)+';stroke:rgba(255,255,255,.18);stroke-width:1;cursor:pointer'};
        const N='fill:#15171c;stroke:rgba(255,255,255,.1);stroke-width:1';
        const front='<svg viewBox="0 0 100 215" style="width:100%;max-width:120px">'+
          '<circle cx="50" cy="14" r="10" style="'+N+'"/>'+
          '<rect x="35" y="27" width="30" height="52" rx="9" style="'+N+'"/>'+
          '<circle cx="33" cy="32" r="7" onclick="bmSel(\'hombro\')" style="'+zs('hombro')+'"/>'+
          '<circle cx="67" cy="32" r="7" onclick="bmSel(\'hombro\')" style="'+zs('hombro')+'"/>'+
          '<rect x="22" y="38" width="7" height="38" rx="3.5" style="'+N+'"/>'+
          '<rect x="71" y="38" width="7" height="38" rx="3.5" style="'+N+'"/>'+
          '<rect x="35" y="81" width="30" height="13" rx="6" onclick="bmSel(\'cadera\')" style="'+zs('cadera')+'"/>'+
          '<rect x="35" y="96" width="13" height="40" rx="6" onclick="bmSel(\'cuadriceps\')" style="'+zs('cuadriceps')+'"/>'+
          '<rect x="52" y="96" width="13" height="40" rx="6" onclick="bmSel(\'cuadriceps\')" style="'+zs('cuadriceps')+'"/>'+
          '<circle cx="41.5" cy="142" r="5.5" onclick="bmSel(\'rodilla\')" style="'+zs('rodilla')+'"/>'+
          '<circle cx="58.5" cy="142" r="5.5" onclick="bmSel(\'rodilla\')" style="'+zs('rodilla')+'"/>'+
          '<rect x="37" y="149" width="9" height="36" rx="4" style="'+N+'"/>'+
          '<rect x="54" y="149" width="9" height="36" rx="4" style="'+N+'"/>'+
          '<rect x="35" y="188" width="13" height="8" rx="3" onclick="bmSel(\'pie_tobillo\')" style="'+zs('pie_tobillo')+'"/>'+
          '<rect x="52" y="188" width="13" height="8" rx="3" onclick="bmSel(\'pie_tobillo\')" style="'+zs('pie_tobillo')+'"/>'+
          '<text x="50" y="210" text-anchor="middle" style="fill:#8e95a3;font-size:8px;font-weight:800">FRONT</text></svg>';
        const back='<svg viewBox="0 0 100 215" style="width:100%;max-width:120px">'+
          '<circle cx="50" cy="14" r="10" style="'+N+'"/>'+
          '<rect x="35" y="27" width="30" height="24" rx="8" onclick="bmSel(\'espalda_alta\')" style="'+zs('espalda_alta')+'"/>'+
          '<rect x="35" y="53" width="30" height="15" onclick="bmSel(\'espalda_baja\')" style="'+zs('espalda_baja')+'"/>'+
          '<rect x="22" y="38" width="7" height="38" rx="3.5" style="'+N+'"/>'+
          '<rect x="71" y="38" width="7" height="38" rx="3.5" style="'+N+'"/>'+
          '<rect x="35" y="70" width="30" height="16" rx="7" onclick="bmSel(\'cadera\')" style="'+zs('cadera')+'"/>'+
          '<rect x="35" y="88" width="13" height="42" rx="6" style="'+N+'"/>'+
          '<rect x="52" y="88" width="13" height="42" rx="6" style="'+N+'"/>'+
          '<rect x="37" y="140" width="9" height="40" rx="4" onclick="bmSel(\'gemelos\')" style="'+zs('gemelos')+'"/>'+
          '<rect x="54" y="140" width="9" height="40" rx="4" onclick="bmSel(\'gemelos\')" style="'+zs('gemelos')+'"/>'+
          '<rect x="35" y="188" width="13" height="8" rx="3" onclick="bmSel(\'pie_tobillo\')" style="'+zs('pie_tobillo')+'"/>'+
          '<rect x="52" y="188" width="13" height="8" rx="3" onclick="bmSel(\'pie_tobillo\')" style="'+zs('pie_tobillo')+'"/>'+
          '<text x="50" y="210" text-anchor="middle" style="fill:#8e95a3;font-size:8px;font-weight:800">BACK</text></svg>';
        const detail=Object.keys(Z).map(function(k){
          const z=Z[k];const bits=[];
          if(z.pain)bits.push('<b style="color:#e8593c">niggle '+(z.pain.level||'?')+'/10</b>'+(z.pain.since?' since '+fmtDate(z.pain.since):''));
          (z.therapies||[]).slice(0,2).forEach(function(t){bits.push('<span style="color:#3dd68c">'+(tName[t.kind]||t.kind)+'</span> '+fmtDate(t.date))});
          return bits.length?'<div class="row"><div class="r-main"><div class="r-title">'+(zName[k]||k)+'</div><div class="r-sub">'+bits.join(' · ')+'</div></div></div>':'';
        }).join('');
        return '<div class="card"><div class="head"><h3>Body map</h3><span>'+(bm.active_pains||0)+' niggle(s) · '+(bm.therapies_7d||0)+' therapies 7d</span></div>'+
          '<div style="display:flex;gap:18px;justify-content:center;margin:4px 0 8px">'+front+back+'</div>'+
          '<div style="display:flex;gap:12px;justify-content:center;font-size:9px;color:var(--muted);margin-bottom:6px"><span><span style="color:#e8593c">●</span> active niggle</span><span><span style="color:#3dd68c">●</span> maintained (7d)</span><span>tap a zone to log</span></div>'+
          detail+
        '</div>';
      })()+
      streakCard+
      (pains.length?'<div class="card" style="border-color:rgba(232,89,60,.3)"><div class="head"><h3 style="color:#e8593c">Active niggles</h3><span>'+pains.length+'</span></div>'+
        pains.map(p=>'<div class="row"><div class="r-main"><div class="r-title">'+esc(p.pain_zone||'—')+'</div></div><div class="r-val" style="color:#e8593c">Level '+(p.pain_level||'?')+'/10</div></div>').join('')+'</div>':'')+
      (items.length?'<div class="card"><div class="head"><h3>Entries</h3></div>'+
        ...
        '</div>').join('')+'</div>':'');
  }catch(e){$('well-data').innerHTML='<div class="card" style="color:var(--muted)">'+esc(e.message)+'</div>'}
}
...
async function saveMorningCheck(){
  ...
  if(!hr&&!fatigue&&!sleep&&!estado){toast('Fill in at least one field');return;}
  ...
  if(d.ok){
    ...
    toast('✅ Morning check saved — recovery active');
    loadWell();
...
async function loadEficiencia(){
  ...
  try{
    ...
    el.innerHTML=`
      ...
      <div class="card">
        <div class="head"><h3>Speed/HR efficiency — ${trend.length} weeks</h3><span style="color:#3dd68c">${pct}%</span></div>
      ...
      <div class="card">
        <div class="head"><h3>Load: acute · chronic · freshness</h3><span style="color:${tsbCol}">${carga.estado||'—'}</span></div>
        ...
        ${ctlV.length>=2?sparkSVG(ctlV,'#4a9eff',50):'<div style="color:var(--muted);font-size:12px">No load history</div>'}
      </div>`;
...
async function loadCorrelaciones(){
  ...
  try{
    ...
    function scatterSVG(xs,ys,col){
      if(!xs||xs.length<3)return '<div style="color:var(--muted);font-size:12px;padding:8px">More weeks of data needed</div>';
    ...
    el.innerHTML=`
      <div class="card">
        ...
        <div style="font-size:12px;color:var(--muted);margin-bottom:10px">${esc(d.interpretation||'')}</div>
        ${scatterSVG(hrs,effs,'#3dd68c')}
        <div style="font-size:10px;color:var(--muted);margin-top:4px">Each point = 1 week · X axis: avg HR · Y axis: speed/HR</div>
      </div>
      ${wkgs.length>=3?`<div class="card"><div class="head"><h3>Weight vs avg HR</h3></div>${scatterSVG(wkgs,hrs,'#4a9eff')}<div style="font-size:10px;color:var(--muted);margin-top:4px">Eje X: peso kg · Eje Y: FC prom bpm</div></div>`:'<div class="card"><div style="color:var(--muted);font-size:13px;padding:8px">Agrega mas registros de peso para la correlacion</div></div>'}
      <div class="grid2">
...
async function loadNutricion(){
  ...
  try{
    ...
    const GL={agave_casero:'Homemade agave',miel_casero:'Homemade honey',comercial:'Commercial',fecha:'Dates/fruit',ninguno:'No gel'};
    el.innerHTML=tipos.length?`<div class="card"><div class="head"><h3>Gels 8 weeks</h3><span>${tipos.reduce((a,t)=>a+(t.usos||0),0)} entries</span></div>${tipos.map(t=>row('·',GL[t.gel_type]||t.gel_type,t.usos+' usos'+(t.gi_issues>0?' · '+t.gi_issues+' GI':''),t.avg_carbos?t.avg_carbos.toFixed(0)+'g':'')  ).join('')}</div>`
    :'<div class="card" style="margin-bottom:12px"><div style="color:var(--muted);text-align:center;padding:12px">No entries yet</div></div>';
...
(function(){
  ...
  const g=h<12?'Good morning,':h<20?'Good afternoon,':'Good evening,';
...
async function loadCapacidades(){
  ...
  try{
    const _capSettled=await Promise.allSettled([
      ...
      fetch(API+'/gpt/capability-evidence?weeks=4').then(r=>r.ok?r.json():{}),
      fetch(API+'/gpt/capability-levels').then(r=>r.ok?r.json():{})
    ]);
    if(_capSettled[0].status==='rejected')throw new Error('Could not compute capacities');
    const d=_capSettled[0].value;
    const _lvls=_capSettled[10]&&_capSettled[10].status==='fulfilled'?(_capSettled[10].value.levels||[]):[];
    const _lvlByKey={};_lvls.forEach(function(L){_lvlByKey[L.key]=L});
    ...
    const capEvidence=_capSettled[9]&&_capSettled[9].status==='fulfilled'?_capSettled[9].value:{};
    ...
    function readinessStatusLabel(status){
      return {listo:'Ready',bien_encaminado:'On track',forma_en_desarrollo:'Developing',base_en_construccion:'Building base',inicio:'Starting out'}[status]||status;
    }
      ...
      const cards=readiness.filter(function(r){return r.ok}).map(function(r){
        ...
        const comps=(r.components||[]).map(function(c){
          ...
          return '<div style="display:grid;grid-template-columns:minmax(0,1fr) 32px;gap:6px;align-items:center;padding:5px 0;border-bottom:1px solid var(--line)">'+
            '<div><div style="font-size:11px;margin-bottom:3px">'+esc(c.nombre)+gap+'</div>'+
        ...
        return '<div class="card" style="border-left:3px solid '+color+'">'+
          ...
          comps+
          gaps+
          (r.limiting_factor_nombre&&r.score!==null?'<div style="font-size:10px;color:var(--muted);margin-top:6px">Main limiter: '+r.limiting_factor_nombre+'</div>':'')+
        '</div>';
      }).join('');
      if(!cards)return '';
      return '<div class="head" style="margin:16px 0 4px"><h3>Readiness Scenarios</h3><span>readiness per scenario · not active goals</span></div>'+
    ...
    function historyView(){
      ...
      const rows=years.map(function(y){
        ...
        const sub=preliminary?'preliminary · maturity '+Number(y.maturity||0).toFixed(0)+'%':sport+' · '+(y.active_weeks||0)+' weeks';
      ...
      return '<div class="card" style="border-left:3px solid #3dd68c">'+
        '<div class="head"><h3>Aerobic Engine History</h3><span>12-week blocks</span></div>'+
        '<div class="grid2">'+
          metric('Today (official)',official.score!=null?Number(official.score).toFixed(1):'--','validated')+
          metric('Best era',best.score!=null?Number(best.score).toFixed(1):'--',best.year||'no reference')+
        '</div>'+
        '<div style="font-size:12px;line-height:1.5;margin:2px 0 10px"><strong>'+bestPeriod+'</strong><br><span style="color:var(--muted)">'+(best.sport_context==='running'?'Running':'Cycling')+' · '+(best.limitante_principal||'')+'</span></div>'+
        rows+
        '<div style="font-size:10px;line-height:1.45;color:var(--muted);padding-top:9px">Faded years stay visible, but do not compete for \u201cbest era\u201d until they reach 60% maturity and 70% confidence.</div>'+
      '</div>';
    }
    function bodyCompositionHistoryView(){
      ...
      const noDataNote=skipped>0?'<div style="font-size:10px;color:var(--muted);padding:7px 0;border-bottom:1px solid var(--line)">'+skipped+' year(s) without weight data — not an error: there was simply no scale connected. ~12 weeks of weight are needed to compare eras.</div>':'';
      const fewDataNote=(measuredWeeks>0&&measuredWeeks<12)?'<div style="font-size:10px;color:#f59e0b;padding:7px 0">Only '+measuredWeeks+' week'+(measuredWeeks!==1?'s':'')+' of weight logged; ~12 needed to compare eras with confidence.</div>':'';
      const rows=years.map(function(y){
        const preliminary=y.best_period_tag==='historia_preliminar';
        const bestYear=y.best_period_tag==='mejor_epoca_historica';
        ...
        const sub=preliminary?'preliminary · maturity '+Number(y.maturity||0).toFixed(0)+'%':'min '+minW+' · '+(y.measured_weeks||0)+' weeks';
      ...
      return '<div class="card" style="border-left:3px solid #f59e0b">'+
        '<div class="head"><h3>Body Composition History</h3><span>12-week blocks</span></div>'+
        '<div class="grid2">'+
          metric('Today (official)',official.score!=null?Number(official.score).toFixed(1):'--','validated')+
          metric('Best era',best.score!=null?Number(best.score).toFixed(1):'--',best.year||'no reference')+
          metric('All-time low',personalMin,'lowest personal weight')+
          metric('Semanas medidas',ref.total_weeks_measured||0,'registros de peso')+
        '</div>'+
        '<div style="font-size:12px;line-height:1.5;margin:2px 0 10px"><strong>'+bestPeriod+'</strong><br><span style="color:var(--muted)">'+(best.block_min_weight_kg!=null?'Min weight '+best.block_min_weight_kg+' kg':'')+' '+(best.limitante_principal||'')+'</span></div>'+
        rows+
        '<div style="font-size:10px;line-height:1.45;color:var(--muted);padding-top:9px">Score 100 = the block minimum weight matched your all-time low at that point. Needs ≥40% maturity and ≥60% confidence to compete for best era.</div>'+
      '</div>';
    }
    function climbingHistoryView(){
      ...
      const years=yearsAll.filter(function(y){return y.best_period_tag!=='sin_datos_suficientes'});
      const skipped=yearsAll.length-years.length;
      const noDataNote=skipped>0?'<div style="font-size:10px;color:var(--muted);padding:7px 0;border-bottom:1px solid var(--line)">'+skipped+' year(s) without recorded climbing — omitted, not a data error.</div>':'';
      const rows=years.map(function(y){
        const preliminary=y.best_period_tag==='historia_preliminar';
        const bestYear=y.best_period_tag==='mejor_epoca_historica';
        ...
        const big=y.big_sessions!=null?' · '+y.big_sessions+' sessions >500m':'';
        const sub=preliminary?'preliminary · maturity '+Number(y.maturity||0).toFixed(0)+'%':avgM+big;
        const opacity=preliminary?'.52':'1';
        return '<div style="display:grid;grid-template-columns:38px minmax(0,1fr) 42px;gap:9px;align-items:center;padding:8px 0;border-bottom:1px solid var(--line);opacity:'+opacity+'">'+
          '<div style="font-size:12px;font-weight:900">'+y.year+'</div>'+
          '<div style="min-width:0"><div style="height:7px;background:rgba(255,255,255,.07);overflow:hidden;border-radius:4px"><div style="height:100%;width:'+pct(y.score)+'%;background:'+color+'"></div></div><div style="font-size:10px;color:var(--muted);margin-top:4px;white-space:normal">'+sub+'</div></div>'+
      ...
      return '<div class="card" style="border-left:3px solid #22d3ee">'+
        '<div class="head"><h3>Strength-endurance history</h3><span>effort against resistance · 12-week blocks</span></div>'+
        '<div style="font-size:10px;color:var(--muted);margin-bottom:8px">Measures how much climbing you sustain per week — sustained effort against resistance.</div>'+
        '<div class="grid2">'+
          metric('Today (official)',official.score!=null?Number(official.score).toFixed(1):'--','validated')+
          metric('Best era',best.score!=null?Number(best.score).toFixed(1):'--',best.year||'no reference')+
        ...
        '</div>'+
        '<div style="font-size:12px;line-height:1.5;margin:2px 0 10px"><strong>'+bestPeriod+'</strong><br><span style="color:var(--muted)">'+(best.avg_weekly_ascent_m!=null?best.avg_weekly_ascent_m+' m/wk · ':'')+''+(best.limitante_principal||'')+'</span></div>'+
        rows+
        '<div style="font-size:10px;line-height:1.45;color:var(--muted);padding-top:9px">Score is referenced to your historical weekly p90 at that point. Needs ≥40% maturity and ≥60% confidence to compete for best era.</div>'+
      '</div>';
    }
    function patronHistoricoView(){
      if(!patronData.ok||!(patronData.matches||[]).length)return '';
      const patronColor={'Performance peak':'#3dd68c','Decline in form':'#e8593c','Training gap':'#f59e0b','Steady era':'#22d3ee','Not enough data':'#8b929f'};
      const patronIcon={'Performance peak':'↑','Decline in form':'↓','Training gap':'—','Steady era':'→','Not enough data':'?'};
      const cards=(patronData.matches||[]).map(function(m){
        ...
        return '<div class="card" style="border-left:3px solid '+col+'">'+
          ...
          '<div style="font-size:9px;color:var(--muted);margin-top:4px">— 8 weeks later</div>'+
        '</div>';
      }).join('');
      ...
      return '<div class="head" style="margin:16px 0 4px"><h3>Similar eras in your history</h3><span>what happened next?</span></div>'+
        '<div class="card" style="border-left:3px solid #4a9eff">'+
          '<div style="font-size:12px;line-height:1.55;color:var(--text);margin-bottom:10px">'+esc(patronData.mensaje)+'</div>'+
    ...
    function capCard(c){
      ...
      const score=c.score==null?'No score':Number(c.score).toFixed(1);
      ...
      const environmentHtml=environment.current_altitude_m!=null
        ?'<div style="margin:10px 0;padding:10px;background:rgba(34,211,238,.06);border:1px solid rgba(34,211,238,.14);border-radius:8px">'+
          ...
          '<div style="font-size:12px;line-height:1.55">Actual '+Number(environment.current_altitude_m).toLocaleString('es-MX',{maximumFractionDigits:0})+' m · usual '+Number(environment.habitual_altitude_m||0).toLocaleString('es-MX',{maximumFractionDigits:0})+' m · '+(environment.comparison_method==='similar_altitude_history'?'compared with '+environment.reference_sample+' sessions at similar altitude':'general reference — no comparable sample')+'</div>'+
          '<div style="font-size:10px;color:var(--muted);margin-top:3px">'+environment.prior_21d_exposure_days+' days of comparable exposure in 21 days · confidence '+Math.round(Number(environment.altitude_confidence||0)*100)+'%</div>'+
        '</div>'
        ...
        ?'<div style="margin:10px 0;padding:10px;background:rgba(34,211,238,.06);border:1px solid rgba(34,211,238,.14);border-radius:8px">'+
          ...
          '<div style="font-size:12px">Promedio '+Number(environment.recent_avg_altitude_m).toLocaleString('es-MX',{maximumFractionDigits:0})+' m · max '+Number(environment.recent_max_altitude_m).toLocaleString('es-MX',{maximumFractionDigits:0})+' m · '+environment.high_altitude_sessions_8w+' high-altitude sessions</div>'+
        '</div>':'';
      const indicators=(c.indicators||[]).map(function(i){
        ...
        const reference=i.reference==null?'no reference':'ref. '+Number(i.reference).toLocaleString('es-MX',{maximumFractionDigits:3})+(i.unit?' '+i.unit:'');
        return '<div class="row"><div class="r-main"><div class="r-title">'+i.label+'</div><div class="r-sub">'+reference+' · confidence '+Math.round(Number(i.confidence||0)*100)+'%</div></div><div class="r-val" style="color:'+color+'">'+value+'</div></div>';
      }).join('');
      const confPct=Math.round(Number(c.confidence||0)*100);
      const confBadge=confPct>=75
        ?'<span style="background:rgba(61,214,140,.15);color:#3dd68c;padding:2px 7px;border-radius:6px;font-size:9px;font-weight:900;margin-left:6px">High '+confPct+'%</span>'
        :confPct>=50
        ?'<span style="background:rgba(245,158,11,.15);color:#f59e0b;padding:2px 7px;border-radius:6px;font-size:9px;font-weight:900;margin-left:6px">Medium '+confPct+'%</span>'
        :'<span style="background:rgba(232,89,60,.12);color:#e8593c;padding:2px 7px;border-radius:6px;font-size:9px;font-weight:900;margin-left:6px">Low '+confPct+'%</span>';
      const _L=_lvlByKey[c.key]||{};
      const lvlChip=_L.level?'<span style="background:rgba(167,139,250,.15);color:#a78bfa;padding:2px 8px;border-radius:6px;font-size:10px;font-weight:950;margin-left:6px">Lv '+_L.level+'</span>':'';
      const nextStep=_L.next_step?'<div style="font-size:11px;line-height:1.5;margin-top:8px;padding:8px 10px;background:rgba(167,139,250,.07);border-left:2px solid #a78bfa;border-radius:6px"><b style="color:#a78bfa">'+(_L.level&&_L.level<10?'Next level ('+(_L.points_to_next||0)+' pts away):':'Hold the level:')+'</b> '+_L.next_step+'</div>':'';
      return '<div class="card" style="border-left:3px solid '+color+'">'+
        '<div class="head"><h3>'+esc(c.nombre)+lvlChip+confBadge+'</h3><span style="color:'+color+'">'+(c.status==='calculated'?'calculated':'to calibrate')+'</span></div>'+
        '<div class="grid2">'+
          metric('Capacity',score,scoreUnit)+
          metric('Confidence',Math.round(Number(c.confidence||0)*100)+'%','data quality')+
          metric('Maturity',Number(c.maturity||0).toFixed(0)+'%','model calibration')+
          metric('Similar anchor',similar.similarity!=null?Math.round(Number(similar.similarity)*100)+'%':'--',similar.week?fmtDate(similar.week):'not available yet')+
        '</div>'+
        ...
        '<div style="display:flex;gap:8px;flex-wrap:wrap;margin:10px 0;font-size:10px;color:var(--muted)">'+
          '<span>Limiter: '+c.limitante_principal+'</span>'+
          '<span>Driver: '+c.impulsor_principal+'</span>'+
          (anchor.week?'<span>Best anchor: '+fmtDate(anchor.week)+'</span>':'')+
        '</div>'+
        environmentHtml+
        indicators+
        '<div style="font-size:12px;line-height:1.5;color:var(--text);padding-top:10px">'+esc(c.recomendacion)+'</div>'+
        nextStep+
        '<details style="margin-top:12px;border-top:1px solid rgba(255,255,255,.07);padding-top:10px">'+
          '<summary style="cursor:pointer;font-size:11px;font-weight:700;color:var(--muted);letter-spacing:.05em;list-style:none;display:flex;align-items:center;gap:6px" onclick="loadAcademia(this,\''+c.key+'\')">'+
            '<span style="font-size:14px">📖</span> WHAT DOES THIS SCORE MEAN?'+
          '</summary>'+
          '<div class="academia-content" style="margin-top:10px;font-size:12px;line-height:1.6;color:var(--muted)">Loading...</div>'+
        '</details>'+
      '</div>';
    }
    window.loadAcademia=function(summaryEl,key){
      ...
      fetch(API+'/gpt/academia/'+key).then(function(r){return r.json();}).then(function(data){
        if(!data.ok){container.textContent='No learning notes for this indicator.';return;}
        ...
        let html='';
        if(edu.qué_es)html+='<div style="margin-bottom:8px"><strong style="color:var(--text)">What does it measure?</strong><br>'+edu.qué_es+'</div>';
        ...
        if(edu.cómo_mejorar)html+='<div style="margin-bottom:8px"><strong style="color:var(--text)">How to improve:</strong><br>'+edu.cómo_mejorar+'</div>';
        ...
        container.innerHTML=html||'No description available.';
      }).catch(function(){container.textContent='Could not load the learning notes.';});
    };
    ...
    el.innerHTML=
      '<div class="grid2">'+
        ...
        metric('History',d.generated_from.snapshot_weeks,'weeks')+
        metric('Modelo','v1','historia personal')+
      '</div>'+
      '<div class="card"><div class="head"><h3>Honest read</h3><span>score ≠ confidence</span></div><div style="font-size:12px;line-height:1.55;color:var(--muted)">A score measures development. Confidence measures how well-supported it is. When data is missing, Epoch does not turn the gap into a bad grade — it tells you what is missing and why.</div></div>'+
      (function(){
        const ce=(capEvidence&&capEvidence.capacities)||[];
        if(!ce.length)return '';
        const lbl={aerobic_fitness:'Aerobic engine',endurance:'Endurance',power:'Power',strength_endurance:'Strength-endurance',recovery:'Recovery'};
        const rows=ce.slice(0,5).map(function(c){
          const ev=(c.evidence||[]).slice(0,2).map(function(s2){return (s2.name||s2.intent)+' ('+fmtShort(s2.date)+')'}).join(' · ');
          return '<div class="row"><div class="r-main"><div class="r-title">'+(lbl[c.capability]||c.capability)+'</div>'+
            '<div class="r-sub">'+ev+'</div></div>'+
            '<div class="r-val" style="color:#3dd68c">'+c.hours+' h</div></div>';
        }).join('');
        return '<div class="card" style="border-left:3px solid #3dd68c"><div class="head"><h3>What your month built</h3><span>last 4 weeks · with evidence</span></div>'+rows+
          '<div style="font-size:10px;color:var(--muted);margin-top:6px">Each capacity cites the sessions that built it.</div></div>';
      })()+
      ...
      evd('Escenarios de readiness',readinessView())+
      evd('History per capacity · 12-week blocks',historyView()+climbingHistoryView()+bodyCompositionHistoryView())+
      evd('Épocas similares en el historial',patronHistoricoView());
  }catch(e){
    el.innerHTML='<div class="card" style="color:var(--muted)">'+esc(e.message)+'</div>';
  }
}
...
// U6 — tap en zona del body map: pre-selecciona la zona y lleva al registro
function bmSel(zone){
  const sel=document.getElementById('wv-painzone');
  if(sel){sel.value=zone;sel.scrollIntoView({behavior:'smooth',block:'center'});}
  toast('Zone selected — log a niggle or a therapy');
}
