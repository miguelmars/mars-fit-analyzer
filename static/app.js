...
async function loadGear(){
  ...
  try{
    ...
    let html=`<div class="card"><div class="head"><h3>Alerts</h3><span>${(a.alerts||[]).length}</span></div>${(a.alerts||[]).length?(a.alerts||[]).map(x=>row('',x.name||x.type||'Alert',x.message||x.detail||'',x.km_left?x.km_left+' km':'' )).join(''):'No gear alerts'}</div>`;
...
async function loadCapacidades(){
  ...
  try{
    ...
    function capCard(c){
      ...
      return '<div class="card" style="border-left:3px solid '+color+'">'+
        '<div class="head"><h3>'+c.nombre+lvlChip+confBadge+'</h3><span style="color:'+color+'">'+(c.status==='calculated'?'calculated':'to calibrate')+'</span></div>'+
        '<div class="grid2">'+
