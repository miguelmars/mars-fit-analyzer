...
from db import DATABASE_URL, db_conn, get_db, _init_db, _ensure_gear_service_table, _ensure_gear_activity_links_table, _ensure_nutrition_table, _ensure_weight_table, _ensure_wellness_table, _ensure_fuerza_table, _ensure_accidents_table, _ensure_garmin_staging_tables, _ensure_clean_sessions_table, _ensure_clean_sessions_compat_view, RESULTS_STORE_MAX
...
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width,initial-scale=1">
  <title>Graficas no disponibles</title>
  <style>
    body{{margin:0;background:#08090b;color:#f7f7f4;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif}}
    ...
    p{{color:#cfd3dc;font-size:17px;line-height:1.45;margin:0}}
...
<body>
  <main>
    ...
    <section class="card">
      <h1>Graficas no disponibles.</h1>
      <p>Esta sesion viene del resumen limpio Garmin. Para mapa, altimetria y curvas segundo a segundo falta enlazar el FIT/TCX original.</p>
      <div class="actions">
        <a class="btn" href="/session/{session_id}">Detalle</a>
        <a class="btn" href="/activities">Actividades</a>
        <a class="btn" href="/activities?telemetry=1">Ver sesiones con curvas/mapa</a>
      </div>
    </section>
  </main>
...
def admin_backup(token: str = ""):
    ...
    tables = ["sessions", "routes", "post_session", "gear", "maintenance",
              ...
              "clean_sessions", "gear_activity_links"]
    backup = {}
    try:
        with conn.cursor() as cur:
            for table in tables:
...
async function loadActs(){
  try{
    ...
    const url=API+'/sessions?sport=cycling&limit=20'+(onlyTelemetry?'&telemetry=true':'');
    ...
    const filterHtml='<div style="display:flex;gap:8px;flex-wrap:wrap;padding:0 0 12px">'+
      '<button onclick="window.location.href=\x27/activities\x27" style="background:'+(onlyTelemetry?'rgba(255,255,255,.06)':'rgba(232,89,60,.18)')+';border:1px solid rgba(255,255,255,.1);border-radius:10px;padding:8px 10px;color:var(--text);font-weight:900;cursor:pointer">Todas</button>'+
      '<button onclick="window.location.href=\x27/activities?telemetry=1\x27" style="background:'+(onlyTelemetry?'rgba(232,89,60,.18)':'rgba(255,255,255,.06)')+';border:1px solid rgba(255,255,255,.1);border-radius:10px;padding:8px 10px;color:var(--text);font-weight:900;cursor:pointer">Con curvas/mapa</button>'+
    '</div>'+showingHtml;
    if(!arr.length){
