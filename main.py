...
from db import DATABASE_URL, db_conn, get_db
from mars_context import MARS_PROFILE_DEFAULT, _ensure_profile_table, _get_profile, get_zone_label, analyze_session_quick
...
def service_worker():
    ...
    sw_code = """const CACHE='bitacora-v18';
...
# ═══════════════════════════════════════════════════════════════════════════════
# MODULE: mars_context — Athlete profile, zones, goals
# See also: mars_context.py (standalone helper)
...
async function loadHome(){
  try{
    ...
    const failedEPs=[];
    if(!mp.zonas_ciclismo) failedEPs.push('/gpt/mars-context');
    if(!d.athlete) failedEPs.push('/gpt/dashboard');
    if(!w.sueno_promedio_horas&&!w.molestias_activas) failedEPs.push('/gpt/wellness-summary');
    ...
    $('home-data').innerHTML=
      ...
      '</div>'+
      (failedEPs.length?'<div style="background:rgba(232,89,60,.1);border:1px solid rgba(232,89,60,.3);border-radius:10px;padding:8px 12px;font-size:11px;color:#e8593c;margin-bottom:8px">⚠ Endpoints sin respuesta: '+failedEPs.join(', ')+'</div>':'')+'<div class="card" style="margin-bottom:8px">'+
