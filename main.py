...
<style>
:root{--bg:#0a0a0a;--s:#111;--s2:#181818;--b:#222;--a:#e8593c;--a2:#f2a623;--g:#3dd68c;--bl:#4a9eff;--t:#e8e6e0;--m:#555;--mono:'Azeret Mono',monospace;--serif:'Instrument Serif',serif}
*{box-sizing:border-box;margin:0;padding:0}
body{background:var(--bg);color:var(--t);font-family:var(--mono);min-height:100vh}
header{position:sticky;top:0;z-index:100;padding:0 28px;height:52px;display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid var(--b);background:rgba(10,10,10,.94);backdrop-filter:blur(12px)}
.logo{display:flex;align-items:baseline;gap:10px}
.logo-mark{font-family:var(--serif);font-style:italic;font-size:19px}
.logo-sub{font-size:9px;letter-spacing:.2em;color:var(--m);text-transform:uppercase}
.hdr-stats{display:flex;gap:20px;align-items:center}
.hdr-dot{width:6px;height:6px;border-radius:50%;background:var(--g);animation:pulse 2s infinite}
@keyframes pulse{0%,100%{opacity:1}50%{opacity:.3}}
.hdr-stat{font-size:10px;color:var(--m)}
.hdr-stat span{color:var(--t);font-weight:500}
main{max-width:1200px;margin:0 auto;padding:32px 28px}

/* hero */
.hero{display:grid;grid-template-columns:repeat(4,1fr);gap:1px;background:var(--b);border:1px solid var(--b);border-radius:14px;overflow:hidden;margin-bottom:36px}
.hs{background:var(--s);padding:24px 20px;position:relative;overflow:hidden;transition:background .2s}
.hs:hover{background:var(--s2)}
.hs::before{content:'';position:absolute;top:0;left:0;right:0;height:2px}
.hs:nth-child(1)::before{background:var(--a)}
.hs:nth-child(2)::before{background:var(--a2)}
.hs:nth-child(3)::before{background:var(--g)}
.hs:nth-child(4)::before{background:var(--bl)}
.hs-label{font-size:9px;letter-spacing:.2em;text-transform:uppercase;color:var(--m);margin-bottom:10px}
.hs-value{font-family:var(--serif);font-size:38px;letter-spacing:-.03em;line-height:1;margin-bottom:4px}
.hs-unit{font-size:10px;color:var(--m)}

/* filters */
.filters{display:flex;gap:10px;flex-wrap:wrap;margin-bottom:24px;align-items:center}
.filter-label{font-size:9px;letter-spacing:.15em;text-transform:uppercase;color:var(--m);margin-right:4px}
.filter-btn{padding:6px 14px;border-radius:20px;border:1px solid var(--b);background:transparent;color:var(--m);font-family:var(--mono);font-size:10px;cursor:pointer;transition:all .2s;white-space:nowrap}
.filter-btn:hover{border-color:#444;color:var(--t)}
.filter-btn.active{background:var(--a);border-color:var(--a);color:#fff}
.filter-btn.all.active{background:var(--bl);border-color:var(--bl)}
.filter-sep{width:1px;height:20px;background:var(--b);margin:0 4px}
.dist-filter{display:flex;align-items:center;gap:8px}
.dist-filter input{background:var(--s);border:1px solid var(--b);border-radius:6px;color:var(--t);font-family:var(--mono);font-size:10px;padding:5px 10px;width:70px}
.dist-filter input:focus{outline:none;border-color:var(--a)}

/* section */
.sec-hdr{display:flex;align-items:baseline;justify-content:space-between;margin-bottom:16px}
.sec-title{font-size:10px;letter-spacing:.2em;text-transform:uppercase;color:var(--m)}
.sec-count{font-size:10px;color:var(--m)}
.btn-refresh{background:none;border:none;color:var(--a);font-family:var(--mono);font-size:10px;cursor:pointer;letter-spacing:.1em}

/* grid */
.routes-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(320px,1fr));gap:14px;margin-bottom:40px}
.card{background:var(--s);border:1px solid var(--b);border-radius:12px;padding:18px;cursor:pointer;transition:border-color .2s,background .15s,transform .15s}
.card:hover{border-color:#333;background:var(--s2);transform:translateY(-1px)}
...
.card-name{font-family:var(--serif);font-style:italic;font-size:16px;line-height:1.2;max-width:68%}
.card-badge{font-size:9px;color:var(--a2);background:var(--s2);border:1px solid var(--b);border-radius:20px;padding:3px 9px;white-space:nowrap}
.card-metrics{display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;margin-bottom:12px}
.cm-label{font-size:8px;letter-spacing:.15em;text-transform:uppercase;color:var(--m);margin-bottom:3px}
.cm-value{font-size:15px;font-weight:500}
.cm-unit{font-size:9px;color:var(--m)}
.card-footer{padding-top:10px;border-top:1px solid var(--b);display:flex;gap:6px;flex-wrap:wrap}
.sport-tag{font-size:9px;padding:3px 8px;border-radius:10px;background:rgba(255,255,255,.04);border:1px solid var(--b);color:var(--m);text-transform:uppercase;letter-spacing:.05em}
.sport-cycling{color:var(--a);border-color:rgba(232,89,60,.2);background:rgba(232,89,60,.06)}
.sport-running{color:var(--g);border-color:rgba(61,214,140,.2);background:rgba(61,214,140,.06)}
.sport-walking{color:var(--bl);border-color:rgba(74,158,255,.2);background:rgba(74,158,255,.06)}
.sport-strength,.sport-strength_training{color:var(--a2);border-color:rgba(242,166,35,.2);background:rgba(242,166,35,.06)}
.sport-yoga{color:#c084fc;border-color:rgba(192,132,252,.2);background:rgba(192,132,252,.06)}

/* detail */
.detail{background:var(--s);border:1px solid var(--b);border-radius:14px;margin-bottom:40px;overflow:hidden;display:none}
.detail.show{display:block}
.det-hdr{padding:20px 24px;border-bottom:1px solid var(--b);display:flex;align-items:center;justify-content:space-between}
.det-title{font-family:var(--serif);font-style:italic;font-size:20px}
.det-close{background:none;border:1px solid var(--b);border-radius:8px;color:var(--m);font-family:var(--mono);font-size:10px;padding:5px 12px;cursor:pointer}
.det-close:hover{color:var(--t)}
.det-body{padding:24px}
.det-stats{display:grid;grid-template-columns:repeat(3,1fr);gap:14px;margin-bottom:24px}
.prog-grid{display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:24px}
.prog-item .prog-lbl{font-size:9px;letter-spacing:.15em;text-transform:uppercase;color:var(--m);margin-bottom:6px;display:flex;justify-content:space-between}
.prog-bg{height:4px;background:var(--b);border-radius:2px;overflow:hidden;margin-bottom:5px}
.prog-fill{height:100%;border-radius:2px;transition:width 1s ease;width:0}
.prog-note{font-size:10px;color:var(--m);line-height:1.4}
table{width:100%;border-collapse:collapse;font-size:11px}
th{text-align:left;font-size:9px;letter-spacing:.15em;text-transform:uppercase;color:var(--m);padding:0 10px 10px 0;border-bottom:1px solid var(--b)}
td{padding:10px 10px 10px 0;border-bottom:1px solid var(--b);color:var(--t)}
tr:last-child td{border-bottom:none}
tr:hover td{color:#fff}
.td-muted{color:var(--m)!important;font-size:10px}

/* loading / empty */
.loading,.empty{text-align:center;padding:48px 0;color:var(--m);font-size:10px;letter-spacing:.15em}
.spinner{width:20px;height:20px;border:2px solid var(--b);border-top-color:var(--a);border-radius:50%;animation:spin .8s linear infinite;margin:0 auto 14px}
...
@media(max-width:768px){
  ...
  .hdr-stats .hdr-stat{display:none}
}
</style>
</head>
<body>
<header>
  ...
  <div class="hdr-stats">
    ...
    <div class="hdr-stat" id="hSess">— <span>sesiones</span></div>
    <div class="hdr-stat" id="hRoutes">— <span>rutas</span></div>
  </div>
</header>

<main>
  <div class="hero">
    <div class="hs"><div class="hs-label">Rutas</div><div class="hs-value" id="stRoutes">—</div><div class="hs-unit">identificadas</div></div>
    <div class="hs"><div class="hs-label">Sesiones</div><div class="hs-value" id="stSess">—</div><div class="hs-unit">actividades</div></div>
    <div class="hs"><div class="hs-label">FC promedio</div><div class="hs-value" id="stHR">—</div><div class="hs-unit">bpm global</div></div>
    <div class="hs"><div class="hs-label">Vel. promedio</div><div class="hs-value" id="stSpd">—</div><div class="hs-unit">km/h global</div></div>
  </div>

  <div class="filters" id="filtersBar">
    <span class="filter-label">Deporte</span>
    <button class="filter-btn all active" onclick="setFilter('sport','all',this)">Todos</button>
    <div class="filter-sep"></div>
    <span class="filter-label">Distancia mín.</span>
    <div class="dist-filter">
      <input type="number" id="minDist" value="0" min="0" max="200" placeholder="km" onchange="applyFilters()">
      <span style="font-size:10px;color:var(--m)">km</span>
    </div>
    <div class="filter-sep"></div>
    <button class="filter-btn" onclick="setSort('times',this)">Más veces</button>
    <button class="filter-btn" onclick="setSort('distance',this)">Mayor dist.</button>
    <button class="filter-btn" onclick="setSort('recent',this)">Más reciente</button>
  </div>

  <div class="sec-hdr">
    <div class="sec-title">Rutas <span class="sec-count" id="routeCount"></span></div>
    <button class="btn-refresh" onclick="loadRoutes()">↻ Actualizar</button>
  </div>

  <div id="routesContainer"><div class="loading"><div class="spinner"></div>Cargando rutas...</div></div>

  <div class="detail" id="detailPanel">
    <div class="det-hdr">
      <div class="det-title" id="detTitle">—</div>
      <button class="det-close" onclick="closeDetail()">✕ Cerrar</button>
    </div>
...
let allRoutes=[];
let activeFilters={sport:'all'};
let activeSort='times';

const $=id=>document.getElementById(id);

const sportClass=s=>{
  const m={'cycling':'sport-cycling','running':'sport-running','walking':'sport-walking',
           'strength_training':'sport-strength','yoga':'sport-yoga','trail_running':'sport-running'};
  return m[s]||'';
};

const sportLabel=s=>{
  const m={'cycling':'Ciclismo','running':'Running','walking':'Caminata',
           'strength_training':'Fuerza','yoga':'Yoga','trail_running':'Trail',
           'indoor_cardio':'Cardio','lap_swimming':'Natación','open_water_swimming':'Natación'};
  return m[s]||(s||'—');
};
...
function setFilter(key,val,btn){
  ...
  if(btn){
    btn.closest('.filters').querySelectorAll('.filter-btn[data-group="'+key+'"], .filter-btn.all').forEach(b=>b.classList.remove('active'));
...
function setSort(val,btn){
  ...
  document.querySelectorAll('.filters .filter-btn').forEach(b=>{
    if(['Más veces','Mayor dist.','Más reciente'].includes(b.textContent)) b.classList.remove('active');
  });
  if(btn) btn.classList.add('active');
  renderRoutes();
}

function applyFilters(){ renderRoutes(); }
...
async function loadRoutes(){
  ...
  try{
    ...
    // Build sport filters dynamically
    const sports=[...new Set(allRoutes.map(r=>r.sport).filter(Boolean))];
    const filtersBar=$('filtersBar');
    // Remove old sport buttons
    filtersBar.querySelectorAll('.filter-btn[data-group="sport"]').forEach(b=>b.remove());
    const sep=filtersBar.querySelector('.filter-sep');
    sports.sort().forEach(sp=>{
      const btn=document.createElement('button');
      btn.className=`filter-btn ${sportClass(sp)}`;
      btn.setAttribute('data-group','sport');
      btn.textContent=sportLabel(sp);
      btn.onclick=()=>{
        filtersBar.querySelectorAll('.filter-btn.all,.filter-btn[data-group="sport"]').forEach(b=>b.classList.remove('active'));
        btn.classList.add('active');
        activeFilters.sport=sp;
        renderRoutes();
      };
      filtersBar.insertBefore(btn,sep);
    });

    // Hero stats
    const total=allRoutes.reduce((a,r)=>a+(r.times_ridden||0),0);
    const hrs=allRoutes.filter(r=>r.avg_hr_bpm);
    const spds=allRoutes.filter(r=>r.avg_speed_kmh&&r.avg_speed_kmh>0);
    ...
    $('hSess').innerHTML=`${total} <span>sesiones</span>`;
    $('hRoutes').innerHTML=`${allRoutes.length} <span>rutas</span>`;

    renderRoutes();
  }catch(e){
    $('routesContainer').innerHTML=`<div class="empty">Error: ${e.message}</div>`;
  }
}
...
function renderRoutes(){
  ...
  let filtered=allRoutes.filter(r=>{
    if(activeFilters.sport!=='all' && r.sport!==activeFilters.sport) return false;
  ...
  else if(activeSort==='recent') filtered.sort((a,b)=>(b.last_ride||'').localeCompare(a.last_ride||''));
  ...
  filtered.forEach(r=>{
    ...
    card.onclick=()=>loadDetail(r.route_id,r.name);
    card.innerHTML=`
      <div class="card-hdr">
        <div class="card-name">${r.name||'Ruta sin nombre'}</div>
        <div class="card-badge">${r.times_ridden||0} veces</div>
      </div>
      <div class="card-metrics">
        <div><div class="cm-label">Distancia</div><div class="cm-value">${(r.distance_km||0).toFixed(1)}<span class="cm-unit"> km</span></div></div>
        <div><div class="cm-label">FC prom.</div><div class="cm-value">${r.avg_hr_bpm||'—'}<span class="cm-unit"> bpm</span></div></div>
        <div><div class="cm-label">Vel. prom.</div><div class="cm-value">${r.avg_speed_kmh?(+r.avg_speed_kmh).toFixed(1):'—'}<span class="cm-unit"> km/h</span></div></div>
      </div>
      <div class="card-footer">
        <span class="sport-tag ${sportClass(r.sport)}">${sportLabel(r.sport)}</span>
        <span class="sport-tag">${r.first_ride||'—'} → ${r.last_ride||'—'}</span>
      </div>`;
...
async function loadDetail(id,name){
  ...
  $('detTitle').textContent=name||'Ruta';
  $('detBody').innerHTML='<div class="loading"><div class="spinner"></div>Cargando...</div>';
  ...
  try{
    ...
    let html=`<div class="det-stats">
      <div><div class="cm-label">Distancia</div><div style="font-size:28px;font-family:var(--serif);font-style:italic">${(d.distance_km||0).toFixed(1)} <span style="font-size:13px;color:var(--m)">km</span></div></div>
      <div><div class="cm-label">Ascenso</div><div style="font-size:28px;font-family:var(--serif);font-style:italic">+${d.ascent_m||'—'} <span style="font-size:13px;color:var(--m)">m</span></div></div>
      <div><div class="cm-label">Ejecuciones</div><div style="font-size:28px;font-family:var(--serif);font-style:italic">${d.times_ridden||0} <span style="font-size:13px;color:var(--m)">veces</span></div></div>
    </div>`;

    if(p.hr_note||p.speed_note){
      html+=`<div class="prog-grid">
        ${p.speed_note?`<div class="prog-item">
          <div class="prog-lbl"><span>Velocidad</span><span style="color:${p.speed_change_kmh>0?'var(--g)':'var(--a)'}">${p.speed_change_kmh>0?'+':''}${(p.speed_change_kmh||0).toFixed(1)} km/h</span></div>
          <div class="prog-bg"><div class="prog-fill" style="width:${Math.min(Math.abs(p.speed_change_kmh||0)*20,100)}%;background:${p.speed_change_kmh>0?'var(--g)':'var(--a)'}"></div></div>
          <div class="prog-note">${p.speed_note}</div>
        </div>`:''}
        ${p.hr_note?`<div class="prog-item">
          <div class="prog-lbl"><span>FC</span><span style="color:${p.hr_change_bpm<0?'var(--g)':'var(--a)'}">${p.hr_change_bpm>0?'+':''}${p.hr_change_bpm||0} bpm</span></div>
          <div class="prog-bg"><div class="prog-fill" style="width:${Math.min(Math.abs(p.hr_change_bpm||0)*5,100)}%;background:${p.hr_change_bpm<0?'var(--g)':'var(--a)'}"></div></div>
    ...
    if(rides.length){
      html+=`<div style="margin-top:20px">
        <div class="sec-title" style="margin-bottom:12px">Historial de ejecuciones</div>
        <table><thead><tr>
          <th>Fecha</th><th>Entrenamiento</th><th>FC avg</th><th>Vel avg</th><th>Cadencia</th>
        </tr></thead><tbody>
        ${rides.map((r,i)=>`<tr>
          <td class="td-muted">${r.date||'—'} ${i===0?'<span style="font-size:8px;color:var(--m)">primera</span>':''} ${i===rides.length-1?'<span style="font-size:8px;color:var(--a2)">última</span>':''}</td>
          <td class="td-muted">${(r.workout_name||'—').slice(0,28)}</td>
          <td>${r.avg_hr_bpm||'—'} <span class="td-muted">bpm</span></td>
          <td>${r.avg_speed_kmh?(+r.avg_speed_kmh).toFixed(1):'—'} <span class="td-muted">km/h</span></td>
          <td>${r.avg_cadence||'—'} <span class="td-muted">rpm</span></td>
        </tr>`).join('')}
        </tbody></table>
      </div>`;
    }
    ...
    $('detBody').innerHTML=`<div class="empty">Error: ${e.message}</div>`;
  }
}
