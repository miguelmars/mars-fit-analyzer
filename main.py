...
def service_worker():
    ...
    sw_code = """const CACHE='bitacora-v16';
...
async function loadAll() {
  // Load mars context first for personalized analysis
  try{
    const mp = await fetch(API+'/gpt/mars-context').then(r=>r.json());
    const z = mp.zonas_ciclismo||{};
    const z2 = z.z2||[134,150];
    const plan = mp.plan_garmin||{};
    const a = mp.athlete||{};
    const el = document.getElementById('mars-context-banner');
    if(el){
      el.innerHTML =
        '<div style="display:flex;flex-wrap:wrap;gap:8px;align-items:center">'+
        '<span style="background:rgba(61,214,140,.12);color:#3dd68c;padding:4px 10px;border-radius:20px;font-size:11px;font-weight:800">Z2: '+z2[0]+'–'+z2[1]+' bpm</span>'+
        '<span style="background:rgba(74,158,255,.12);color:#4a9eff;padding:4px 10px;border-radius:20px;font-size:11px;font-weight:800">'+( plan.fase||'Base aerobica')+'</span>'+
        '<span style="background:rgba(245,158,11,.12);color:#f59e0b;padding:4px 10px;border-radius:20px;font-size:11px;font-weight:800">Cad obj: '+(mp.cadencia_obj||100)+' rpm</span>'+
        '<span style="background:rgba(232,89,60,.12);color:#e8593c;padding:4px 10px;border-radius:20px;font-size:11px;font-weight:800">Peso: '+(a.peso_actual_kg||'—')+' → '+(a.peso_objetivo_kg||80)+' kg</span>'+
        '</div>';
...
<script>
const API=window.location.origin;
const THEME={home:'#ffffff',perfil:'#3dd68c',coach:'#4a9eff',progress:'#a78bfa',dashboard:'#e8593c',activities:'#e8593c',gear:'#f59e0b',calendar:'#22d3ee',performance:'#a78bfa',fuerza:'#c8f135',wellness:'#4a9eff',eficiencia:'#3dd68c',correlaciones:'#a78bfa',nutricion:'#f59e0b'};
  ...
  try{
    ...
    $('home-data').innerHTML=
      ...
      '<div class="card"><div class="head"><h3>Estado del atleta</h3><span>'+(a.fitness||'—')+'</span></div>'+
        ...
        row('·','Fatiga',a.fatiga||'—','')+
        row('·','Molestias',pains?pains+' activa(s)':'Sin alertas','')+
        '<div class="row"><div class="r-ico">·</div><div class="r-main"><div class="r-title">Peso</div><div class="r-sub">obj '+pesoObj+' kg'+(pesoDiff>0?' · faltan '+pesoDiff+' kg':'')+'</div></div><div class="r-val" style="color:#3dd68c">'+peso+' kg</div></div>'+
...
async function uploadFit(file){
  if(!file)return;
  $('upload-result').innerHTML='<div class="card">Procesando '+file.name+'...</div>';
...
async function loadCoach(){
  ...
  try{
    ...
    const z=mp.zonas_ciclismo||{},z2=z.z2||[134,150],z3=z.z3||[151,160];
