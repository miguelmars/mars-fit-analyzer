...
async function loadHome(){
  ...
  if($('home-greeting'))$('home-greeting').innerHTML=greet+'<br>Mars.';
  if($('home-date')){const D=new Date();const M=['enero','febrero','marzo','abril','mayo','junio','julio','agosto','septiembre','octubre','noviembre','diciembre'];$('home-date').textContent=D.getDate()+' de '+M[D.getMonth()]+' '+D.getFullYear();}
  try{
    ...
    const a=d.athlete||{},s=d.semana_actual||{},z=d.z2_check||{},carga=d.carga||{};
    const pains=w.molestias_activas||[];
    const tsbCol=carga.tsb>5?'#3dd68c':carga.tsb<-15?'#e8593c':'#4a9eff';
    // Weight sparkline
    const wHist=(wh.historial||[]).slice(-8);
    const currKg=wh.current_kg;
    const wDelta=wh.delta_kg;
    const wDeltaStr=wDelta!=null?(wDelta>0?'+':'')+wDelta+' kg':'';
    const wDeltaCol=wDelta!=null?(wDelta<0?'#3dd68c':wDelta>0?'#e8593c':'#8e95a3'):'#8e95a3';
    function miniSpark(vals,col){
      if(!vals||vals.length<2)return '';
      const mx=Math.max(...vals),mn=Math.min(...vals),rng=mx-mn||0.1;
      const W=80,H=28;
      const pts=vals.map((v,i)=>Math.round(i/(vals.length-1)*W)+','+Math.round(H-(v-mn)/rng*(H-4)-2)).join(' ');
      return `<svg viewBox="0 0 ${W} ${H}" style="width:80px;height:28px;display:block"><polyline points="${pts}" fill="none" stroke="${col}" stroke-width="1.8" stroke-linejoin="round"/></svg>`;
    }
    const wVals=wHist.map(e=>e.weight_kg).filter(Boolean);
    const wSpark=miniSpark(wVals,'#3dd68c');
    $('home-data').innerHTML=`
      <div class="card">
        <div class="head"><h3>Estado actual</h3><span style="color:var(--theme)">${a.fitness||'—'}</span></div>
        <div class="row" style="padding:10px 0">
          <div class="r-ico">⚖</div>
          <div class="r-main">
            <div class="r-title">Peso</div>
            <div class="r-sub">${wDeltaStr?`<span style="color:${wDeltaCol}">${wDeltaStr}</span> vs inicio`:'Sin registros anteriores'}</div>
          </div>
          <div style="text-align:right">
            <div style="font-size:20px;font-weight:950;color:#3dd68c">${currKg?currKg+' kg':'—'}</div>
            ${wSpark}
          </div>
        </div>
        ${row('⚡','Carga / Fatiga',`${a.fatiga||'—'} · TSB`,`<span style="color:${tsbCol}">${carga.tsb||0}</span>`)}
        ${row('🫀','Molestias',pains.length?pains.length+' activa(s)':'Sin alertas',pains.length?'⚠️':'✓')}
        ${row('🚴','Esta semana',Number(s.km||0).toFixed(0)+' km · '+Number(s.horas||0).toFixed(1)+' h',s.sesiones||0+' sesiones')}
        ${row('🎯','Z2 (4 semanas)',Number(z.pct_z2_4_semanas||0).toFixed(0)+'%','obj 70-80%')}
      </div>
      <div class="grid2">
        ${metric('Mars Index',a.mars_index?Number(a.mars_index).toFixed(1):'—','fitness score')}
        ${metric('Peso objetivo','80 kg',currKg?Math.max(0,(currKg-80)).toFixed(1)+' kg restantes':'—')}
      </div>
      ${d.recommendation?`<div class="card"><p style="font-size:13px;color:#c8cbd2;line-height:1.55">${d.recommendation}</p></div>`:''}`;
  }catch(e){$('home-data').innerHTML='<div class="card">Error: '+e.message+'</div>'}
}
...
app.router.routes = [r for r in app.router.routes if getattr(r, "path", None) not in _override_paths]
