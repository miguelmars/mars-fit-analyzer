    ...
    try:
        query = """
            SELECT session_id, start_time, sport, distance_km, duration_s,
                   ...
                   route_id, calories, source, quality
            FROM sessions_clean_compat
...
def get_charts(session_id: str):
    ...
    if not records:
        return HTMLResponse("<h2 style='color:white;background:#111;padding:20px;font-family:sans-serif'>Gráficas no disponibles.<br><small style='opacity:.6;font-size:14px'>Esta sesión viene del resumen limpio Garmin. Para mapa, altimetría y curvas segundo a segundo falta enlazar el FIT/TCX original.</small></h2>")
    ...
    try:
        ...
        with conn.cursor() as cur:
            cur.execute("""
                ...
                WHERE LOWER(COALESCE(status,'')) NOT LIKE '%retired%'
                ORDER BY type, name
            """)
        ...
        for r in garmin_rows:
            ...
            if str(d.get("name") or "").strip().lower() not in existing_names:
                components.append(d)
...
<body>
  ...
  <script>
    ...
    fetch('/gpt/session/' + sid).then(async r => {{
      ...
      document.getElementById('summary').textContent = 'Actividad limpia importada desde Garmin. Esta ficha ya usa la columna corregida; mapas y curvas se activarán cuando enlacemos el FIT/TCX original.';
      document.getElementById('chips').innerHTML = [
        s.start_time ? 'Fecha: ' + String(s.start_time).slice(0,10) : null,
...
async function loadDash(){
  try{
    ...
    const a=d.athlete||{},s=d.semana_actual||{},c=d.carga||{},z=d.z2_check||{};
