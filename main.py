...
def adaptive_coach():
    ...
    try:
        ...
        caps_raw = caps_data.get("capacidades", [])
        # capacidades es lista [{key, score, ...}] — convertir a dict para lookup rápido
...
@app.post("/gpt/rebuild-snapshots")
def rebuild_snapshots():
    ...
    Necesario para activar las capacidades (motor_aerobico, escalada, etc.).
    """
    conn = get_db()
    if not conn:
        return {"ok": False, "message": "DB no disponible"}
    try:
        # ── Paso 1: calcular efficiency faltante ─────────────────────────────
        with conn.cursor() as cur:
            cur.execute("""
        ...
        # ── Paso 2: rebuild athlete_snapshots ────────────────────────────────
        from tools.backfill_athlete_snapshots import build_snapshots, write_snapshots
        snapshots = build_snapshots(conn)
        if not snapshots:
