...
def add_weight(body: WeightIn):
    ...
    try:
        with conn.cursor() as cur:
            cur.execute("""
                INSERT INTO weight_log (date, weight_kg, waist_cm, body_fat_pct, notes)
...
def get_weight_history(limit: int = 60):
    ...
    try:
        with conn.cursor() as cur:
            cur.execute("""
                SELECT date, weight_kg, waist_cm, body_fat_pct, notes
                FROM weight_log ORDER BY date DESC LIMIT %s
            """, (limit,))
            rows = cur.fetchall()
            cols = [d[0] for d in cur.description]
        ...
        delta = None
        if len(entries_asc) >= 2:
            w0 = float(entries_asc[0].get('weight_kg') or 0)
            w1 = float(entries_asc[-1].get('weight_kg') or 0)
            if w0 and w1: delta = round(w1 - w0, 2)
        current = float(entries[0]['weight_kg']) if entries and entries[0].get('weight_kg') else None
        return {"current_kg": current, "delta_kg": delta, "registros": len(entries), "historial": entries_asc}
    except Exception as e:
        raise HTTPException(500, str(e))
...
def _ensure_gear_service_table(conn):
    with conn.cursor() as cur:
        cur.execute("""
            CREATE TABLE IF NOT EXISTS gear_service (
                id SERIAL PRIMARY KEY,
                gear_id TEXT,
                gear_name TEXT,
...
def add_gear_service(body: GearServiceIn):
    ...
    try:
        with conn.cursor() as cur:
            cur.execute("""
                INSERT INTO gear_service
                    (gear_id, gear_name, service_type, description, date,
                     km_at_service, cost_mxn, shop, notes)
...
def get_gear_service_history(limit: int = 50):
    ...
    try:
        with conn.cursor() as cur:
            cur.execute("""
                SELECT gs.*,
                  (CURRENT_DATE - gs.date) as days_since
                FROM gear_service gs
                ORDER BY gs.date DESC LIMIT %s
            """, (limit,))
            rows = cur.fetchall()
            cols = [d[0] for d in cur.description]
        entries = []
...
# ═══════════════════════════════════════════════════════════════════════════════
# TRENDS  — FC, cadencia, peso, horas (8 semanas sparklines)
# ═══════════════════════════════════════════════════════════════════════════════
...
def gpt_trends(weeks: int = 8):
    ...
    try:
        ...
        with conn.cursor() as cur:
            cur.execute("""
                SELECT date::text, weight_kg::float, waist_cm::float
                FROM weight_log
        ...
        return {
            ...
            "weight": weight,
            "sparklines": {
                "hr":     sparkline(weekly, "avg_hr"),
                "cad":    sparkline(weekly, "avg_cad"),
                "hours":  sparkline(weekly, "hours"),
                "km":     sparkline(weekly, "km"),
                "weight": sparkline(weight, "kg"),
            }
        }
    except Exception as e:
        raise HTTPException(500, str(e))
...
_override_paths = {"/home", "/dashboard", "/activities", "/gear", "/calendar", "/performance", "/app"}
