...
def service_worker():
    ...
    sw_code = """const CACHE='bitacora-v7';
...
@media(min-width:900px){
  ...
  .content{max-width:1100px;margin:0 auto;padding-left:28px;padding-right:28px;padding-bottom:72px;display:grid;grid-template-columns:200px 1fr;gap:0;align-items:start;box-sizing:border-box}
...
<div class="app">
  ...
  <div class="card" style="margin-top:4px">
    ...
    <div class="grid2">
      <div class="field"><label>Tipo</label><select id="gs-type"><option value="cambio_cadena">Cambio cadena</option><option value="cambio_llanta_del">Llanta del.</option><option value="cambio_llanta_tra">Llanta tra.</option><option value="cambio_cassette">Cassette</option><option value="ajuste_transmision">Ajuste transmision</option><option value="lubricacion">Lubricacion</option></select></div>
...
<script>
const API=window.location.origin;
const THEME={home:'#ffffff',dashboard:'#e8593c',activities:'#e8593c',gear:'#f59e0b',calendar:'#22d3ee',performance:'#a78bfa',fuerza:'#c8f135',wellness:'#4a9eff',eficiencia:'#3dd68c',correlaciones:'#a78bfa',nutricion:'#f59e0b'};
const TITLE={home:['Bitácora Mars','Home'],dashboard:['Dashboard','stats'],activities:['Bici','sesiones'],gear:['Gear','mantenimiento'],calendar:['Calendario','heatmap'],performance:['Récords','personales'],fuerza:['Fuerza','Compex'],wellness:['Wellness','recuperación'],eficiencia:['Eficiencia','aeróbica'],correlaciones:['Correlaciones','FC · Peso'],nutricion:['Nutrición','geles']};
let current='home';
...
async function loadActs(){try{let d=await fetch(API+'/sessions?sport=cycling&limit=20').then(r=>r.json());const arr=d.sessions||d||[];$('act-count').textContent=arr.length;$('act-list').innerHTML=arr.length?arr.map(s=>row('',s.workout_name||'Ciclismo',`${fmtDate(s.start_time)} · ${s.duration_hms||hms(s.duration_s)}`,`${s.distance_km||'—'} km`)).join(''):'Sin actividades'}catch(e){$('act-list').innerHTML='Error: '+e.message}}
...
async function loadWell(){try{const d=await fetch(API+'/gpt/wellness-summary?weeks=4').then(r=>r.json());const pains=d.molestias_activas||[];$('well-data').innerHTML=`<div class="grid2">${metric('Sueño',d.sueno_promedio_horas||'—','h promedio')}${metric('FC reposo',d.fc_reposo_promedio||'—','bpm')}${metric('Estrés',d.estres_promedio||'—','/10')}${metric('Molestias',pains.length,'activas')}</div><div class="card"><div class="head"><h3>Molestias activas</h3><span>${pains.length}</span></div>${pains.length?pains.map(p=>row('',p.pain_zone||'Molestia','Nivel '+(p.pain_level||'?')+'/10','')).join(''):'Sin molestias activas'}</div>`}catch(e){$('well-data').innerHTML='<div class="card">Error: '+e.message+'</div>'}}
...
async function loadGearHistory(){
  ...
  try{
    ...
    el.innerHTML='<div class="card"><div class="head"><h3>Historial servicios</h3><span>'+hist.length+'</span></div>'+hist.map(s=>row('·',SL[s.service_type]||s.service_type,fmtDate(s.date)+(s.km_at_service?' · '+Number(s.km_at_service).toLocaleString()+' km':'')+(s.days_since!=null?' · hace '+s.days_since+'d':''),s.cost_mxn?'$'+s.cost_mxn:'')).join('')+'</div>';
  }catch(e){el.innerHTML=''}
}
