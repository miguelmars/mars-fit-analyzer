...
def service_worker():
    ...
    sw_code = """const CACHE='bitacora-v6';
...
<style>
:root{
  --bg:#0a0b0e;--s1:#111318;--s2:#181c23;--s3:#1f242e;
  --ln:rgba(255,255,255,.07);--ln2:rgba(255,255,255,.14);
  --tx:#f4f4f1;--m1:#8b929f;--m2:#4e5663;
  --bike:#e8593c;--fza:#c8f135;--well:#4a9eff;--stats:#a78bfa;--gear:#f59e0b;
  --c:var(--bike);
...
#app{
  ...
  background:
    radial-gradient(ellipse 600px 300px at 50% -80px,color-mix(in srgb,var(--c) 12%,transparent),transparent 70%),
    var(--bg);
...
#topbar{
  ...
  padding:calc(env(safe-area-inset-top)+10px) 18px 8px;
  display:flex;align-items:center;justify-content:space-between;
}
...
.logo-m{
  ...
  font-size:18px;font-weight:950;color:#0a0b0e;
  transition:background .3s,box-shadow .3s;
}
.brand-txt small{display:block;font-size:10px;color:var(--m1);font-weight:700;letter-spacing:.04em;line-height:1.2}
.brand-txt strong{display:block;font-size:15px;font-weight:800;letter-spacing:-.025em}
.icon-btn{
...
#content{flex:1;overflow-y:auto;overflow-x:hidden;-webkit-overflow-scrolling:touch;padding:8px 16px 96px}
.screen{display:none;animation:fu .18s ease}
.screen.active{display:block}
@keyframes fu{from{opacity:.2;transform:translateY(6px)}to{opacity:1;transform:none}}
#bnav{
  ...
  border-radius:20px;display:flex;z-index:100;
  box-shadow:0 12px 40px rgba(0,0,0,.55);
}
.nav{
  flex:1;border:none;background:transparent;color:var(--m2);
  display:flex;flex-direction:column;align-items:center;justify-content:center;gap:2px;cursor:pointer;
  padding:0 2px;
}
.nav svg{transition:transform .2s,filter .2s,opacity .2s;opacity:.45}
.nav .lbl{font-size:8px;font-weight:900;text-transform:uppercase;letter-spacing:.02em;transition:color .2s}
.nav.active{color:var(--c)}
.nav.active svg{opacity:1;transform:translateY(-1px);filter:drop-shadow(0 0 5px var(--c))}
.card{background:linear-gradient(170deg,var(--s2),var(--s1));border:1px solid var(--ln);border-radius:var(--r);padding:16px;margin-bottom:12px}
.card-sm{background:linear-gradient(170deg,var(--s2),var(--s1));border:1px solid var(--ln);border-radius:var(--r2);padding:13px}
...
.kl{font-size:10px;text-transform:uppercase;letter-spacing:.08em;color:var(--m1);font-weight:700;margin-bottom:4px}
.kv{font-size:24px;font-weight:950;letter-spacing:-.04em;line-height:1;color:var(--c)}
.ku{font-size:10px;color:var(--m1);margin-top:2px}
.sh{display:flex;align-items:center;justify-content:space-between;margin-bottom:10px}
.sh h3{font-size:11px;font-weight:800;text-transform:uppercase;letter-spacing:.08em;color:var(--m1)}
.sh a{font-size:12px;color:var(--c);font-weight:700;cursor:pointer}
.carga-card{
  border-radius:var(--r);padding:18px 20px 16px;margin-bottom:12px;
  background:linear-gradient(135deg,color-mix(in srgb,var(--bike) 28%,#1a1108) 0%,#131510 55%,#0e1010 100%);
  border:1px solid color-mix(in srgb,var(--bike) 20%,var(--ln));
  position:relative;overflow:hidden;
}
.carga-card::after{content:"";position:absolute;right:-40px;top:-40px;width:140px;height:140px;
  border-radius:50%;background:color-mix(in srgb,var(--bike) 14%,transparent);filter:blur(12px);pointer-events:none}
.c-label{font-size:10px;font-weight:800;letter-spacing:.12em;text-transform:uppercase;
  color:color-mix(in srgb,var(--bike) 85%,#fff);margin-bottom:2px}
.c-sub{font-size:11px;color:var(--m1);margin-bottom:10px}
.c-num{font-size:50px;font-weight:950;letter-spacing:-.05em;line-height:1;color:var(--bike)}
.c-badge{display:inline-flex;align-items:center;
  background:rgba(163,230,53,.14);border:1px solid rgba(163,230,53,.3);
  border-radius:20px;padding:3px 10px;font-size:11px;font-weight:800;color:#a3e635;margin-left:10px}
.spark-wrap{margin-top:14px;display:flex;align-items:flex-end;gap:5px;height:48px}
.sc{flex:1;display:flex;flex-direction:column;align-items:center}
.sb{width:100%;border-radius:3px 3px 0 0;background:rgba(255,255,255,.09);min-height:3px}
.sb.tod{background:var(--bike)}
.sd{font-size:9px;color:var(--m2);margin-top:3px;font-weight:700}
.sd.tod{color:var(--bike)}
.status-row{display:flex;align-items:center;gap:12px;padding:11px 0;border-bottom:1px solid var(--ln)}
.status-row:last-child{border-bottom:none}
.status-ico{width:34px;height:34px;border-radius:10px;flex-shrink:0;
  background:color-mix(in srgb,var(--c) 10%,var(--s2));
  display:flex;align-items:center;justify-content:center}
.status-lbl{flex:1;font-size:13px;font-weight:700}
.status-sub{font-size:11px;color:var(--m1);margin-top:1px}
.status-val{font-size:14px;font-weight:900;text-align:right;flex-shrink:0}
.act-row{display:flex;align-items:center;gap:12px;padding:12px 0;border-bottom:1px solid var(--ln);cursor:pointer}
.act-row:last-child{border-bottom:none}
.act-row:active{opacity:.7}
.act-ico{width:42px;height:42px;border-radius:13px;flex-shrink:0;
  background:color-mix(in srgb,var(--bike) 12%,var(--s2));
  display:flex;align-items:center;justify-content:center}
.act-info{flex:1;min-width:0}
.act-name{font-size:14px;font-weight:800;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;margin-bottom:2px}
.act-date{font-size:11px;color:var(--m1);margin-bottom:5px}
.act-stats{display:flex;gap:10px;flex-wrap:wrap}
.stat-item{font-size:11px;color:var(--m1)}
.stat-item span{color:var(--tx);font-weight:700}
.act-km{font-size:17px;font-weight:950;color:var(--bike);text-align:right;flex-shrink:0}
.upload-zone{
  border:2px dashed color-mix(in srgb,var(--bike) 30%,var(--ln));
  background:color-mix(in srgb,var(--bike) 4%,var(--s1));
  border-radius:var(--r);padding:28px 20px;text-align:center;
  cursor:pointer;margin-bottom:12px;display:block;
}
.upload-zone:active{background:color-mix(in srgb,var(--bike) 8%,var(--s1))}
.upload-zone input{display:none}
.upload-zone h3{font-size:16px;font-weight:900;margin:10px 0 4px}
.upload-zone p{font-size:12px;color:var(--m1)}
.pbar{height:5px;background:var(--s3);border-radius:3px;overflow:hidden;margin:6px 0}
.pfill{height:100%;border-radius:3px;background:var(--c);transition:width .4s}
.zone-row{display:flex;align-items:center;gap:10px;padding:7px 0;border-bottom:1px solid var(--ln)}
.zone-row:last-child{border-bottom:none}
.zdot{width:8px;height:8px;border-radius:50%;flex-shrink:0}
.zlbl{font-size:12px;color:var(--m1);flex:1;font-weight:600}
.zbar-wrap{flex:2;height:5px;background:var(--s3);border-radius:3px;overflow:hidden}
.zbar-fill{height:100%;border-radius:3px}
.zpct{font-size:11px;font-weight:800;width:34px;text-align:right}
.gear-row{display:flex;align-items:center;gap:12px;padding:12px 0;border-bottom:1px solid var(--ln)}
.gear-row:last-child{border-bottom:none}
.gear-ico{width:38px;height:38px;border-radius:11px;background:color-mix(in srgb,var(--gear) 10%,var(--s2));
  display:flex;align-items:center;justify-content:center;flex-shrink:0}
.gear-info{flex:1;min-width:0}
.gear-name{font-size:13px;font-weight:800;margin-bottom:4px}
.gear-bar-wrap{height:4px;background:var(--s3);border-radius:2px;overflow:hidden;margin-bottom:3px}
.gear-bar-fill{height:100%;border-radius:2px}
.gear-km{font-size:10px;color:var(--m1)}
.gear-pct{font-size:15px;font-weight:950;flex-shrink:0}
.rec-row{display:flex;align-items:center;gap:12px;padding:11px 0;border-bottom:1px solid var(--ln)}
.rec-row:last-child{border-bottom:none}
.rec-dot{width:10px;height:10px;border-radius:50%;flex-shrink:0}
.rec-lbl{font-size:12px;color:var(--m1);font-weight:600}
.rec-date{font-size:10px;color:var(--m2);margin-top:1px}
.rec-val{font-size:16px;font-weight:950;text-align:right}
.well-ring-wrap{display:flex;flex-direction:column;align-items:center;margin:4px 0 16px}
.well-ring{
  width:88px;height:88px;border-radius:50%;
  background:conic-gradient(var(--well) var(--p,75%),rgba(255,255,255,.05) 0);
  display:flex;align-items:center;justify-content:center;position:relative;
  box-shadow:0 0 24px color-mix(in srgb,var(--well) 25%,transparent);
}
.well-ring::before{content:"";position:absolute;width:64px;height:64px;border-radius:50%;background:var(--s1)}
.well-ring span{position:relative;font-size:22px;font-weight:950;color:var(--well)}
.well-status{font-size:12px;color:var(--m1);margin-top:8px;text-align:center;line-height:1.4}
.well-status strong{display:block;font-size:14px;font-weight:800;color:var(--tx);margin-bottom:2px}
.cal-grid{display:grid;grid-template-columns:repeat(7,1fr);gap:4px}
.cday{aspect-ratio:1;border-radius:7px;background:var(--s2);
  display:flex;align-items:center;justify-content:center;font-size:10px;color:var(--m2);font-weight:700}
.cday.h1{background:color-mix(in srgb,var(--bike) 18%,var(--s2))}
.cday.h2{background:color-mix(in srgb,var(--bike) 38%,var(--s2))}
.cday.h3{background:color-mix(in srgb,var(--bike) 62%,var(--s2));color:#fff}
.cday.h4{background:var(--bike);color:#0a0b0e;font-weight:900}
.cdow{font-size:9px;text-align:center;color:var(--m2);font-weight:700;padding:2px 0}
.pill-row{display:flex;gap:6px;overflow-x:auto;scrollbar-width:none;padding-bottom:2px;margin-bottom:12px}
.pill-row::-webkit-scrollbar{display:none}
.pill{padding:7px 14px;border-radius:20px;font-size:12px;font-weight:800;
  border:1px solid var(--ln);background:var(--s2);color:var(--m1);
  white-space:nowrap;cursor:pointer;flex-shrink:0;transition:all .15s}
.pill.on{background:var(--c);border-color:var(--c);color:#0a0b0e}
input,select,textarea{
  width:100%;background:var(--s3);border:1px solid var(--ln2);
  border-radius:var(--r3);padding:11px 14px;font-size:14px;
  color:var(--tx);outline:none;-webkit-appearance:none;
}
input:focus,select:focus,textarea:focus{border-color:var(--c);box-shadow:0 0 0 3px color-mix(in srgb,var(--c) 12%,transparent)}
.field{margin-bottom:10px}
.field label{display:block;font-size:10px;text-transform:uppercase;letter-spacing:.07em;color:var(--m1);font-weight:800;margin-bottom:5px}
.btn{width:100%;border:none;border-radius:var(--r2);padding:15px;
  background:var(--c);color:#0a0b0e;font-size:15px;font-weight:950;cursor:pointer}
.btn2{background:rgba(255,255,255,.05);color:var(--tx);border:1px solid var(--ln2);
  border-radius:var(--r2);padding:12px 16px;font-size:13px;font-weight:700;width:100%;cursor:pointer}
.toast{position:fixed;top:calc(env(safe-area-inset-top)+12px);left:50%;transform:translateX(-50%);
  padding:10px 20px;border-radius:999px;font-size:13px;font-weight:900;
  z-index:999;display:none;color:#0a0b0e;white-space:nowrap}
.spin{display:inline-block;width:20px;height:20px;border:2px solid var(--ln2);
  border-top-color:var(--c);border-radius:50%;animation:sp .7s linear infinite}
@keyframes sp{to{transform:rotate(360deg)}}
.loading{display:flex;align-items:center;justify-content:center;padding:40px;gap:10px;color:var(--m1);font-size:13px}
@media(min-width:700px){
  #app{max-width:420px;margin:0 auto;border-left:1px solid var(--ln);border-right:1px solid var(--ln)}
}
</style>
</head>
<body>
...
<div id="topbar">
  <div class="brand">
    <div class="logo-m" id="logo-m">M</div>
    <div class="brand-txt">
      <small id="kicker">Bitácora Mars</small>
      <strong id="titletxt">Inicio</strong>
    </div>
  </div>
  <button class="icon-btn" onclick="refresh()"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M23 4v6h-6"/><path d="M1 20v-6h6"/><path d="M3.51 9a9 9 0 0 1 14.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0 0 20.49 15"/></svg></button>
</div>
<div id="content">
  <div class="screen active" id="s-home">
    <div id="home-data"><div class="loading"><div class="spin"></div>Cargando...</div></div>
  </div>
  <div class="screen" id="s-activities">
    <label class="upload-zone" id="drop-zone">
      <input type="file" accept=".fit,.zip" onchange="uploadFit(this.files[0])">
      ...
      <h3>Subir FIT / ZIP</h3>
      <p>Garmin Connect export</p>
    </label>
    <div id="upload-prog" style="display:none;margin-bottom:10px">
      <div style="font-size:12px;color:var(--m1);margin-bottom:5px" id="up-lbl">Procesando...</div>
      <div class="pbar"><div class="pfill" id="up-fill" style="width:0%"></div></div>
    </div>
    <div id="upload-result"></div>
    <div class="sh" style="margin-top:8px"><h3>Ultimas actividades</h3><span id="act-cnt" style="font-size:11px;color:var(--m1)"></span></div>
    <div class="card" style="padding:8px 14px" id="act-list"><div class="loading"><div class="spin"></div></div></div>
  </div>
  <div class="screen" id="s-dashboard">
    <div id="dash-data"><div class="loading"><div class="spin"></div>Cargando...</div></div>
  </div>
  <div class="screen" id="s-gear">
    <div id="gear-data"><div class="loading"><div class="spin"></div>Cargando...</div></div>
  </div>
  <div class="screen" id="s-calendar">
    <div id="cal-data"><div class="loading"><div class="spin"></div>Cargando...</div></div>
  </div>
  <div class="screen" id="s-performance">
    <div id="perf-data"><div class="loading"><div class="spin"></div>Cargando...</div></div>
  </div>
  <div class="screen" id="s-fuerza">
    <div class="card">
      <div class="sh"><h3>Categoria</h3></div>
      <div class="pill-row" id="fcat-pills">
        <div class="pill on" onclick="setFcat('compex',this)">Compex</div>
        <div class="pill" onclick="setFcat('gym',this)">Gimnasio</div>
        <div class="pill" onclick="setFcat('plyo',this)">Pliometria</div>
        <div class="pill" onclick="setFcat('core',this)">Core</div>
        <div class="pill" onclick="setFcat('bands',this)">Bandas</div>
      </div>
      <div id="f-compex-extra">
        <div class="field"><label>Programa</label>
          <select id="fv-prog"><option value="strength">Strength</option><option value="explosive_strength">Explosive Strength</option><option value="resistance">Resistance</option><option value="active_recovery">Active Recovery</option></select>
        </div>
        <div class="field">
          <label>Intensidad &mdash; <span id="fv-int-lbl" style="color:var(--fza);font-size:17px;font-weight:950">58</span></label>
          <input type="range" min="1" max="120" value="58" id="fv-int" oninput="$('fv-int-lbl').textContent=this.value" style="accent-color:var(--fza);margin-top:6px;width:100%">
        </div>
      </div>
      <div class="field"><label>Musculos</label>
        <div class="pill-row" id="fmuscle-pills" style="flex-wrap:wrap">
          <div class="pill" onclick="this.classList.toggle('on')">Cuadriceps</div>
          <div class="pill" onclick="this.classList.toggle('on')">Isquios</div>
          <div class="pill" onclick="this.classList.toggle('on')">Gluteos</div>
          <div class="pill" onclick="this.classList.toggle('on')">Pantorrillas</div>
          <div class="pill" onclick="this.classList.toggle('on')">Core</div>
          <div class="pill" onclick="this.classList.toggle('on')">Espalda</div>
          <div class="pill" onclick="this.classList.toggle('on')">Hombros</div>
        </div>
      </div>
      <div class="g2">
        <div class="field"><label>Duracion (min)</label><input type="number" id="fv-duration" placeholder="32"></div>
        <div class="field"><label>RPE (1-10)</label><input type="number" id="fv-rpe" min="1" max="10" placeholder="7"></div>
      </div>
      <div class="field"><label>Notas</label><input id="fv-notes" placeholder="Sensaciones..."></div>
      <button class="btn" style="--c:var(--fza)" onclick="saveFuerza()">Guardar fuerza</button>
    </div>
    <div id="fuerza-data"></div>
  </div>
  <div class="screen" id="s-wellness">
    <div id="well-summary"><div class="loading"><div class="spin"></div></div></div>
    <div class="card">
      <div class="sh"><h3>Registro rapido</h3></div>
      <div class="pill-row" id="wcat-pills">
        <div class="pill on" onclick="setWcat('sleep',this)">Sueno</div>
        <div class="pill" onclick="setWcat('compex_recovery',this)">Compex</div>
        <div class="pill" onclick="setWcat('massage_gun',this)">Pistola</div>
        <div class="pill" onclick="setWcat('ceragem',this)">Ceragem</div>
        <div class="pill" onclick="setWcat('pain',this)">Molestia</div>
        <div class="pill" onclick="setWcat('stress',this)">Estres</div>
      </div>
      <div id="well-fields">
        <div class="g2">
          <div class="field"><label>Horas sueno</label><input type="number" step="0.5" id="wf-sh" placeholder="7.5"></div>
          <div class="field"><label>Calidad</label><select id="wf-sq"><option value="">-</option><option value="bueno">Bueno</option><option value="regular">Regular</option><option value="malo">Malo</option></select></div>
        </div>
        <div class="g2">
          <div class="field"><label>FC reposo</label><input type="number" id="wf-hr" placeholder="52"></div>
          <div class="field"><label>Score Garmin</label><input type="number" id="wf-gs" placeholder="78"></div>
        </div>
      </div>
      <div class="g2">
        <div class="field"><label>Fatiga (1-10)</label><input type="number" id="wf-fat" min="1" max="10" placeholder="5"></div>
        <div class="field"><label>Notas</label><input id="wf-notes" placeholder="..."></div>
      </div>
      <button class="btn" style="--c:var(--well)" onclick="saveWellness()">Guardar wellness</button>
    </div>
  </div>
</div>
<nav id="bnav">
  <button class="nav active" data-s="home" onclick="go('home')"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg><span class="lbl">Inicio</span></button>
  <button class="nav" data-s="activities" onclick="go('activities')"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><circle cx="5.5" cy="17.5" r="3.5"/><circle cx="18.5" cy="17.5" r="3.5"/><path d="M15 6a1 1 0 1 0 0-2 1 1 0 0 0 0 2"/><path d="M10 7L7.5 12H5.5m4.5-5l5 3 3.5 7.5M10 7l2-5"/></svg><span class="lbl">Bici</span></button>
  <button class="nav" data-s="dashboard" onclick="go('dashboard')"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="20" x2="18" y2="10"/><line x1="12" y1="20" x2="12" y2="4"/><line x1="6" y1="20" x2="6" y2="14"/></svg><span class="lbl">Stats</span></button>
  <button class="nav" data-s="gear" onclick="go('gear')"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 0 1-2.83 2.83l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-4 0v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 0 1-2.83-2.83l.06-.06A1.65 1.65 0 0 0 4.68 15a1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1 0-4h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 0 1 2.83-2.83l.06.06A1.65 1.65 0 0 0 9 4.68a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 4 0v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 0 1 2.83 2.83l-.06.06A1.65 1.65 0 0 0 19.4 9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 0 4h-.09a1.65 1.65 0 0 0-1.51 1z"/></svg><span class="lbl">Gear</span></button>
  <button class="nav" data-s="calendar" onclick="go('calendar')"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg><span class="lbl">Cal</span></button>
  <button class="nav" data-s="fuerza" onclick="go('fuerza')"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M6.5 6.5h11M6.5 17.5h11M5 12h14"/><circle cx="3.5" cy="12" r="1.5"/><circle cx="20.5" cy="12" r="1.5"/><circle cx="3.5" cy="6.5" r="1.5"/><circle cx="20.5" cy="6.5" r="1.5"/><circle cx="3.5" cy="17.5" r="1.5"/><circle cx="20.5" cy="17.5" r="1.5"/></svg><span class="lbl">Fuerza</span></button>
  <button class="nav" data-s="wellness" onclick="go('wellness')"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg><span class="lbl">Wellness</span></button>
  <button class="nav" data-s="performance" onclick="go('performance')"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg><span class="lbl">Records</span></button>
</nav>
</div>
<div class="toast" id="toast"></div>
<script>
const API=window.location.origin;
const THEME={home:'#ffffff',activities:'#e8593c',dashboard:'#a78bfa',gear:'#f59e0b',calendar:'#22d3ee',performance:'#a78bfa',fuerza:'#c8f135',wellness:'#4a9eff'};
const LABELS={home:'Inicio',activities:'Bici',dashboard:'Stats',gear:'Gear',calendar:'Calendario',performance:'Records',fuerza:'Fuerza',wellness:'Wellness'};
let _cur='home',_wcat='sleep',_fcat='compex';
function $(id){return document.getElementById(id)}
function setTheme(s){
  const col=THEME[s]||'#fff';
  document.documentElement.style.setProperty('--c',col);
  $('kicker').textContent='Bitacora Mars';
  $('titletxt').textContent=LABELS[s]||s;
  $('logo-m').style.background='linear-gradient(135deg,'+col+',color-mix(in srgb,'+col+' 45%,#000))';
  $('logo-m').style.boxShadow='0 0 20px color-mix(in srgb,'+col+' 25%,transparent)';
}
function go(s){
  _cur=s;setTheme(s);
...
function refresh(){load(_cur)}
function toast(m,col){
  const t=$('toast');t.textContent=m;t.style.background=col||'var(--c)';t.style.display='block';
...
function load(s){
  if(s==='home')loadHome();
  else if(s==='activities')loadActs();
  else if(s==='dashboard')loadDash();
  else if(s==='gear')loadGear();
  else if(s==='calendar')loadCal();
  else if(s==='performance')loadPerf();
  else if(s==='fuerza')loadFuerza();
  else if(s==='wellness')loadWell();
}
async function loadHome(){
  const el=$('home-data');
  const h=new Date().getHours();
  ...
  const dateStr=D.getDate()+' de '+MN[D.getMonth()]+' '+D.getFullYear();
  try{
    const [dash,wh]=await Promise.all([
      fetch(API+'/gpt/dashboard').then(r=>r.json()),
      fetch(API+'/weight/history?limit=8').then(r=>r.json()).catch(()=>({})),
    ]);
    const a=dash.athlete||{},s=dash.semana_actual||{},z=dash.z2_check||{},c=dash.carga||{};
    const km=Number(s.km||0).toFixed(0);
    const tsbCol=c.tsb>5?'#3dd68c':c.tsb<-15?'var(--bike)':'var(--well)';
    const tsbLbl=c.tsb>5?'Optimo':c.tsb<-15?'Fatigado':'En carga';
    const z2pct=Number(z.pct_z2_4_semanas||0).toFixed(0);
    const wCurr=wh.current_kg,wDelta=wh.delta_kg;
    const D2=new Date(),dow=(D2.getDay()+6)%7;
    const DAYS=['L','M','X','J','V','S','D'];
    const sparks=DAYS.map((_,i)=>{
      const isT=i===dow;
      const ht=isT?Math.min(100,Math.round(Number(s.km||0)*5)):Math.round(Math.random()*50+8);
      return '<div class="sc"><div class="sb'+(isT?' tod':'')+'" style="height:'+ht+'%"></div><div class="sd'+(isT?' tod':'')+'">'+DAYS[i]+'</div></div>';
    }).join('');
    el.innerHTML=`
      <div style="font-size:12px;color:var(--m1);margin-bottom:4px">${dateStr}</div>
      <div style="font-size:26px;font-weight:950;letter-spacing:-.04em;margin-bottom:16px;line-height:1.2">${greet},<br>Mars</div>
      <div class="carga-card">
        <div class="c-label">Carga semanal</div>
        <div class="c-sub">Esta semana</div>
        <div><span class="c-num">${km}</span><span style="font-size:14px;color:var(--m1)"> km</span><span class="c-badge">${tsbLbl}</span></div>
        <div class="spark-wrap">${sparks}</div>
      </div>
      <div class="g4" style="margin-bottom:12px">
        <div class="card-sm"><div class="kl">Km</div><div class="kv" style="font-size:20px">${km}</div></div>
        <div class="card-sm"><div class="kl">Horas</div><div class="kv" style="font-size:20px">${Number(s.horas||0).toFixed(1)}</div></div>
        <div class="card-sm"><div class="kl">Z2%</div><div class="kv" style="font-size:20px;color:var(--well)">${z2pct}</div></div>
        <div class="card-sm"><div class="kl">TSB</div><div class="kv" style="font-size:20px;color:${tsbCol}">${c.tsb||0}</div></div>
      </div>
      <div class="card">
        <div class="sh"><h3>Estado actual</h3><span style="color:var(--c);font-size:12px;font-weight:800">${a.fitness||'-'}</span></div>
        <div class="status-row">
          <div class="status-ico"><div style="width:8px;height:8px;border-radius:50%;background:var(--bike)"></div></div>
          <div style="flex:1"><div class="status-lbl">Fitness / Mars Index</div><div class="status-sub">${dash.recommendation||'Sin recomendacion'}</div></div>
          <div class="status-val" style="color:var(--c)">${a.mars_index?Number(a.mars_index).toFixed(1):'-'}</div>
        </div>
        <div class="status-row">
          <div class="status-ico"><div style="width:8px;height:8px;border-radius:50%;background:var(--m1)"></div></div>
          <div style="flex:1"><div class="status-lbl">Fatiga</div></div>
          <div class="status-val" style="color:${a.fatiga==='baja'?'#3dd68c':a.fatiga==='alta'?'var(--bike)':'var(--m1)'}">${a.fatiga||'-'}</div>
        </div>
        ${wCurr?`<div class="status-row">
          <div class="status-ico"><div style="width:8px;height:8px;border-radius:50%;background:#3dd68c"></div></div>
          <div style="flex:1"><div class="status-lbl">Peso</div>${wDelta!=null?'<div class="status-sub">'+(wDelta>0?'+':'')+wDelta+' kg vs inicio</div>':''}</div>
          <div class="status-val" style="color:#3dd68c">${wCurr} kg</div>
        </div>`:''}</div>
      <div class="sh" style="margin-top:4px"><h3>Zona 2 objetivo 70-80%</h3><span style="color:var(--well);font-weight:800">${z2pct}%</span></div>
      <div class="pbar" style="margin-bottom:14px"><div class="pfill" style="width:${Math.min(z2pct,100)}%;background:var(--well)"></div></div>
      <button class="btn" onclick="go('activities')">+ Subir actividad</button>`;
  }catch(e){el.innerHTML='<div class="card" style="color:var(--m1);padding:20px">'+e.message+'</div>'}
}
async function loadActs(){
  try{
    const d=await fetch(API+'/sessions?sport=cycling&limit=20').then(r=>r.json());
    const arr=d.sessions||d||[];
    $('act-cnt').textContent=arr.length+' sesiones';
    $('act-list').innerHTML=arr.length?arr.map(s=>`
      <div class="act-row" onclick="window.open('/charts/${s.session_id}','_blank')">
        <div class="act-ico"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><circle cx="5.5" cy="17.5" r="3.5"/><circle cx="18.5" cy="17.5" r="3.5"/><path d="M10 7L7.5 12H5.5m4.5-5l5 3 3.5 7.5M10 7l2-5"/></svg></div>
        <div class="act-info">
          <div class="act-name">${s.workout_name||'Ciclismo'}</div>
          <div class="act-date">${fmtDate(s.start_time)}</div>
          <div class="act-stats">
            <div class="stat-item"><span>${hms(s.duration_s)}</span></div>
            <div class="stat-item">ascenso <span>${s.ascent_m||0} m</span></div>
            <div class="stat-item">FC <span>${s.avg_hr_bpm||'-'} bpm</span></div>
          </div>
        </div>
        <div class="act-km">${s.distance_km||'-'} km</div>
      </div>`).join(''):'<div style="color:var(--m1);padding:16px;text-align:center;font-size:13px">Sin actividades</div>';
    const dz=$('drop-zone');
    dz.addEventListener('dragover',e=>{e.preventDefault();dz.style.borderColor='var(--bike)'});
    dz.addEventListener('dragleave',()=>dz.style.borderColor='');
    dz.addEventListener('drop',e=>{e.preventDefault();dz.style.borderColor='';const f=e.dataTransfer.files[0];if(f)uploadFit(f)});
  }catch(e){$('act-list').innerHTML='<div style="color:var(--m1)">'+e.message+'</div>'}
}
async function uploadFit(file){
  if(!file)return;
  $('upload-result').innerHTML='';
  $('upload-prog').style.display='block';
  $('up-lbl').textContent='Procesando '+file.name+'...';
  $('up-fill').style.width='30%';
  const fd=new FormData();fd.append('file',file);
  try{
    const d=await fetch(API+'/analyze-fit',{method:'POST',body:fd}).then(r=>r.json());
    $('up-fill').style.width='100%';
    setTimeout(()=>{$('upload-prog').style.display='none';showResult(d)},400);
  }catch(e){$('upload-prog').style.display='none';toast('Error: '+e.message)}
}
function showResult(d){
  const s=d.session||{},isDup=d.duplicate===true;
  $('upload-result').innerHTML=`
    <div class="card" style="border-color:${isDup?'rgba(242,166,35,.35)':'rgba(61,214,140,.3)'};margin-bottom:12px">
      <div class="sh"><h3>${isDup?'Ya registrada':'Sesion guardada'}</h3><span style="font-size:10px;font-family:monospace;color:var(--m1)">${d.session_id||''}</span></div>
      <div class="g2">
        <div class="card-sm"><div class="kl">Fecha</div><div style="font-size:12px;font-weight:800">${fmtDate(s.start_time)}</div></div>
        <div class="card-sm"><div class="kl">Distancia</div><div style="font-size:18px;font-weight:950;color:var(--bike)">${s.distance_km||'-'} km</div></div>
        <div class="card-sm"><div class="kl">Duracion</div><div style="font-size:14px;font-weight:800">${s.duration_hms||hms(s.duration_s)||'-'}</div></div>
        <div class="card-sm"><div class="kl">FC prom</div><div style="font-size:14px;font-weight:800">${s.avg_hr_bpm||'-'} bpm</div></div>
      </div>
    </div>`;
  const ach=d.achievements||[];
  if(ach.length)ach.forEach((a,i)=>setTimeout(()=>toast('PR: '+a,'#f59e0b'),i*800));
  loadActs();
}
async function loadDash(){
  const el=$('dash-data');
  try{
    const [d,zd]=await Promise.all([
      fetch(API+'/gpt/performance-profile?sport=cycling').then(r=>r.json()),
      fetch(API+'/gpt/zones-summary?sport=cycling&weeks=4').then(r=>r.json()).catch(()=>({})),
    ]);
    const r=d.records||{},c=d.carga||{};
    const tsbCol=c.estado==='fresco'?'#3dd68c':c.estado==='fatigado'?'var(--bike)':'var(--well)';
    const zones=zd.zonas||{};
    const ZD=[
      {k:'z5',l:'Z5 Maximo',col:'#c026d3'},
      {k:'z4',l:'Z4 Umbral',col:'var(--bike)'},
      {k:'z3',l:'Z3 Tempo',col:'#f59e0b'},
      {k:'z2',l:'Z2 Aerobico',col:'#3dd68c'},
      {k:'z1',l:'Z1 Recuperacion',col:'var(--well)'},
    ];
    const RD=[
      {k:'max_distance',l:'Mayor distancia',col:'var(--bike)',f:v=>Number(v).toFixed(1)+' km'},
      {k:'max_ascent',l:'Mayor ascenso',col:'#c8f135',f:v=>'+'+parseInt(v)+' m'},
      {k:'max_duration',l:'Sesion mas larga',col:'var(--well)',f:v=>hms(v)},
      {k:'max_speed',l:'Mayor velocidad',col:'var(--stats)',f:v=>Number(v).toFixed(1)+' km/h'},
    ];
    el.innerHTML=`
      <div class="g2">
        <div class="card-sm"><div class="kl">VO2Max est.</div><div class="kv" style="color:var(--stats)">${d.vo2max_estimado||'-'}</div><div class="ku">ml/kg/min</div></div>
        <div class="card-sm"><div class="kl">Carga</div><div class="kv" style="font-size:16px;color:${tsbCol}">${c.estado||'-'}</div><div class="ku">TSB ${c.tsb||0}</div></div>
        <div class="card-sm"><div class="kl">Eficiencia</div><div class="kv" style="font-size:18px">${d.eficiencia_aerobica?.delta_pct_6_meses!=null?(d.eficiencia_aerobica.delta_pct_6_meses>=0?'+':'')+Number(d.eficiencia_aerobica.delta_pct_6_meses).toFixed(1)+'%':'-'}</div></div>
        <div class="card-sm"><div class="kl">Cadencia</div><div class="kv" style="font-size:18px">${d.cadencia_trend||'-'}</div></div>
      </div>
      <div class="card">
        <div class="sh"><h3>Zonas FC 4 semanas</h3></div>
        ${ZD.map(z=>{const p=Number(zones[z.k+'_pct']||zones[z.k]||0).toFixed(0);return '<div class="zone-row"><div class="zdot" style="background:'+z.col+'"></div><div class="zlbl">'+z.l+'</div><div class="zbar-wrap"><div class="zbar-fill" style="width:'+p+'%;background:'+z.col+'"></div></div><div class="zpct" style="color:'+z.col+'">'+p+'%</div></div>';}).join('')}
      </div>
      <div class="card">
        <div class="sh"><h3>Records personales</h3></div>
        ${RD.map(rd=>{const v=r[rd.k];if(!v)return '';return '<div class="rec-row"><div class="rec-dot" style="background:'+rd.col+'"></div><div style="flex:1"><div class="rec-lbl">'+rd.l+'</div><div class="rec-date">'+fmtDate(v.date)+'</div></div><div class="rec-val" style="color:'+rd.col+'">'+rd.f(v.value)+'</div></div>';}).join('')}
      </div>`;
  }catch(e){el.innerHTML='<div class="card" style="color:var(--m1)">'+e.message+'</div>'}
}
async function loadGear(){
  const el=$('gear-data');
  try{
    const [st,al]=await Promise.all([
      fetch(API+'/gpt/gear-status').then(r=>r.json()).catch(()=>({components:[]})),
      fetch(API+'/gpt/gear-alerts').then(r=>r.json()).catch(()=>({alerts:[]})),
    ]);
    const items=st.components||st.gear||[],alerts=al.alerts||[];
    let html='';
    if(alerts.length){
      html+='<div class="card" style="border-color:rgba(232,89,60,.3);margin-bottom:12px"><div class="sh"><h3 style="color:var(--bike)">Alertas</h3><span>'+alerts.length+'</span></div>'+alerts.map(a=>'<div class="gear-row"><div class="gear-ico"><div style="width:8px;height:8px;border-radius:50%;background:var(--bike)"></div></div><div class="gear-info"><div class="gear-name">'+a.name+'</div><div class="gear-km">'+a.message+'</div></div><div class="gear-pct" style="color:var(--bike)">'+a.pct_used+'%</div></div>').join('')+'</div>';
    }
    html+='<div class="card"><div class="sh"><h3>Componentes</h3><span>'+items.length+'</span></div>';
    if(items.length){
      html+=items.map(g=>{
        const cur=Number(g.km_current||g.current_km||g.km_used||0);
        const lim=Number(g.km_limit||g.limit_km||4500);
        const pct=Math.min(100,Math.round((cur/lim)*100));
        const col=pct>=90?'var(--bike)':pct>=70?'#f59e0b':'#3dd68c';
        return '<div class="gear-row"><div class="gear-ico"><div style="width:8px;height:8px;border-radius:50%;background:var(--gear)"></div></div><div class="gear-info"><div class="gear-name">'+g.name+'</div><div class="gear-bar-wrap"><div class="gear-bar-fill" style="width:'+pct+'%;background:'+col+'"></div></div><div class="gear-km">'+cur.toLocaleString()+' / '+lim.toLocaleString()+' km</div></div><div class="gear-pct" style="color:'+col+'">'+pct+'%</div></div>';
      }).join('');
    }else{
      html+='<div style="color:var(--m1);text-align:center;padding:20px;font-size:13px">Sin componentes registrados</div>';
    }
    html+='</div>';
    el.innerHTML=html;
  }catch(e){el.innerHTML='<div class="card" style="color:var(--m1)">'+e.message+'</div>'}
...
async function loadCal(){
  ...
  try{
    ...
    const days=d.days||d.calendar||[];
    const recent=days.slice(-42);
    const active=recent.filter(x=>x.count||x.sessions).length;
    const DOWS=['L','M','X','J','V','S','D'];
    el.innerHTML='<div class="g2" style="margin-bottom:14px"><div class="card-sm"><div class="kl">Dias activos</div><div class="kv" style="color:var(--cal,#22d3ee)">'+active+'</div><div class="ku">6 semanas</div></div><div class="card-sm"><div class="kl">Sesiones</div><div class="kv">'+recent.reduce((a,x)=>a+(x.count||x.sessions||0),0)+'</div></div></div>'+
      '<div class="card"><div style="display:grid;grid-template-columns:repeat(7,1fr);gap:3px;margin-bottom:4px">'+DOWS.map(d=>'<div class="cdow">'+d+'</div>').join('')+'</div><div class="cal-grid">'+
      recent.map(x=>{const n=x.count||x.sessions||0;const cls=n===0?'':n===1?'h1':n===2?'h2':n===3?'h3':'h4';return '<div class="cday '+cls+'" title="'+x.date+'">'+((x.date||'').slice(8,10)||'')+'</div>';}).join('')+'</div></div>';
  }catch(e){el.innerHTML='<div class="card" style="color:var(--m1)">'+e.message+'</div>'}
}
async function loadPerf(){
  ...
  try{
    ...
    const c=d.carga||{};
    const tsbCol=c.estado==='fresco'?'#3dd68c':c.estado==='fatigado'?'var(--bike)':'var(--well)';
    el.innerHTML='<div class="g2"><div class="card-sm"><div class="kl">ATL agudo</div><div class="kv" style="font-size:22px">'+( c.atl||'-')+'</div></div><div class="card-sm"><div class="kl">CTL cronico</div><div class="kv" style="font-size:22px">'+( c.ctl||'-')+'</div></div><div class="card-sm"><div class="kl">TSB balance</div><div class="kv" style="font-size:22px;color:'+tsbCol+'">'+( c.tsb||0)+'</div></div><div class="card-sm"><div class="kl">Estado</div><div class="kv" style="font-size:16px;color:'+tsbCol+'">'+( c.estado||'-')+'</div></div></div>';
  }catch(e){el.innerHTML='<div class="card" style="color:var(--m1)">'+e.message+'</div>'}
}
function setFcat(v,el){
  _fcat=v;
  document.querySelectorAll('#fcat-pills .pill').forEach(p=>p.classList.remove('on'));
  el.classList.add('on');
  $('f-compex-extra').style.display=v==='compex'?'block':'none';
}
async function loadFuerza(){
  try{
    const d=await fetch(API+'/gpt/fuerza-summary?weeks=8').then(r=>r.json());
    $('fuerza-data').innerHTML='<div class="g2"><div class="card-sm"><div class="kl">Sesiones 8s</div><div class="kv" style="color:var(--fza)">'+( d.total_sesiones||0)+'</div></div><div class="card-sm"><div class="kl">Horas total</div><div class="kv" style="color:var(--fza)">'+( d.total_horas||0)+'</div></div></div>';
  }catch(e){}
}
async function saveFuerza(){
  const muscles=[...document.querySelectorAll('#fmuscle-pills .pill.on')].map(b=>b.textContent.trim().toLowerCase());
  const body={date:new Date().toISOString().slice(0,10),category:_fcat,
    subcategory:_fcat==='compex'?($('fv-prog')?.value||null):null,
    intensity:_fcat==='compex'?parseInt($('fv-int')?.value)||null:null,
    muscle_groups:muscles,
    duration_min:parseInt($('fv-duration')?.value)||null,
    rpe:parseInt($('fv-rpe')?.value)||null,
    notes:$('fv-notes')?.value||null};
  const d=await fetch(API+'/fuerza',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)}).then(r=>r.json());
  toast(d.ok?'Fuerza guardada':'Error','var(--fza)');
  if(d.ok){document.querySelectorAll('#fmuscle-pills .pill').forEach(p=>p.classList.remove('on'));loadFuerza();}
}
const WFIELDS={
  sleep:'<div class="g2"><div class="field"><label>Horas sueno</label><input type="number" step="0.5" id="wf-sh" placeholder="7.5"></div><div class="field"><label>Calidad</label><select id="wf-sq"><option value="">-</option><option value="bueno">Bueno</option><option value="regular">Regular</option><option value="malo">Malo</option></select></div></div><div class="g2"><div class="field"><label>FC reposo</label><input type="number" id="wf-hr" placeholder="52"></div><div class="field"><label>Score Garmin</label><input type="number" id="wf-gs" placeholder="78"></div></div>',
  compex_recovery:'<div class="g2"><div class="field"><label>Programa</label><select id="wf-prog"><option value="active_recovery">Active Recovery</option><option value="recovery">Recovery</option><option value="massage">Massage</option></select></div><div class="field"><label>Duracion (min)</label><input type="number" id="wf-dur" placeholder="20"></div></div>',
  massage_gun:'<div class="field"><label>Duracion (min)</label><input type="number" id="wf-dur" placeholder="15"></div>',
  ceragem:'<div class="g2"><div class="field"><label>Duracion (min)</label><input type="number" id="wf-dur" placeholder="40"></div><div class="field"><label>Sensacion antes</label><input type="number" id="wf-sb" placeholder="6"></div></div><div class="field"><label>Sensacion despues</label><input type="number" id="wf-sa" placeholder="8"></div>',
  pain:'<div class="g2"><div class="field"><label>Zona</label><input id="wf-pz" placeholder="Isquiotibial izq."></div><div class="field"><label>Nivel (1-10)</label><input type="number" id="wf-pl" placeholder="4"></div></div>',
  stress:'<div class="g2"><div class="field"><label>Nivel (1-10)</label><input type="number" id="wf-sl" placeholder="6"></div><div class="field"><label>Causa</label><select id="wf-sc"><option value="">-</option><option value="work">Trabajo</option><option value="fatigue">Fatiga</option><option value="illness">Enfermedad</option></select></div></div>',
};
function setWcat(v,el){
  _wcat=v;
  document.querySelectorAll('#wcat-pills .pill').forEach(p=>p.classList.remove('on'));
  el.classList.add('on');
  $('well-fields').innerHTML=WFIELDS[v]||'';
}
async function loadWell(){
  const el=$('well-summary');
  try{
    const d=await fetch(API+'/gpt/wellness-summary?weeks=4').then(r=>r.json());
    const pains=d.molestias_activas||[];
    const sleep=d.sueno_promedio_horas||'-',stress=d.estres_promedio||'-',fc=d.fc_reposo_promedio||'-';
    const score=Math.round(85-(Number(stress||5)-5)*3);
    el.innerHTML='<div class="card" style="margin-bottom:12px"><div class="well-ring-wrap"><div class="well-ring" style="--p:'+score+'%"><span>'+score+'</span></div><div class="well-status"><strong>Estado general</strong>'+( score>=80?'Listo para entrenar fuerte.':score>=60?'Recuperacion normal.':'Prioriza descanso.')+'</div></div><div class="g3"><div class="card-sm"><div class="kl">Sueno</div><div class="kv" style="font-size:18px;color:var(--well)">'+sleep+' <span style="font-size:10px;color:var(--m1)">h</span></div></div><div class="card-sm"><div class="kl">FC reposo</div><div class="kv" style="font-size:18px">'+fc+' <span style="font-size:10px;color:var(--m1)">bpm</span></div></div><div class="card-sm"><div class="kl">Estres</div><div class="kv" style="font-size:18px;color:'+(Number(stress)>6?'var(--bike)':'#3dd68c')+'">'+stress+' <span style="font-size:10px;color:var(--m1)">/10</span></div></div></div>'+(pains.length?'<div style="border-top:1px solid var(--ln);padding-top:10px"><div class="sh"><h3 style="color:var(--bike)">Molestias activas</h3><span>'+pains.length+'</span></div>'+pains.map(p=>'<div style="display:flex;justify-content:space-between;padding:6px 0;font-size:13px;border-bottom:1px solid var(--ln)"><span>'+p.pain_zone+'</span><span style="color:var(--bike);font-weight:800">Nivel '+p.pain_level+'/10</span></div>').join('')+'</div>':'<div style="color:#3dd68c;font-size:12px;text-align:center;padding:8px 0;font-weight:700">Sin molestias activas</div>')+'</div>';
  }catch(e){el.innerHTML=''}
}
async function saveWellness(){
  const today=new Date().toISOString().slice(0,10);
  const body={date:today,category:_wcat,fatigue:parseInt($('wf-fat')?.value)||null,notes:$('wf-notes')?.value||null};
  if(_wcat==='sleep'){body.sleep_hours=parseFloat($('wf-sh')?.value)||null;body.sleep_quality=$('wf-sq')?.value||null;body.hr_rest=parseInt($('wf-hr')?.value)||null;body.garmin_sleep_score=parseInt($('wf-gs')?.value)||null;}
  else if(_wcat==='pain'){body.pain_zone=$('wf-pz')?.value||null;body.pain_level=parseInt($('wf-pl')?.value)||null;}
  else if(_wcat==='ceragem'){body.ceragem_duration_min=parseInt($('wf-dur')?.value)||null;body.ceragem_sensation_before=parseInt($('wf-sb')?.value)||null;body.ceragem_sensation_after=parseInt($('wf-sa')?.value)||null;}
  else if(_wcat==='stress'){body.stress_level=parseInt($('wf-sl')?.value)||null;body.stress_cause=$('wf-sc')?.value||null;}
  else{body.duration_min=parseInt($('wf-dur')?.value)||null;body.compex_program=$('wf-prog')?.value||null;}
  ...
  toast(d.ok?'Wellness guardado':'Error','var(--well)');
  if(d.ok)loadWell();
}
go('home');
</script>
