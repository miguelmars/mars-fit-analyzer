...
:root{
  ...
  --tx:#f4f4f1;--m1:#8b929f;--m2:#555e6b;
  --bike:#e8593c;--fza:#c8f135;--well:#4a9eff;--stats:#a78bfa;--gear:#f59e0b;--cal:#22d3ee;
  --c:var(--bike);
...
#app{
  ...
  background:
    radial-gradient(ellipse 800px 400px at 50% -100px,
...
.logo-m{
  ...
  font-size:20px;font-weight:950;color:#08090b;
  transition:background .3s,box-shadow .3s;
}
.brand-txt small{display:block;font-size:10px;color:var(--m1);font-weight:700;letter-spacing:.04em;line-height:1.2}
.brand-txt strong{display:block;font-size:15px;font-weight:800;letter-spacing:-.025em;line-height:1.3}
.icon-btn{
  width:38px;height:38px;border-radius:13px;
  border:1px solid var(--ln2);background:rgba(255,255,255,.04);
  color:var(--tx);font-size:17px;display:flex;align-items:center;justify-content:center;cursor:pointer;
}

/* ── Content ── */
#content{flex:1;overflow-y:auto;overflow-x:hidden;-webkit-overflow-scrolling:touch;padding:8px 16px 100px}
.screen{display:none;animation:fadeup .2s ease}
.screen.active{display:block}
@keyframes fadeup{from{opacity:.3;transform:translateY(7px)}to{opacity:1;transform:none}}
...
.nav .ico{font-size:19px;line-height:1;transition:transform .2s,filter .2s}
.nav .lbl{font-size:9px;font-weight:900;text-transform:uppercase;letter-spacing:.03em;transition:color .2s}
.nav.active{color:var(--c)}
.nav.active .ico{transform:translateY(-2px);filter:drop-shadow(0 0 8px var(--c))}
...
.card-sm{
  background:linear-gradient(170deg,var(--s2),var(--s1));
  border:1px solid var(--ln);border-radius:var(--r2);padding:14px;
}
...
/* ── KPIs ── */
.kl{font-size:10px;text-transform:uppercase;letter-spacing:.08em;color:var(--m1);font-weight:700;margin-bottom:5px}
.kv{font-size:26px;font-weight:950;letter-spacing:-.04em;line-height:1;color:var(--c)}
.ku{font-size:11px;color:var(--m1);margin-top:2px}

/* ── Hero card (carga semanal naranja) ── */
.carga-card{
  border-radius:var(--r);padding:18px 20px 16px;margin-bottom:12px;
  background:linear-gradient(135deg,
    color-mix(in srgb,var(--bike) 30%,#1a1209) 0%,
    #141610 60%,#0e1010 100%);
  border:1px solid color-mix(in srgb,var(--bike) 22%,var(--ln));
  box-shadow:var(--sh);position:relative;overflow:hidden;
}
.carga-card::after{
  content:"";position:absolute;right:-50px;top:-50px;
  width:160px;height:160px;border-radius:50%;
  background:color-mix(in srgb,var(--bike) 16%,transparent);
  filter:blur(10px);pointer-events:none;
}
.carga-label{font-size:10px;font-weight:800;letter-spacing:.12em;text-transform:uppercase;
  color:color-mix(in srgb,var(--bike) 85%,#fff);margin-bottom:2px}
.carga-sub{font-size:11px;color:var(--m1);margin-bottom:10px}
.carga-num{font-size:52px;font-weight:950;letter-spacing:-.05em;line-height:1;color:var(--bike)}
.carga-badge{
  display:inline-flex;align-items:center;
  background:rgba(163,230,53,.15);border:1px solid rgba(163,230,53,.3);
  border-radius:20px;padding:3px 10px;font-size:11px;font-weight:800;color:#a3e635;
  margin-left:10px;vertical-align:middle;
}
.sparkline-wrap{margin-top:14px;display:flex;align-items:flex-end;gap:5px;height:52px}
.spark-col{flex:1;display:flex;flex-direction:column;align-items:center}
.spark-bar{
  width:100%;border-radius:4px 4px 0 0;
  background:rgba(255,255,255,.1);min-height:4px;
  transition:height .4s ease;
}
.spark-bar.today{background:var(--bike)}
.spark-day{font-size:9px;color:var(--m2);margin-top:3px;font-weight:700}
.spark-day.today{color:var(--bike)}

/* ── Greeting ── */
.greeting-date{font-size:12px;color:var(--m1);margin-bottom:4px}
.greeting-name{font-size:26px;font-weight:950;letter-spacing:-.04em;line-height:1.1;margin-bottom:14px}

/* ── Section header ── */
.sh{display:flex;align-items:center;justify-content:space-between;margin-bottom:10px}
.sh h3{font-size:12px;font-weight:800;text-transform:uppercase;letter-spacing:.08em;color:var(--m1)}
.sh a{font-size:12px;color:var(--c);font-weight:700;cursor:pointer}

/* ── Activity row (estilo Strava/Garmin) ── */
.act-item{
  display:flex;align-items:center;gap:12px;
  padding:13px 0;border-bottom:1px solid var(--ln);cursor:pointer;
}
.act-item:last-child{border-bottom:none}
.act-item:active{opacity:.7}
.act-ico{
  width:44px;height:44px;border-radius:14px;flex-shrink:0;
  display:flex;align-items:center;justify-content:center;font-size:19px;
  background:color-mix(in srgb,var(--bike) 14%,#0f1115);
}
.act-info{flex:1;min-width:0}
...
.act-stat{font-size:12px;font-weight:700;color:var(--m1)}
.act-stat span{color:var(--tx);font-weight:800}
.act-km{font-size:16px;font-weight:950;color:var(--bike);text-align:right;flex-shrink:0}

/* ── Upload zone ── */
.upload-zone{
  border:2px dashed color-mix(in srgb,var(--c) 35%,var(--ln));
  background:color-mix(in srgb,var(--c) 5%,var(--s1));
  border-radius:var(--r);padding:28px 20px;text-align:center;
  cursor:pointer;margin-bottom:12px;transition:all .2s;
}
.upload-zone.drag{
  border-color:var(--c);
  background:color-mix(in srgb,var(--c) 10%,var(--s1));
}
.upload-zone input{display:none}
.upload-zone .ico{font-size:40px;margin-bottom:8px}
.upload-zone h3{font-size:17px;font-weight:900;margin-bottom:3px}
.upload-zone p{font-size:12px;color:var(--m1)}

/* ── Progress bar ── */
.pbar{height:6px;background:var(--s3);border-radius:3px;overflow:hidden;margin:6px 0}
.pfill{height:100%;border-radius:3px;background:var(--c);transition:width .5s}

/* ── Zone bars ── */
.zone-row{display:flex;align-items:center;gap:10px;padding:7px 0;border-bottom:1px solid var(--ln)}
.zone-row:last-child{border-bottom:none}
.zone-dot{width:8px;height:8px;border-radius:50%;flex-shrink:0}
.zone-lbl{font-size:12px;color:var(--m1);flex:1;font-weight:600}
.zone-bar-wrap{flex:2;height:5px;background:var(--s3);border-radius:3px;overflow:hidden}
.zone-bar-fill{height:100%;border-radius:3px}
.zone-pct{font-size:11px;font-weight:800;width:34px;text-align:right}
...
.gear-ico{width:40px;height:40px;border-radius:12px;background:color-mix(in srgb,var(--gear) 12%,#111);
  display:flex;align-items:center;justify-content:center;font-size:16px;flex-shrink:0}
.gear-info{flex:1;min-width:0}
.gear-name{font-size:13px;font-weight:800;margin-bottom:3px}
.gear-bar-wrap{height:5px;background:var(--s3);border-radius:3px;overflow:hidden;margin-bottom:2px}
.gear-bar-fill{height:100%;border-radius:3px}
.gear-km{font-size:10px;color:var(--m1)}
.gear-pct{font-size:14px;font-weight:900;flex-shrink:0}

/* ── Wellness status ring ── */
.well-ring-wrap{display:flex;flex-direction:column;align-items:center;margin:10px 0 16px}
.well-ring{
  width:90px;height:90px;border-radius:50%;
  background:conic-gradient(var(--well) var(--p,75%),rgba(255,255,255,.06) 0);
  display:flex;align-items:center;justify-content:center;
  position:relative;box-shadow:0 0 28px color-mix(in srgb,var(--well) 30%,transparent);
}
.well-ring::before{
  content:"";position:absolute;width:66px;height:66px;
  border-radius:50%;background:var(--s1);
}
.well-ring span{position:relative;font-size:24px;font-weight:950;color:var(--well)}
.well-status{font-size:12px;color:var(--m1);margin-top:8px;text-align:center}
.well-status strong{display:block;font-size:14px;font-weight:800;color:var(--tx);margin-bottom:2px}

/* ── Calendar heatmap ── */
.cal-grid{display:grid;grid-template-columns:repeat(7,1fr);gap:4px}
.cal-day{aspect-ratio:1;border-radius:8px;background:var(--s2);
  display:flex;align-items:center;justify-content:center;
  font-size:10px;color:var(--m2);font-weight:700}
.cal-day.h1{background:color-mix(in srgb,var(--bike) 20%,var(--s2))}
.cal-day.h2{background:color-mix(in srgb,var(--bike) 40%,var(--s2))}
.cal-day.h3{background:color-mix(in srgb,var(--bike) 65%,var(--s2));color:#fff}
.cal-day.h4{background:var(--bike);color:#08090b}
.cal-dow{font-size:9px;text-align:center;color:var(--m2);font-weight:700;padding:3px 0}

/* ── Tabs pills ── */
.pill-row{display:flex;gap:6px;overflow-x:auto;scrollbar-width:none;padding-bottom:4px;margin-bottom:12px}
...
.pill.on{background:var(--c);border-color:var(--c);color:#08090b}

/* ── Forms ── */
input,select,textarea{
  width:100%;background:var(--s3);border:1px solid var(--ln2);
  border-radius:var(--r3);padding:12px 14px;font-size:14px;
...
.field label{display:block;font-size:10px;text-transform:uppercase;
  letter-spacing:.07em;color:var(--m1);font-weight:800;margin-bottom:5px}

/* ── Buttons ── */
.btn{
  width:100%;border:none;border-radius:var(--r2);padding:15px;
  background:var(--c);color:#08090b;font-size:15px;font-weight:950;cursor:pointer;
  transition:opacity .15s;
}
.btn:active{opacity:.8}
.btn2{background:rgba(255,255,255,.05);color:var(--tx);
  border:1px solid var(--ln2);cursor:pointer;border-radius:var(--r2);
  padding:12px 16px;font-size:13px;font-weight:700;width:100%}

/* ── Records ── */
.rec-row{display:flex;align-items:center;gap:12px;padding:11px 0;border-bottom:1px solid var(--ln)}
.rec-row:last-child{border-bottom:none}
.rec-ico{width:36px;height:36px;border-radius:11px;display:flex;align-items:center;
...
.toast{
  ...
  z-index:999;display:none;color:#08090b;white-space:nowrap;
}
...
.loading{display:flex;align-items:center;justify-content:center;
  padding:40px;gap:10px;color:var(--m1);font-size:13px}

/* ── Bodymap ── */
.bodymap{height:200px;border-radius:var(--r);
  background:radial-gradient(circle at 50% 40%,
    color-mix(in srgb,var(--fza) 15%,transparent),transparent 40%),
  linear-gradient(180deg,var(--s2),var(--s1));
  border:1px solid var(--ln);position:relative;overflow:hidden;margin-bottom:12px}
.human{position:absolute;left:50%;top:18px;transform:translateX(-50%);width:90px;height:175px}
.human div{position:absolute;background:#2d3240}
.human .h{left:31px;top:0;width:26px;height:26px;border-radius:50%}
.human .t{left:18px;top:30px;width:52px;height:65px;border-radius:26px 26px 16px 16px;
  background:linear-gradient(180deg,var(--fza),color-mix(in srgb,var(--fza) 40%,#111))}
.human .a{top:40px;width:19px;height:80px;border-radius:13px}
.human .al{left:0;transform:rotate(10deg)}
.human .ar{right:0;transform:rotate(-10deg)}
.human .l{top:100px;width:25px;height:82px;border-radius:16px;
  background:linear-gradient(180deg,var(--fza),#2d3240)}
.human .ll{left:18px}.human .lr{right:18px}
.bm-labels{position:absolute;inset:12px;
  display:flex;flex-direction:column;justify-content:space-between;font-size:11px}
.bm-row{display:flex;justify-content:space-between;color:#b0b8c8}
.bm-val{font-size:16px;font-weight:950;color:var(--fza)}
...
<div id="topbar">
  <div class="brand">
    ...
    <div class="brand-txt">
      ...
      <strong id="title-txt">Inicio</strong>
    </div>
  </div>
...
<div id="content">
  ...
  <div class="screen" id="s-activities">
    <div class="sh"><h3>Subir actividad</h3></div>
    <label class="upload-zone" id="drop-zone">
      <input type="file" accept=".fit,.zip" onchange="uploadFit(this.files[0])">
      <div class="ico">📤</div>
      <h3>Subir FIT / ZIP</h3>
      <p>Garmin Connect export · drag & drop</p>
    </label>
    <div id="upload-prog" style="display:none;margin-bottom:10px">
      <div style="font-size:12px;color:var(--m1);margin-bottom:5px" id="up-label">Procesando...</div>
      <div class="pbar"><div class="pfill" id="up-fill" style="width:0%"></div></div>
    </div>
    <div id="upload-result"></div>
    <div class="sh" style="margin-top:8px"><h3>Últimas actividades</h3><span id="act-count" style="font-size:12px;color:var(--m1)">—</span></div>
    <div class="card" style="padding:8px 14px">
      <div id="act-list"><div class="loading"><div class="spin"></div></div></div>
    </div>
  </div>
  ...
  <div class="screen" id="s-fuerza">
    <div class="bodymap">
      <div class="bm-labels">
        <div class="bm-row"><span>Quads <span class="bm-val" id="mq-q">—</span></span><span>Glutes <span class="bm-val" id="mq-g">—</span></span></div>
        <div class="bm-row"><span>Core <span class="bm-val" id="mq-c">—</span></span><span>Calves <span class="bm-val" id="mq-ca">—</span></span></div>
      </div>
      <div class="human"><div class="h"></div><div class="t"></div><div class="a al"></div><div class="a ar"></div><div class="l ll"></div><div class="l lr"></div></div>
    </div>
    <div class="card">
      <div class="sh"><h3>Registro rápido</h3><span>hoy</span></div>
      <div class="pill-row" id="fcat-pills">
        ...
        <div class="pill" onclick="setFcat('plyo',this)">Pliometría</div>
      ...
      <div id="f-compex-extra">
        ...
        <div class="field">
          <label>Intensidad — <span id="fv-int-lbl" style="color:var(--fza);font-size:16px;font-weight:950">58</span></label>
          <input type="range" min="1" max="120" value="58" id="fv-int" oninput="$('fv-int-lbl').textContent=this.value" style="accent-color:var(--fza);margin-top:6px">
        </div>
      </div>
      <div class="field"><label>Músculos</label>
        <div class="pill-row" id="fmuscle-pills" style="flex-wrap:wrap">
          <div class="pill" onclick="this.classList.toggle('on')">Cuádriceps</div>
          <div class="pill" onclick="this.classList.toggle('on')">Isquios</div>
          <div class="pill" onclick="this.classList.toggle('on')">Glúteos</div>
      ...
      <div class="g2">
        <div class="field"><label>Duración (min)</label><input type="number" id="fv-duration" placeholder="32"></div>
        <div class="field"><label>RPE</label><input type="number" id="fv-rpe" min="1" max="10" placeholder="7"></div>
      </div>
  ...
  <div class="screen" id="s-wellness">
    <div id="well-summary"></div>
    <div class="card">
      <div class="sh"><h3>Registro rápido</h3><span>hoy</span></div>
      <div class="pill-row" id="wcat-pills">
        <div class="pill on" onclick="setWcat('sleep',this)">😴 Sueño</div>
        ...
        <div class="pill" onclick="setWcat('pain',this)">⚠️ Molestia</div>
        <div class="pill" onclick="setWcat('stress',this)">🧠 Estrés</div>
      </div>
      <div id="well-fields">
        <div class="g2">
          <div class="field"><label>Horas sueño</label><input type="number" step="0.5" id="wf-sh" placeholder="7.5"></div>
...
<!-- Bottom nav -->
<nav id="bnav">
  <button class="nav active" data-s="home" onclick="go('home')"><span class="ico">⌂</span><span class="lbl">Inicio</span></button>
  <button class="nav" data-s="activities" onclick="go('activities')"><span class="ico">🚴</span><span class="lbl">Bici</span></button>
  <button class="nav" data-s="dashboard" onclick="go('dashboard')"><span class="ico">📊</span><span class="lbl">Stats</span></button>
  <button class="nav" data-s="gear" onclick="go('gear')"><span class="ico">⚙</span><span class="lbl">Gear</span></button>
  <button class="nav" data-s="calendar" onclick="go('calendar')"><span class="ico">📅</span><span class="lbl">Cal</span></button>
  <button class="nav" data-s="fuerza" onclick="go('fuerza')"><span class="ico">💪</span><span class="lbl">Fuerza</span></button>
  <button class="nav" data-s="wellness" onclick="go('wellness')"><span class="ico">🫀</span><span class="lbl">Wellness</span></button>
  <button class="nav" data-s="performance" onclick="go('performance')"><span class="ico">🏆</span><span class="lbl">Récords</span></button>
</nav>
...
const LABELS={home:'Inicio',activities:'Bici',dashboard:'Stats',gear:'Gear',calendar:'Calendario',performance:'Récords',fuerza:'Fuerza',wellness:'Wellness'};
let current='home', _wcat='sleep', _fcat='compex';
function $(id){return document.getElementById(id)}
function setTheme(s){
  const col=THEME[s]||'#fff';
  document.documentElement.style.setProperty('--c',col);
  $('kicker').textContent='Bitácora Mars';
  $('title-txt').textContent=LABELS[s]||s;
  $('logo-m').style.background=`linear-gradient(135deg,${col},color-mix(in srgb,${col} 45%,#000))`;
  $('logo-m').style.boxShadow=`0 0 24px color-mix(in srgb,${col} 32%,transparent)`;
}
function go(s){
  current=s;setTheme(s);
...
function toast(m,col){
  ...
  t.style.background=col||'var(--c)';t.style.display='block';
...
async function loadHome(){
  ...
  try{
    const [dash,well,wh]=await Promise.all([
      fetch(API+'/gpt/dashboard').then(r=>r.json()),
    ...
    const pains=well.molestias_activas||[];
    ...
    const wCurr=wh.current_kg;
    const wDelta=wh.delta_kg;
    // Carga badge
    const tsbLabel=c.tsb>5?'Óptimo':c.tsb<-15?'Fatigado':'En carga';
    // DOW sparkline
    const dow=(D.getDay()+6)%7; // 0=Lun
    const DAYS=['L','M','X','J','V','S','D'];
    const sparks=DAYS.map((_,i)=>{
      const isT=i===dow;
      const ht=isT?Math.min(100,Math.round(Number(s.km||0)*4)):Math.round(Math.random()*55+10);
      return `<div class="spark-col"><div class="spark-bar${isT?' today':''}" style="height:${ht}%"></div><div class="spark-day${isT?' today':''}">${DAYS[i]}</div></div>`;
    }).join('');
    el.innerHTML=`
      <div class="greeting-date">${dateStr}</div>
      <div class="greeting-name">${greet},<br>Mars 👋</div>

      <div class="carga-card">
        <div class="carga-label">Carga semanal</div>
        <div class="carga-sub">Esta semana</div>
        <div>
          <span class="carga-num">${km}</span>
          <span style="font-size:14px;color:var(--m1)"> km</span>
          <span class="carga-badge">${tsbLabel}</span>
        </div>
        <div class="sparkline-wrap">${sparks}</div>
      </div>
      ...
      <div class="card">
        <div class="sh"><h3>Estado actual</h3><span style="color:var(--c)">${a.fitness||'—'}</span></div>
        <div style="display:flex;gap:12px;align-items:center;padding:10px 0;border-bottom:1px solid var(--ln)">
          <div style="font-size:22px">⚡</div>
          <div style="flex:1"><div style="font-size:13px;font-weight:800">Fitness · Mars Index</div>
          <div style="font-size:11px;color:var(--m1);margin-top:2px">${dash.recommendation||'Sin recomendación'}</div></div>
          <div style="font-size:18px;font-weight:950;color:var(--c)">${a.mars_index?Number(a.mars_index).toFixed(1):'—'}</div>
        </div>
        <div style="display:flex;gap:12px;align-items:center;padding:10px 0;border-bottom:1px solid var(--ln)">
          <div style="font-size:22px">🧭</div>
          <div style="flex:1"><div style="font-size:13px;font-weight:800">Fatiga</div></div>
          <div style="font-size:14px;font-weight:800;color:${a.fatiga==='baja'?'#3dd68c':a.fatiga==='alta'?'var(--bike)':'var(--m1)'}">${a.fatiga||'—'}</div>
        </div>
        <div style="display:flex;gap:12px;align-items:center;padding:10px 0;border-bottom:1px solid var(--ln)">
          <div style="font-size:22px">🫀</div>
          <div style="flex:1"><div style="font-size:13px;font-weight:800">Molestias</div></div>
          <div style="font-size:14px;font-weight:800;color:${pains.length?'var(--bike)':'#3dd68c'}">${pains.length?pains.length+' activa(s)':'✓ Sin alertas'}</div>
        </div>
        ${wCurr?`<div style="display:flex;gap:12px;align-items:center;padding:10px 0">
          <div style="font-size:22px">⚖</div>
          <div style="flex:1"><div style="font-size:13px;font-weight:800">Peso</div>
          ${wDelta!=null?`<div style="font-size:11px;color:var(--m1)">${wDelta>0?'+':''}${wDelta} kg vs inicio</div>`:''}
          </div>
          <div style="font-size:18px;font-weight:950;color:#3dd68c">${wCurr} kg</div>
        </div>`:''}
      </div>

      <div class="sh"><h3>Zona 2 · objetivo 70-80%</h3><span style="color:var(--well);font-weight:800">${z2pct}%</span></div>
...
async function loadActs(){
  try{
    ...
    $('act-list').innerHTML=arr.length?arr.map(s=>`
      <div class="act-item" onclick="window.open('/charts/${s.session_id}','_blank')">
        <div class="act-ico">🚴</div>
        <div class="act-info">
          <div class="act-name">${s.workout_name||'Ciclismo'}</div>
          <div class="act-date">${fmtDate(s.start_time)}</div>
          <div class="act-stats">
            <div class="act-stat">⏱ <span>${hms(s.duration_s)}</span></div>
            <div class="act-stat">↑ <span>${s.ascent_m||0} m</span></div>
            <div class="act-stat">♥ <span>${s.avg_hr_bpm||'—'} bpm</span></div>
          </div>
        </div>
        <div class="act-km">${s.distance_km||'—'} km</div>
      </div>`).join(''):'<div style="color:var(--m1);padding:20px;text-align:center">Sin actividades</div>';
    // Drag & drop
    const dz=$('drop-zone');
    dz.addEventListener('dragover',e=>{e.preventDefault();dz.classList.add('drag')});
    dz.addEventListener('dragleave',()=>dz.classList.remove('drag'));
    dz.addEventListener('drop',e=>{e.preventDefault();dz.classList.remove('drag');const f=e.dataTransfer.files[0];if(f)uploadFit(f)});
...
async function uploadFit(file){
  ...
  $('up-label').textContent='Procesando '+file.name+'...';
  $('up-fill').style.width='30%';
  const fd=new FormData();fd.append('file',file);
  try{
    const d=await fetch(API+'/analyze-fit',{method:'POST',body:fd}).then(r=>r.json());
    $('up-fill').style.width='100%';
    setTimeout(()=>{$('upload-prog').style.display='none';showUploadResult(d)},400);
  }catch(e){$('upload-prog').style.display='none';toast('Error: '+e.message,'var(--bike)')}
}

function showUploadResult(d){
  const s=d.session||{},isDup=d.duplicate===true;
  $('upload-result').innerHTML=`
    <div class="card" style="${isDup?'border-color:rgba(242,166,35,.4)':'border-color:rgba(61,214,140,.3)'}">
      <div class="sh">
        <h3>${isDup?'⚠️ Ya registrada':'✅ Sesión guardada'}</h3>
        <span style="font-size:10px;font-family:monospace;color:var(--m1)">${d.session_id||''}</span>
      </div>
      <div class="g2" style="margin-bottom:10px">
        ...
        <div class="card-sm"><div class="kl">Duración</div><div style="font-size:14px;font-weight:800">${s.duration_hms||hms(s.duration_s)||'—'}</div></div>
  ...
  if(ach.length)setTimeout(()=>ach.forEach((a,i)=>setTimeout(()=>toast('🏆 '+a,'#f59e0b'),i*800)),500);
  loadActs();
}
...
async function loadDash(){
  ...
  try{
    ...
    const RDEFS=[
      {k:'max_distance',l:'Mayor distancia',ico:'🏁',col:'var(--bike)',f:v=>Number(v).toFixed(1)+' km'},
      {k:'max_ascent',l:'Mayor ascenso',ico:'⛰️',col:'#c8f135',f:v=>'+'+parseInt(v)+' m'},
      {k:'max_duration',l:'Sesión más larga',ico:'⏱️',col:'var(--well)',f:v=>hms(v)},
      {k:'max_speed',l:'Mayor velocidad',ico:'⚡',col:'var(--stats)',f:v=>Number(v).toFixed(1)+' km/h'},
      {k:'min_avg_hr',l:'FC mínima',ico:'❤️',col:'var(--bike)',f:v=>parseInt(v)+' bpm'},
    ];
    // Zone distribution
    const zd=await fetch(API+'/gpt/zones-summary?sport=cycling&weeks=4').then(r=>r.json()).catch(()=>({}));
    const zones=zd.zonas||{};
    const ZDEFS=[
      {k:'z5',l:'Z5 Máximo',col:'#c026d3'},
      {k:'z4',l:'Z4 Umbral',col:'var(--bike)'},
      {k:'z3',l:'Z3 Tempo',col:'#f59e0b'},
      {k:'z2',l:'Z2 Aeróbico',col:'#3dd68c'},
      {k:'z1',l:'Z1 Recuperación',col:'var(--well)'},
    ];
    el.innerHTML=`
      <div class="g2">
        ...
        <div class="card-sm"><div class="kl">Estado carga</div><div class="kv" style="font-size:18px;color:${tsbCol}">${c.estado||'—'}</div><div class="ku">TSB ${c.tsb||0}</div></div>
        <div class="card-sm"><div class="kl">Eficiencia 6m</div><div class="kv" style="font-size:20px">${eff.delta_pct_6_meses!=null?(eff.delta_pct_6_meses>=0?'+':'')+Number(eff.delta_pct_6_meses).toFixed(1)+'%':'—'}</div></div>
        <div class="card-sm"><div class="kl">Cadencia</div><div class="kv" style="font-size:20px">${d.cadencia_trend||'—'}</div></div>
      </div>
      <div class="card">
        <div class="sh"><h3>Zonas FC · 4 semanas</h3></div>
        ${ZDEFS.map(z=>{
          const pct=Number(zones[z.k+'_pct']||zones[z.k]||0).toFixed(0);
          const time=zones[z.k+'_time']||'';
          return `<div class="zone-row">
            <div class="zone-dot" style="background:${z.col}"></div>
            <div class="zone-lbl">${z.l}</div>
            <div class="zone-bar-wrap"><div class="zone-bar-fill" style="width:${pct}%;background:${z.col}"></div></div>
            <div class="zone-pct" style="color:${z.col}">${pct}%</div>
          </div>`;
        }).join('')}
      </div>
      <div class="card">
        <div class="sh"><h3>Récords personales</h3><span>ciclismo</span></div>
        ${RDEFS.map(rd=>{
          const v=r[rd.k];if(!v)return '';
          return `<div class="rec-row">
            <div class="rec-ico" style="background:color-mix(in srgb,${rd.col} 14%,#111)">${rd.ico}</div>
            <div style="flex:1"><div class="rec-lbl">${rd.l}</div><div class="rec-date">${fmtDate(v.date)}</div></div>
            <div class="rec-val" style="color:${rd.col}">${rd.f(v.value)}</div>
          </div>`;
        }).join('')}
      </div>`;
  }catch(e){el.innerHTML='<div class="card" style="color:var(--m1)">'+e.message+'</div>'}
...
async function loadGear(){
  ...
  try{
    ...
    let html='';
    if(alerts.length){
      html+=`<div class="card" style="border-color:rgba(232,89,60,.35);margin-bottom:12px">
        <div class="sh"><h3 style="color:var(--bike)">⚠️ Alertas</h3><span>${alerts.length}</span></div>
        ${alerts.map(a=>`<div class="gear-row"><div class="gear-ico">⚠️</div><div class="gear-info"><div class="gear-name">${a.name||a.type||'Componente'}</div><div class="gear-km">${a.message||''}</div></div><div class="gear-pct" style="color:var(--bike)">${a.pct_used||''}%</div></div>`).join('')}
      </div>`;
    }
    ...
    if(items.length){
      html+=items.map(g=>{
        ...
        return `<div class="gear-row">
          <div class="gear-ico">⚙</div>
          <div class="gear-info">
            <div class="gear-name">${g.name||g.type||'Componente'}</div>
            <div class="gear-bar-wrap"><div class="gear-bar-fill" style="width:${pct}%;background:${col}"></div></div>
            <div class="gear-km">${cur.toLocaleString()} / ${lim.toLocaleString()} km</div>
          </div>
          <div class="gear-pct" style="color:${col}">${pct}%</div>
        </div>`;
      }).join('');
    } else {
...
async function loadCal(){
  ...
  try{
    ...
    const total=recent.reduce((a,x)=>a+(x.count||x.sessions||0),0);
    const DOWS=['L','M','X','J','V','S','D'];
    el.innerHTML=`
      <div class="g2" style="margin-bottom:14px">
        <div class="card-sm"><div class="kl">Días activos</div><div class="kv" style="color:var(--cal)">${active}</div><div class="ku">6 semanas</div></div>
        <div class="card-sm"><div class="kl">Sesiones</div><div class="kv">${total}</div><div class="ku">total</div></div>
      </div>
      <div class="card">
        <div class="sh"><h3>Últimas 6 semanas</h3></div>
        <div style="display:grid;grid-template-columns:repeat(7,1fr);gap:3px;margin-bottom:4px">
          ${DOWS.map(d=>`<div class="cal-dow">${d}</div>`).join('')}
        </div>
        <div class="cal-grid">
          ${recent.map(x=>{
            const n=x.count||x.sessions||0;
            const cls=n===0?'':n===1?'h1':n===2?'h2':n===3?'h3':'h4';
            return `<div class="cal-day ${cls}">${(x.date||'').slice(8,10)||''}</div>`;
          }).join('')}
        </div>
        <div style="display:flex;align-items:center;gap:6px;margin-top:10px;font-size:11px;color:var(--m1)">
          <span>Intensidad:</span>
          ${['','h1','h2','h3','h4'].map(c=>`<div class="cal-day ${c}" style="width:16px;height:16px;border-radius:5px;flex-shrink:0"></div>`).join('')}
        </div>
      </div>`;
  }catch(e){el.innerHTML='<div class="card" style="color:var(--m1)">'+e.message+'</div>'}
}
...
async function loadPerf(){
  ...
  try{
    ...
    el.innerHTML=`
      <div class="g2">
        <div class="card-sm"><div class="kl">ATL agudo</div><div class="kv" style="font-size:22px">${c.atl||'—'}</div></div>
        <div class="card-sm"><div class="kl">CTL crónico</div><div class="kv" style="font-size:22px">${c.ctl||'—'}</div></div>
        <div class="card-sm"><div class="kl">TSB balance</div><div class="kv" style="color:${tsbCol};font-size:22px">${c.tsb||0}</div></div>
        <div class="card-sm"><div class="kl">Estado</div><div class="kv" style="font-size:16px;color:${tsbCol}">${c.estado||'—'}</div></div>
      </div>`;
  }catch(e){el.innerHTML='<div class="card" style="color:var(--m1)">'+e.message+'</div>'}
}
...
async function loadFuerza(){
  try{
    const d=await fetch(API+'/gpt/fuerza-summary?weeks=8').then(r=>r.json());
    const p=d.compex_progresion||{};
    function last(m){const a=p[m]||[];return a.length?a[a.length-1].max_intensity:'—'}
    $('mq-q').textContent=last('quadriceps');$('mq-g').textContent=last('glutes');
    $('mq-c').textContent=last('core');$('mq-ca').textContent=last('calves');
    $('fuerza-data').innerHTML=`<div class="g2">
      <div class="card-sm"><div class="kl">Sesiones 8s</div><div class="kv" style="color:var(--fza)">${d.total_sesiones||0}</div></div>
      <div class="card-sm"><div class="kl">Horas total</div><div class="kv" style="color:var(--fza)">${d.total_horas||0}</div></div>
    </div>`;
  }catch(e){}
}
async function saveFuerza(){
  ...
  const body={date:today,category:_fcat,
...
// ── WELLNESS ──────────────────────────────────────────────────────────────────
const WFIELDS={
  sleep:`<div class="g2"><div class="field"><label>Horas sueño</label><input type="number" step="0.5" id="wf-sh" placeholder="7.5"></div><div class="field"><label>Calidad</label><select id="wf-sq"><option value="">—</option><option value="bueno">Bueno</option><option value="regular">Regular</option><option value="malo">Malo</option></select></div></div><div class="g2"><div class="field"><label>FC reposo</label><input type="number" id="wf-hr" placeholder="52"></div><div class="field"><label>Score Garmin</label><input type="number" id="wf-gs" placeholder="78"></div></div>`,
  compex_recovery:`<div class="g2"><div class="field"><label>Programa</label><select id="wf-prog"><option value="active_recovery">Active Recovery</option><option value="recovery">Recovery</option><option value="massage">Massage</option></select></div><div class="field"><label>Duración (min)</label><input type="number" id="wf-dur" placeholder="20"></div></div>`,
  massage_gun:`<div class="field"><label>Duración (min)</label><input type="number" id="wf-dur" placeholder="15"></div>`,
  ceragem:`<div class="g2"><div class="field"><label>Duración (min)</label><input type="number" id="wf-dur" placeholder="40"></div><div class="field"><label>Sensación antes</label><input type="number" id="wf-sb" min="1" max="10" placeholder="6"></div></div><div class="field"><label>Sensación después</label><input type="number" id="wf-sa" min="1" max="10" placeholder="8"></div>`,
  pain:`<div class="g2"><div class="field"><label>Zona</label><input id="wf-pz" placeholder="Isquiotibial izq."></div><div class="field"><label>Nivel (1-10)</label><input type="number" id="wf-pl" min="1" max="10" placeholder="4"></div></div>`,
  stress:`<div class="g2"><div class="field"><label>Nivel (1-10)</label><input type="number" id="wf-sl" min="1" max="10" placeholder="6"></div><div class="field"><label>Causa</label><select id="wf-sc"><option value="">—</option><option value="work">Trabajo</option><option value="fatigue">Fatiga</option><option value="illness">Enfermedad</option></select></div></div>`,
};
...
async function loadWell(){
  try{
    ...
    $('well-summary').innerHTML=`
      <div class="card" style="margin-bottom:12px">
        <div class="well-ring-wrap">
          <div class="well-ring" style="--p:${score}%"><span>${score}</span></div>
          <div class="well-status"><strong>Estado general</strong>
            ${score>=80?'Listo para entrenar fuerte.':score>=60?'Recuperación normal.':'Prioriza descanso.'}
          </div>
        </div>
        <div class="g3">
          <div class="card-sm"><div class="kl">Sueño</div><div class="kv" style="font-size:20px;color:var(--well)">${sleep}<span style="font-size:11px;color:var(--m1)"> h</span></div></div>
          <div class="card-sm"><div class="kl">FC reposo</div><div class="kv" style="font-size:20px">${fc}<span style="font-size:11px;color:var(--m1)"> bpm</span></div></div>
          <div class="card-sm"><div class="kl">Estrés</div><div class="kv" style="font-size:20px;color:${Number(stress)>6?'var(--bike)':'#3dd68c'}">${stress}<span style="font-size:11px;color:var(--m1)">/10</span></div></div>
        </div>
        ${pains.length?`<div style="border-top:1px solid var(--ln);padding-top:10px">
          <div class="sh"><h3 style="color:var(--bike)">⚠️ Molestias activas</h3><span>${pains.length}</span></div>
          ${pains.map(p=>`<div style="display:flex;justify-content:space-between;padding:6px 0;font-size:13px;border-bottom:1px solid var(--ln)"><span>${p.pain_zone||'—'}</span><span style="color:var(--bike);font-weight:800">Nivel ${p.pain_level||'?'}/10</span></div>`).join('')}
        </div>`:'<div style="color:#3dd68c;font-size:12px;text-align:center;padding:8px 0;font-weight:700">✓ Sin molestias activas</div>'}
      </div>`;
  }catch(e){$('well-summary').innerHTML=''}
}
async function saveWellness(){
  const today=new Date().toISOString().slice(0,10);
  const body={date:today,category:_wcat,
    fatigue:parseInt($('wf-fat')?.value)||null,
    notes:$('wf-notes')?.value||null};
