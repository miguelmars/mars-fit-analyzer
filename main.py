...
# ═══════════════════════════════════════════════════════════════════════════════
# ETAPA 2 — Endpoints Home: stats/monthly, stats/efficiency, sessions recientes
# ═══════════════════════════════════════════════════════════════════════════════

@app.get("/stats/monthly")
def stats_monthly(year: int = None, month: int = None, sport: Optional[str] = None):
    """
    Stats del mes para el banner Home.
    Devuelve: km totales, horas totales, FC promedio, número de sesiones.
    """
    conn = get_db()
    if not conn:
        raise HTTPException(503, "DB no disponible")
...
def stats_efficiency(weeks: int = 8, sport: str = "cycling"):
    """
    Eficiencia aeróbica por semana — velocidad promedio / FC promedio.
    Usado para la gráfica de barras en Home.
    """
    conn = get_db()
    if not conn:
        raise HTTPException(503, "DB no disponible")
        ...
        with conn.cursor() as cur:
            cur.execute("""
                ...
                WHERE sport = %s
                  ...
                  AND start_time::timestamp >= NOW() - INTERVAL '%s weeks'
                GROUP BY semana
                ORDER BY semana ASC
            """, (sport, weeks))
            rows = cur.fetchall()
...
def sessions_recent(limit: int = 1, sport: Optional[str] = None):
    """
    Últimas N sesiones — para la card de última actividad en Home.
    """
    conn = get_db()
    if not conn:
        raise HTTPException(503, "DB no disponible")
