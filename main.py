...
def service_worker():
    ...
    sw_code = """const CACHE='bitacora-v11';
...
(async function(){{
  try{{
    ...
    const a = await fetch('/gpt/session-analysis/'+sid).then(r=>r.json());
    if(a && a.analysis){{
      const an = a.analysis;
      document.getElementById('marsAnalysis').innerHTML =
        '<div style="display:flex;flex-direction:column;gap:10px">'+
        '<div style="padding:10px;background:rgba(61,214,140,.08);border-left:3px solid #3dd68c;border-radius:6px"><b style="color:#3dd68c">Zona:</b> '+an.zone+'</div>'+
        '<div style="padding:10px;background:rgba(74,158,255,.08);border-left:3px solid #4a9eff;border-radius:6px"><b style="color:#4a9eff">Cadencia:</b> '+an.cadence+'</div>'+
        '<div style="padding:10px;background:rgba(167,139,250,.08);border-left:3px solid #a78bfa;border-radius:6px"><b style="color:#a78bfa">Eficiencia:</b> '+an.efficiency+'</div>'+
        '<div style="padding:10px;background:rgba(232,89,60,.08);border-left:3px solid #e8593c;border-radius:6px"><b style="color:#e8593c">vs Historico:</b> '+an.comparison+'</div>'+
        '</div>';
    }} else {{
      document.getElementById('marsAnalysis').textContent = 'Analisis no disponible';
    }}
  }}catch(e){{
    document.getElementById('marsAnalysis').textContent = 'Analisis no disponible';
  }}
  ...
  try{
    ...
    $('home-data').innerHTML=
      ...
      '<div class="card"><div class="head"><h3>Estado del atleta</h3><span>'+(a.fitness||'—')+'</span></div>'+
        ...
        (function(){
          const wc=wh.current_kg, wd=wh.delta_kg;
          if(!wc)return '';
          const vals=(wh.historial||[]).slice(-6).map(e=>e.weight_kg).filter(Boolean);
          let sp='';
          if(vals.length>=2){
            const mx=Math.max(...vals),mn=Math.min(...vals),rng=mx-mn||0.1,W=50,H=16;
            const pts=vals.map((v,i)=>Math.round(i/(vals.length-1)*W)+','+Math.round(H-(v-mn)/rng*(H-3)-1)).join(' ');
            sp='<svg viewBox="0 0 '+W+' '+H+'" style="width:50px;height:16px;display:inline-block;vertical-align:middle;margin-left:6px"><polyline points="'+pts+'" fill="none" stroke="#3dd68c" stroke-width="1.8"/></svg>';
          }
          const arrow = wd!=null ? (wd<0?' ↓':wd>0?' ↑':'') : '';
          return '<div class="row"><div class="r-ico">·</div><div class="r-main"><div class="r-title">Peso</div><div class="r-sub">objetivo 80 kg'+(wc>80?' · faltan '+(wc-80).toFixed(1)+' kg':'')+'</div></div><div class="r-val" style="color:#3dd68c">'+wc+' kg'+arrow+sp+'</div></div>';
        })()+
      '</div>';
  }catch(e){
...
async async function loadActs(){
  try{
    ...
    $('act-list').innerHTML=arr.map(s=>'<div class="row" style="align-items:flex-start">'+
      ...
      '</div>'+
      '<div class="r-main" style="flex:1;min-width:0;cursor:pointer" onclick="window.open('/charts/'+s.session_id+'','_blank')">'+
      ...
      '<div style="display:flex;flex-direction:column;align-items:flex-end;gap:6px;flex-shrink:0">'+
        ...
        '<button onclick="deleteSession(''+s.session_id+'')" style="background:rgba(232,89,60,.1);border:none;border-radius:8px;padding:4px 8px;color:#e8593c;font-size:10px;font-weight:800;cursor:pointer">borrar</button>'+
      '</div>'+
    '</div>').join('');
...
async async function loadFuerza(){
  try{
