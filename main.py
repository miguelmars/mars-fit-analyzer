...
async function loadRecent(){
  ...
  try{
    ...
    el.innerHTML='<div class="card">'+sess.slice(0,5).map(s=>`<div class="act-row" onclick="location.href='/session/${s.session_id}'"><div class="act-icon" style="background:rgba(232,89,60,.15)">🚴</div><div class="act-info"><div class="act-name">${s.workout_name||'Sesión ciclismo'}</div><div class="act-meta">${(s.start_time||'') · ${s.duration_hms||'—'</div></div><div style="text-align:right"><div class="act-km" style="color:var(--bike)">${s.distance_km||'—'} km</div><div class="act-hr">${s.avg_hr_bpm||'—'} bpm</div></div></div>`).join('')+'</div>';
