...
<body>
<div class="app">
  <div class="top"><button class="icon back-icon" id="backBtn" onclick="appBack()" aria-label="Volver">‹</button><div class="brand"><div class="logo">M</div><div><small id="kicker">Bitácora Mars</small><strong id="title">Home</strong></div></div><button class="icon" id="refreshBtn" onclick="refresh()" aria-label="Actualizar">↻</button></div>
  <main class="content">
...
async function refresh(){
  const b=$('refreshBtn');
  if(b){b.disabled=true;b.style.transform='rotate(180deg)';b.style.opacity='.7'}
  try{await load(current);toast('Actualizado')}
  catch(e){toast('No se pudo actualizar')}
  finally{if(b){setTimeout(()=>{b.disabled=false;b.style.transform='';b.style.opacity=''},220)}}
}
