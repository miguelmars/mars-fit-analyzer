...
def service_worker():
    ...
    sw_code = """const CACHE='bitacora-v9';
...
async function loadHome(){
  // Fetch all in parallel — each with individual catch so one failure doesn't kill all
  ...
  try{
    ...
    $('home-data').innerHTML=
      ...
      '<div class="card"><div class="head"><h3>Estado del atleta</h3><span>'+( a.fitness||'—')+'</span></div>'+
        ...
        row('·','Fatiga',a.fatiga||'—','')+
        row('·','Molestias',pains?'Revisar Wellness':'Sin alertas',pains||'ok')+
      '</div>';
  }catch(e){
    $('home-data').innerHTML='<div class="card" style="color:var(--muted)">Error: '+e.message+'</div>';
  }
}
