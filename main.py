...
function date(v){return v?fmtDate(v):'—'}
function fmtDate(v){
  if(!v)return '—';
  try{
...
function fmtShort(v){
  ...
  try{
    ...
    const M=['ene','feb','mar','abr','may','jun','jul','ago','sep','oct','nov','dic'];
    return d.getDate()+' '+M[d.getMonth()];
  }catch(e){return (v+'').slice(0,10).slice(5);}
}
