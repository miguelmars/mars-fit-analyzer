...
try:
    ...
    import logging
    logging.getLogger("bitacora").warning(f"Strava router no cargado: {_strava_err}")
...
<body>
<div class="app">
  <div class="top"><button class="icon" id="backBtn" onclick="appBack()" aria-label="Volver">‹</button><div class="brand"><div class="logo">M</div><div><small id="kicker">Bitácora Mars</small><strong id="title">Home</strong></div></div></div>
  <main class="content">
    ...
    <section class="screen" id="s-activities"><div class="hero"><div class="kicker">Actividades / historia</div><h1>Sesiones<br>recientes.</h1><p>Toda tu data viva: bici, correr, caminatas, fuerza y Garmin export.</p></div><label class="upload"><input type="file" accept=".fit,.zip" onchange="uploadFit(this.files[0])"><div class="big">↑</div><h3>Subir FIT / ZIP</h3><p>Garmin Connect export</p></label><div id="upload-result"></div><div class="card"><div class="head"><h3>Últimas actividades</h3><span id="act-count">—</span></div><div id="act-list" class="loading"><span class="spin"></span>Cargando...</div></div></section>
  ...
  <option value="otro">Otro</option>
    ...
    <section class="screen" id="s-capacidades">
      <div class="hero"><div class="kicker">Capacidades</div><h1>Tu cuerpo,<br>en seis sistemas.</h1><p>Desarrollo, confianza, limitantes y referencias construidas con tu propia historia.</p></div>
      <div id="cap-data"><div class="loading"><span class="spin"></span>Cargando...</div></div>
    </section>
    ...
    <section class="screen" id="s-progress">
      ...
      <div class="tabs" id="progress-tabs">
        <button class="tab on" data-mode="general" onclick="setProgressMode('general')">General</button>
        <button class="tab" data-mode="cycling" onclick="setProgressMode('cycling')">Bici</button>
        <button class="tab" data-mode="running" onclick="setProgressMode('running')">Correr</button>
      </div>
      <button class="btn btn2" style="margin-bottom:12px" onclick="go('capacidades')">Ver capacidades atléticas</button>
...
const THEME={home:'#c8cbd2',perfil:'#3dd68c',coach:'#4a9eff',progress:'#a78bfa',capacidades:'#22d3ee',dashboard:'#e8593c',activities:'#e8593c',gear:'#f59e0b',calendar:'#22d3ee',performance:'#a78bfa',fuerza:'#c8f135',wellness:'#4a9eff',eficiencia:'#3dd68c',correlaciones:'#a78bfa',nutricion:'#f59e0b'};
const TITLE={home:['Bitácora Mars','Home'],perfil:['Perfil Mars','atleta'],coach:['Coach','recomendacion'],progress:['Historia','atlética'],capacidades:['Capacidades','atléticas'],dashboard:['Dashboard','stats'],activities:['Data','sesiones'],gear:['Gear','mantenimiento'],calendar:['Calendario','heatmap'],performance:['Récords','personales'],fuerza:['Fuerza','Compex'],wellness:['Wellness','recuperación'],eficiencia:['Eficiencia','aeróbica'],correlaciones:['Correlaciones','FC · Peso'],nutricion:['Nutrición','geles']};
let current='home';
let navStack=[];
...
async function loadActs(){
  try{
    ...
    const onlyTelemetry=params.get('telemetry')==='1';
    const sport=params.get('sport')||'all';
    const sportParam=sport==='all'?'':('&sport='+encodeURIComponent(sport));
    const url=API+'/sessions?limit=20'+sportParam+(onlyTelemetry?'&telemetry=true':'');
    const d=await fetch(url).then(r=>r.json());
    const arr=d.sessions||d||[];
    const total=Number(d.total ?? arr.length);
    $('act-count').textContent=onlyTelemetry?total+' con curvas':total;
    const showingHtml=onlyTelemetry&&total>arr.length?'<div style="color:var(--muted);font-size:12px;font-weight:800;padding:0 0 10px;text-align:right">Mostrando '+arr.length+' de '+total+'</div>':'';
    const activeSport={bike:['bike','cycling','indoor_cycling'],run:['run','running','trail_running','treadmill_running'],strength:['strength','fuerza','strength_training'],swim:['swim','swimming','lap_swimming','open_water_swimming'],walking:['walking','walk'],cardio:['cardio','indoor_cardio']};
    function isSport(k){return (activeSport[k]||[k]).indexOf(sport)>=0}
    const filterHtml='<div style="display:flex;gap:8px;flex-wrap:wrap;padding:0 0 12px">'+
      '<button onclick="window.location.href=\x27/activities\x27" style="background:'+(sport==='all'&&!onlyTelemetry?'rgba(232,89,60,.18)':'rgba(255,255,255,.06)')+';border:1px solid rgba(255,255,255,.1);border-radius:10px;padding:8px 10px;color:var(--text);font-weight:900;cursor:pointer">Todo</button>'+
      '<button onclick="window.location.href=\x27/activities?sport=bike\x27" style="background:'+(isSport('bike')&&!onlyTelemetry?'rgba(232,89,60,.18)':'rgba(255,255,255,.06)')+';border:1px solid rgba(255,255,255,.1);border-radius:10px;padding:8px 10px;color:var(--text);font-weight:900;cursor:pointer">Bici</button>'+
      '<button onclick="window.location.href=\x27/activities?sport=run\x27" style="background:'+(isSport('run')&&!onlyTelemetry?'rgba(61,214,140,.18)':'rgba(255,255,255,.06)')+';border:1px solid rgba(255,255,255,.1);border-radius:10px;padding:8px 10px;color:var(--text);font-weight:900;cursor:pointer">Correr</button>'+
      '<button onclick="window.location.href=\x27/activities?sport=strength\x27" style="background:'+(isSport('strength')&&!onlyTelemetry?'rgba(200,241,53,.18)':'rgba(255,255,255,.06)')+';border:1px solid rgba(255,255,255,.1);border-radius:10px;padding:8px 10px;color:var(--text);font-weight:900;cursor:pointer">Fuerza</button>'+
      '<button onclick="window.location.href=\x27/activities?sport=swim\x27" style="background:'+(isSport('swim')&&!onlyTelemetry?'rgba(56,189,248,.18)':'rgba(255,255,255,.06)')+';border:1px solid rgba(255,255,255,.1);border-radius:10px;padding:8px 10px;color:var(--text);font-weight:900;cursor:pointer">Nadar</button>'+
      '<button onclick="window.location.href=\x27/activities?sport=walking\x27" style="background:'+(isSport('walking')&&!onlyTelemetry?'rgba(34,211,238,.18)':'rgba(255,255,255,.06)')+';border:1px solid rgba(255,255,255,.1);border-radius:10px;padding:8px 10px;color:var(--text);font-weight:900;cursor:pointer">Caminar</button>'+
      '<button onclick="window.location.href=\x27/activities?sport=cardio\x27" style="background:'+(isSport('cardio')&&!onlyTelemetry?'rgba(74,158,255,.18)':'rgba(255,255,255,.06)')+';border:1px solid rgba(255,255,255,.1);border-radius:10px;padding:8px 10px;color:var(--text);font-weight:900;cursor:pointer">Cardio</button>'+
      '<button onclick="window.location.href=\x27/activities?telemetry=1'+(sport!=='all'?'&sport='+sport:'')+'\x27" style="background:'+(onlyTelemetry?'rgba(232,89,60,.18)':'rgba(255,255,255,.06)')+';border:1px solid rgba(255,255,255,.1);border-radius:10px;padding:8px 10px;color:var(--text);font-weight:900;cursor:pointer">Con curvas/mapa</button>'+
    '</div>'+showingHtml;
    if(!arr.length){
      const msg=onlyTelemetry?'No encontré actividades con curvas/mapa en este bloque. Cuando una sesión viene solo del resumen Garmin, faltan los datos segundo a segundo del FIT/TCX original.':'Sin actividades';
      $('act-list').innerHTML=filterHtml+'<div style="color:var(--muted);padding:20px;text-align:center">'+msg+'</div>';
      return;
    }
    $('act-list').innerHTML=filterHtml+arr.map(function(s){
      return '<div class="row" style="align-items:flex-start">'+
        ...
        '</div>'+
        '<div class="r-main" style="flex:1;min-width:0;cursor:pointer" onclick="window.location.href=\x27/session/'+s.session_id+'\x27">'+
          ...
          '<div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:3px;font-size:11px;color:var(--muted)">'+
            ...
            '<span>'+(s.calories?Math.round(s.calories)+' kcal':'sin kcal')+'</span>'+
            '<span>'+(s.has_telemetry?'curvas/mapa':'sin telemetría')+'</span>'+
          '</div>'+
        '</div>'+
        '<div style="display:flex;flex-direction:column;align-items:flex-end;gap:6px;flex-shrink:0">'+
          '<div style="font-size:17px;font-weight:950;color:var(--theme)">'+(s.distance_km||'--')+' km</div>'+
          '<div style="display:flex;gap:4px;flex-wrap:wrap;justify-content:flex-end">'+
            '<button onclick="event.stopPropagation();window.location.href=\x27/session/'+s.session_id+'\x27" style="background:rgba(255,255,255,.08);border:none;border-radius:8px;padding:4px 8px;color:var(--text);font-size:10px;font-weight:800;cursor:pointer">ver</button>'+
...
async function loadCapacidades(){
  ...
  try{
    const responses=await Promise.all([
      fetch(API+'/gpt/capacidades'),
      fetch(API+'/gpt/capacidad/motor_aerobico/history'),
      fetch(API+'/gpt/capacidad/composicion_corporal/history'),
      fetch(API+'/gpt/capacidad/escalada/history'),
      fetch(API+'/gpt/readiness?evento=escalera_al_infierno'),
      fetch(API+'/gpt/readiness?evento=gran_fondo_150'),
      fetch(API+'/gpt/readiness?evento=medio_maraton'),
      fetch(API+'/gpt/academia/glosario'),
      fetch(API+'/gpt/patron-historico')
    ]);
    if(!responses[0].ok)throw new Error('No se pudo calcular capacidades');
    const d=await responses[0].json();
    const history=responses[1].ok?await responses[1].json():{};
    const weightHistory=responses[2].ok?await responses[2].json():{};
    const climbHistory=responses[3].ok?await responses[3].json():{};
    const readiness=[
      responses[4].ok?await responses[4].json():{},
      responses[5].ok?await responses[5].json():{},
      responses[6].ok?await responses[6].json():{}
    ];
    const glosario=responses[7].ok?(await responses[7].json()).glosario||{}:{};
    const patronData=responses[8].ok?await responses[8].json():{};
    ...
    function bodyCompositionHistoryView(){
      const years=weightHistory.years||[];
      if(!years.length)return '';
      ...
      const rows=years.map(function(y){
        const preliminary=y.best_period_tag==='historia_preliminar';
        const noData=y.best_period_tag==='sin_datos_suficientes';
        const bestYear=y.best_period_tag==='mejor_epoca_historica';
        const color=bestYear?'#3dd68c':noData?'#8b929f':'#f59e0b';
        const score=y.score==null?'--':Number(y.score).toFixed(1);
        const minW=y.block_min_weight_kg!=null?y.block_min_weight_kg+' kg':'sin dato';
        const sub=noData?'sin datos suficientes':preliminary?'preliminar · madurez '+Number(y.maturity||0).toFixed(0)+'%':'mín '+minW+' · '+(y.measured_weeks||0)+' semanas';
        const opacity=preliminary||noData?'.52':'1';
    ...
    function climbingHistoryView(){
      const years=climbHistory.years||[];
      if(!years.length)return '';
      ...
      const rows=years.map(function(y){
        ...
        const noData=y.best_period_tag==='sin_datos_suficientes';
        const bestYear=y.best_period_tag==='mejor_epoca_historica';
        const color=bestYear?'#3dd68c':noData?'#8b929f':y.score>=90?'#22d3ee':'#4a9eff';
        const score=y.score==null?'--':Number(y.score).toFixed(1);
        ...
        const sub=noData?'sin datos suficientes':preliminary?'preliminar · madurez '+Number(y.maturity||0).toFixed(0)+'%':avgM+big;
        const opacity=preliminary||noData?'.52':'1';
        return '<div style=”display:grid;grid-template-columns:38px minmax(0,1fr) 42px;gap:9px;align-items:center;padding:8px 0;border-bottom:1px solid var(--line);opacity:'+opacity+'”>'+
          '<div style=”font-size:12px;font-weight:900”>'+y.year+'</div>'+
          '<div style=”min-width:0”><div style=”height:7px;background:rgba(255,255,255,.07);overflow:hidden;border-radius:4px”><div style=”height:100%;width:'+pct(y.score)+'%;background:'+color+'”></div></div><div style=”font-size:10px;color:var(--muted);margin-top:4px;white-space:normal”>'+sub+'</div></div>'+
      ...
      return '<div class=”card” style=”border-left:3px solid #22d3ee”>'+
        '<div class=”head”><h3>Historia de Escalada</h3><span>bloques de 12 semanas</span></div>'+
        '<div class=”grid2”>'+
          metric('Hoy oficial',official.score!=null?Number(official.score).toFixed(1):'--','validación aceptada')+
          metric('Mejor época',best.score!=null?Number(best.score).toFixed(1):'--',best.year||'sin referencia')+
    ...
    function capCard(c){
      ...
      return '<div class="card" style="border-left:3px solid '+color+'">'+
        '<div class="head"><h3>'+c.nombre+'</h3><span style="color:'+color+'">'+(c.status==='calculated'?'calculada':'por calibrar')+'</span></div>'+
        '<div class="grid2">'+
    ...
    el.innerHTML=
      '<div class="grid2">'+
        metric('Calculadas',calculated,'de 6 capacidades')+
      ...
      '</div>'+
      '<div class="card"><div class="head"><h3>Lectura honesta</h3><span>score ≠ confianza</span></div><div style="font-size:12px;line-height:1.55;color:var(--muted)">Un score mide desarrollo. La confianza mide qué tan bien respaldado está. Cuando faltan datos, Mars no convierte el vacío en una mala calificación.</div></div>'+
      readinessView()+
      patronHistoricoView()+
      historyView()+
      bodyCompositionHistoryView()+
      climbingHistoryView()+
      items.map(capCard).join('');
  }catch(e){
...
async function loadProgress(mode){
  ...
  try{
    ...
    const [hist,mp,tr,perf,wh,corr,tend,athletic]=await Promise.all([
      fetch(API+'/gpt/athletic-history').then(r=>r.json()).catch(()=>({})),
      fetch(API+'/gpt/mars-context').then(r=>r.json()).catch(()=>({})),
      fetch(API+'/gpt/trends?weeks=8').then(r=>r.json()).catch(()=>({})),
      fetch(API+'/gpt/performance-profile?sport='+sport).then(r=>r.json()).catch(()=>({})),
      fetch(API+'/weight/history?limit=10').then(r=>r.json()).catch(()=>({})),
      mode==='general'?Promise.resolve({}):fetch(API+'/gpt/correlaciones?sport='+sport).then(r=>r.json()).catch(()=>({})),
      mode==='general'?Promise.resolve({}):fetch(API+'/gpt/tendencia?sport='+sport).then(r=>r.json()).catch(()=>({})),
      fetch(API+'/gpt/athletic-status').then(r=>r.json()).catch(()=>({})
      )
    ]);
    ...
    function groupName(g){return {running:'Correr',cycling:'Bici',indoor_cardio:'Cardio indoor',walking:'Caminata',strength:'Fuerza',swimming:'Nadar',mobility:'Movilidad',multi_sport:'Multi-sport',other:'Otros'}[g]||g}
    function groupColor(g){return {running:'#3dd68c',cycling:'#e8593c',indoor_cardio:'#4a9eff',walking:'#22d3ee',strength:'#c8f135',swimming:'#38bdf8',mobility:'#f59e0b',multi_sport:'#a78bfa',other:'#8b929f'}[g]||'#8b929f'}
    function num(v,d){return Number(v||0).toLocaleString('es-MX',{maximumFractionDigits:d||0})}
    const years=[...new Set(yearly.map(function(x){return x.year}))];
    const maxYearKm=Math.max(1,...years.map(function(y){return yearly.filter(function(x){return x.year===y}).reduce(function(a,b){return a+Number(b.km||0)},0)}));
    const timelineHtml=years.map(function(y){
      const rows=yearly.filter(function(x){return x.year===y});
      const bars=rows.map(function(x){
        const w=Math.max(2,Math.round(Number(x.km||0)/maxYearKm*100));
        return '<div title="'+groupName(x.grupo)+' '+num(x.km,1)+' km" style="height:10px;width:'+w+'%;background:'+groupColor(x.grupo)+';border-radius:999px;margin:3px 0"></div>';
      }).join('');
      const km=rows.reduce(function(a,b){return a+Number(b.km||0)},0);
      return '<div style="display:grid;grid-template-columns:44px 1fr 54px;gap:8px;align-items:center;margin:9px 0"><div style="font-weight:950">'+y+'</div><div>'+bars+'</div><div style="text-align:right;color:var(--muted);font-size:11px">'+num(km,0)+' km</div></div>';
    }).join('');
    const groupsHtml=groups.map(function(g){
      return '<div class="row"><div class="r-main"><div class="r-title">'+groupName(g.grupo)+'</div><div class="r-sub">'+num(g.sesiones)+' sesiones · '+num(g.horas,1)+' h · FC '+(g.fc_promedio||'--')+'</div></div><div class="r-val" style="color:'+groupColor(g.grupo)+'">'+num(g.km,1)+' km</div></div>';
    }).join('');
    ...
    el.innerHTML=
      ...
      '<div class="card"><div class="head"><h3>Historia viva</h3><span>'+fmtDate(totals.desde)+' → '+fmtDate(totals.hasta)+'</span></div><div class="grid2">'+
        metric('Sesiones',num(totals.sesiones),'clean_sessions')+
        metric('Con FC',num(totals.con_fc),'motor aeróbico')+
        metric('Distancia',num(totals.km,0)+' km','histórico')+
        metric('Calorías',num(totals.calorias,0)+' kcal','Garmin export')+
      '</div></div>'+
      '<div class="card"><div class="head"><h3>Mapa por deporte</h3><span>arco atlético completo</span></div>'+
        '<div class="grid2">'+
          metric('Correr',num(running.sesiones)+' sesiones',num(running.km,0)+' km · FC '+(running.fc_promedio||'--'))+
          metric('Bici',num(cycling.sesiones)+' sesiones',num(cycling.km,0)+' km · FC '+(cycling.fc_promedio||'--'))+
          metric('Fuerza',num(strength.sesiones)+' sesiones',num(strength.horas,1)+' h · FC '+(strength.fc_promedio||'--'))+
          metric('Nadar',num(swimming.sesiones)+' sesiones',num(swimming.km,1)+' km · '+num(swimming.horas,1)+' h')+
        '</div></div>'+
      '<div class="card"><div class="head"><h3>Línea de tiempo</h3><span>2018 → hoy</span></div>'+timelineHtml+'</div>'+
      '<div class="card"><div class="head"><h3>Disciplinas vivas</h3><span>'+groups.length+'</span></div>'+groupsHtml+'</div>'+
