...
Endpoints:
  ...
  GET  /result/{session_id}  → el GPT consulta el resultado por ID (sin tocar archivos)
  GET  /                     → health check
"""
...
# Almacén en memoria: {session_id: {result, uploaded_at, filename}}
# Se limpia automáticamente al reiniciar Railway — suficiente para el flujo de uso
RESULTS_STORE: dict = {}
...
def summarize_records(records):
    ...
    return {
        ...
        "speed": {
            "min_kmh": round(min(spds), 1) if spds else None,
...
def compute_zones(records):
    ...
    zones.append({
        "zone": 0, "name": "Entre Z1 y Z2 oficial",
        "bpm_low": 109, "bpm_high": 133,
        "seconds": gap_count,
...
def root():
    return {
        ...
        "endpoints": {
            "upload":  "POST /analyze-fit  → sube ZIP/FIT, recibe session_id",
            "query":   "GET  /result/{session_id}  → el GPT consulta el resultado",
        }
    }
...
async def analyze_fit(
    ...
    """
    Tú llamas esto con curl o el script de subida.
    Devuelve el JSON completo + un session_id para que el GPT lo consulte.
    """
    ...
    # Guardar en store con ID único
    sid = str(uuid.uuid4())[:8]   # 8 chars: fácil de copiar/pegar al GPT
    RESULTS_STORE[sid] = {
        "session_id":   sid,
    ...
    # Devolver el JSON completo + el session_id
    return {
        "session_id": sid,
        "message":    f"Resultado guardado. Pasa el session_id '{sid}' al GPT para que lo analice.",
        **result,
    }
...
def get_result(session_id: str):
    """
    El GPT llama esto con el session_id que le pasaste.
    No recibe archivos — solo devuelve el JSON ya procesado.
    """
    entry = RESULTS_STORE.get(session_id)
    if not entry:
        raise HTTPException(
            status_code=404,
            detail=f"No se encontró resultado para session_id '{session_id}'. "
                   "Puede haber expirado si el servidor reinició. Vuelve a subir el archivo."
        )
    return entry["result"]
...
@app.get("/sessions")
def list_sessions():
    """Lista los session_ids disponibles en memoria (útil para debug)."""
    return [
        {
            "session_id":  k,
            "filename":    v["filename"],
            ...
            "start_time":  v["result"]["session"].get("start_time"),
        }
