...
def service_worker():
    ...
    sw_code = """const CACHE='bitacora-v8';
...
async function loadHome(){try{const [d,w,mp]=await Promise.all([fetch(API+'/gpt/dashboard').then(r=>r.json()),fetch(API+'/gpt/wellness-summary?weeks=1').then(r=>r.json()).catch(()=>({})),fetch(API+'/gpt/mars-context').then(r=>r.json()).catch(()=>({}))]);const a=d.athlete||{},s=d.semana_actual||{},z=d.z2_check||{};$('home-data').innerHTML=`<div class="grid2">${metric('Km semana',Number(s.km||0).toFixed(0),'km')}${metric('Horas',Number(s.horas||0).toFixed(1),'h')}${metric('Sesiones',s.sesiones||0,'semana')}${metric('Z2',Number(z.pct_z2_4_semanas||0).toFixed(0)+'%','4 semanas')}</div>${mp&&mp.context_msg?'<div class="card" style="font-size:11px;color:var(--muted);line-height:1.6">'+mp.context_msg+'</div>':''}<div class="card"><div class="head"><h3>Estado del atleta</h3><span>${a.fitness||'—'}</span></div>${row('·','Fitness',d.recommendation||'Sin recomendación',a.mars_index?Number(a.mars_index).toFixed(1):'—')}${row('·','Fatiga',a.fatiga||'—','')}${row('·','Molestias activas',(w.molestias_activas||[]).length?'Revisar Wellness':'Sin alertas',(w.molestias_activas||[]).length||'✓')}</div>`}catch(e){$('home-data').innerHTML='<div class="card">Error: '+e.message+'</div>'}}
